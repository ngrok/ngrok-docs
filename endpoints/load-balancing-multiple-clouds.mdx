...
ngrok's [Endpoint Pooling](/endpoints/endpoint-pooling) allows you to automatically load balance between replicas of your service across multiple clouds by creating two endpoints with the same URL.
...
Start with a single [Agent Endpoint](/endpoints/agent-endpoints/) on an ngrok URL, or a [reserved domain](https://dashboard.ngrok.com/domains), with the `--pooling-enabled` flag on.
...
Pooling works with [internal Agent Endpoints](/endpoints/internal-endpoints/), which can only receive traffic from other endpoints associated with your ngrok account.

A [Cloud Endpoint](/endpoints/cloud-endpoints/) is a persistent public endpoint where you can manage a Traffic Policy for any upstream endpoints it forwards traffic to.
