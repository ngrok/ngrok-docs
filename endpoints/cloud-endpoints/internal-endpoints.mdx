...
- Use the `internal` [binding](/endpoints/bindings/)
...
A common practice is to use a [public](/endpoints/public-endpoints) [Cloud Endpoint](/endpoints/cloud-endpoints/) to manage a centralized Traffic Policy, then forward traffic to one or more internal [endpoints](/endpoints/). See [the guide on using Cloud Endpoints with internal endpoints](/endpoints/cloud-endpoints/forwarding-and-load-balancing//) to learn more.
...
- **Scheme** - Internal endpoints support all [Endpoint Protocols](/endpoints/protocols) (`http`, `https`, `tcp` and `tls`).
...
This means that, two different accounts may have internal endpoints with the same URL (for example, `https://api.internal`) without conflict or interference in [endpoint pooling](/endpoints/endpoint-pooling). As a result, there is no need to reserve an internal domain.
...
- Internal endpoints support [Endpoint Pooling](/endpoints/endpoint-pooling).
...
Internal endpoints are available on the Pay-as-you-go plan. Consult the [Endpoints Pricing](/endpoints/#pricing) documentation for billing details.
