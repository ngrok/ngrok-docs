...
When only a single endpoint remains in a pool, it is no longer pooled with others, but the endpoint remains poolable and can accept additional entries.
...
endpoints](/endpoints/cloud-endpoints) are considered to be online in
