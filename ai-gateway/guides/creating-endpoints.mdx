...
<Tip>
  This guide shows how to create an AI Gateway endpoint manually. For a simpler,
  faster setup, follow [the dashboard quickstart guide](/ai-gateway/quickstart)
  instead.
</Tip>
...
<Note>
  After saving, your endpoint will appear in the [AI
  Gateways](https://dashboard.ngrok.com/ai-gateways) section of the dashboard
  since it uses the `ai-gateway` action.
</Note>
...
<Tip>
  When using an Agent Endpoint as an AI Gateway, the `8080` port doesn't matter
  since the AI Gateway action handles all requests. You can use any port number.
</Tip>
...
<Tip>
  Once you [configure server-side API keys](#configuring-server-side-api-keys),
  clients won't need to provide their own keys.
</Tip>
...
<Warning>
  When you configure server-side API keys, your endpoint becomes publicly
  accessible. Anyone with the URL can make requests using your keys. See
  [Securing Your Gateway](/ai-gateway/guides/securing-endpoints) to add
  authorization.
</Warning>
