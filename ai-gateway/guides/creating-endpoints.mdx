...
<Tip>
  This guide shows how to create an AI Gateway endpoint manually.
  For a simpler, faster setup, follow [the dashboard quickstart guide](/ai-gateway/quickstart) instead.
</Tip>
...
<Tip>
  When using an Agent Endpoint as an AI Gateway, the `8080` port doesn't matter since the AI Gateway action handles all requests.
...
<Warning>
  When you configure server-side API keys, your endpoint becomes publicly accessible.
  Anyone with the URL can make requests using your keys.
  See [Securing Your Gateway](/ai-gateway/guides/securing-endpoints) to add authorization.
</Warning>
