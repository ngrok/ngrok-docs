...
<Note>Cloud Endpoints require a terminal action such as `deny`, `custom-response`, `redirect`, or `forward-internal` to complete the request. See [Cloud Endpoints](/universal-gateway/cloud-endpoints#differences-from-agent-endpoints) for more details.</Note>
