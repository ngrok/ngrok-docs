...
[Rotating provider keys](/api-reference/provider-keys/rotate) lets you update its value without changing its ID, so anything already using it keeps working. This means you can keep your existing configurations as they are.

<Steps>
  <Step title="Rotate the key">
    Update the key by sending its new value to the [Provider Keys API](/api-reference/provider-keys/rotate), using your [ngrok.ai API key](/ai-gateway/concepts/access-keys#credentials-overview):
