...
<Note>
  When accessing API key metrics, always check if the key exists first using
  `'key_id' in m.metrics.api_keys`. See [Metrics
  Reference](/ai-gateway/observability/metrics#quota-metrics-api-key-scope-only)
  for details.
</Note>
## CEL operators
