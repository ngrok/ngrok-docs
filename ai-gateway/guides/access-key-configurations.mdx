---
title: Access Key Configurations
sidebarTitle: Access Key Configurations
description: Scope providers and models and attach provider credentials to access keys.
---

Every request to `https://gateway.ngrok.ai` must include an [access key](/ai-gateway/concepts/api-keys). Upstream provider credentials are managed in [app.ngrok.ai](https://app.ngrok.ai) and injected server-side based on the access key's **configuration**—see [Access keys vs provider keys](/ai-gateway/concepts/api-keys#access-keys-vs-provider-keys).
...
See [Create a configuration](#create-a-configuration) below, or the [Access Key Configurations API](/api-reference/access-key-configurations/create).
...
Create configurations with the [Access Key Configurations API](/api-reference/access-key-configurations/create). Authorize the request with your [AI Gateway API key](/ai-gateway/concepts/api-keys#credentials-overview).
...
See the [Access Key Configurations API reference](/api-reference/access-key-configurations/create) and [Access Keys API](/api-reference/access-keys/update).
...
- [Access Keys](/ai-gateway/concepts/api-keys): Create keys and assign configurations
- [Provider Keys](/ai-gateway/guides/attaching-provider-keys): Store upstream credentials
- [Model selection](/ai-gateway/guides/model-selection-strategies): Choose models in requests
