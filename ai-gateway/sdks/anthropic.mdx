...
The AI Gateway is compatible with Anthropic's official SDKs. Simply change the `baseURL` to your gateway endpoint to get automatic failover, key rotation, and observability.

The gateway will forward requests for Anthropic SDKs unchanged in passthrough mode, making this a good option when interested accessing new Anthropic features immediately.

<Warning>
Failover is limited to providers that support the Anthropic Claude API.  Currently in the ngrok model catalog Anthropic is the only provider.
</Warning>
