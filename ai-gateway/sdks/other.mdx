...
The AI Gateway works with any SDK or HTTP client that supports OpenAI API or Anthropic Claude API format.
If your language or framework isn't covered in the dedicated guides, you can still use the gateway.
...
| Endpoint | API Format | Description | | ---------------------- | ---------- | --------------------- | | `/v1/chat/completions` | OpenAI | Chat completions | | `/v1/completions` | OpenAI | Legacy completions | | `/v1/embeddings` | OpenAI | Text embeddings | | `/v1/models` | OpenAI | List available models | | `/v1/messages` | Anthropic | Messages |
