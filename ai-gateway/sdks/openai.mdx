...
The AI Gateway is fully compatible with OpenAI's official SDKs. Simply change the base URL configuration option for the OpenAI SDK to route all requests through your gateway, get automatic failover, key rotation, and observability.
...
- **BYOK (passthrough mode)**: Use your provider API key (for example, `sk-...` from OpenAI). See [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview).
...
- [Configuring Providers](/ai-gateway/bring-your-own-keys/configuring-providers) - Set up providers and keys
