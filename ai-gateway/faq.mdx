...
Currently **OpenAI** and **Anthropic** are supported with ngrok-managed keys. For other providers (Google, DeepSeek, etc.), use [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview). This list may grow over time.
...
Prepaid balance that funds all AI Gateway usage. For AI Gateway API Keys, credits cover both the processing fee and upstream provider costs. For BYOK, credits cover the processing fee only ($0.05 per million tokens). See [Credits](/ai-gateway/concepts/credits).
...
**All** AI Gateway usage stops—both AI Gateway API Key requests and BYOK requests. Clients receive HTTP 403 with error [`ERR_NGROK_4026`](/errors/err_ngrok_4026). [Purchase more credits](https://dashboard.ngrok.com/ai-gateways/credit/add) to restore access.
...
Yes. When you use [AI Gateway API Keys](/ai-gateway/concepts/api-keys), ngrok manages the provider keys for you—no provider accounts needed. Currently supported for OpenAI and Anthropic. For other providers, you [bring your own keys](/ai-gateway/bring-your-own-keys/overview).
...
Not if you're using AI Gateway API Keys—ngrok handles provider keys for you. If you want to use your own provider keys, see [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview).

---

### How do I secure my gateway when using my own provider keys (BYOK)?

When you configure your own provider keys in the gateway, your endpoint becomes publicly accessible. You need to add your own authorization layer. See [Securing Endpoints (BYOK)](/ai-gateway/bring-your-own-keys/securing-endpoints) for complete examples.
...
**BYOK provider keys**: Add the new key, deploy, then remove the old key. See [Managing Provider Keys](/ai-gateway/bring-your-own-keys/managing-provider-keys#key-rotation) for details.
...
Yes. All AI Gateway requests incur a processing fee of **$0.05 per million tokens**, deducted from your [prepaid credits](/ai-gateway/concepts/credits). When using AI Gateway API Keys, credits also cover the upstream provider fee (ngrok pays OpenAI/Anthropic on your behalf). When using BYOK, you pay providers directly and only the processing fee is deducted from credits.
