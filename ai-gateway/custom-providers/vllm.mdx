...
vLLM exposes an OpenAI-compatible server. Expose it with an ngrok internal endpoint (or use a public HTTPS URL), register it as a custom provider, and attach credentials through an access key configuration.
...
<Steps>
  ...
  <Step title="Expose with ngrok">
    Create an [internal endpoint](/ai-gateway/concepts/custom-providers#self-hosted-on-a-local-network):
  ...
  <Step title="Create the custom provider">
    See [Create a custom provider](/ai-gateway/concepts/custom-providers#create-a-custom-provider). Use provider ID `vllm`, base URL `https://vllm.internal`, API format **OpenAI Chat Completions**, and your model IDs.
  </Step>
...
- [Custom Providers](/ai-gateway/concepts/custom-providers): URL requirements and configuration
