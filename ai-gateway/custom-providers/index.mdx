...
<Warning>
  HTTP URLs are only allowed for ngrok `.internal` endpoints. All external URLs
  must use HTTPS.
</Warning>
...
<Note>
  The first strategy that returns models wins. If your custom provider has
  matching models, only those are tried. OpenAI is only used if no custom
  provider models match. For cross-provider failover when requests fail, have
  clients specify multiple models: `models: ["my-provider:llama3",
  "openai:gpt-4o"]`.
</Note>
...
<CardGroup cols={2}>
  <Card
    title="Ollama"
    icon="terminal"
    href="/ai-gateway/custom-providers/ollama"
  >
  ...
  <Card
    title="LM Studio"
    icon="desktop"
    href="/ai-gateway/custom-providers/lm-studio"
  >
  ...
  <Card
    title="Azure OpenAI"
    icon="microsoft"
    href="/ai-gateway/custom-providers/azure-openai"
  >
