...
<Note>
  ...
  If LM Studio has matching models, only those are tried.
  OpenAI is only used if no LM Studio models match.
  For cross-provider failover when requests fail, have clients specify multiple models: `models: ["lm-studio:llama-3.2-3b-instruct", "openai:gpt-4o"]`.
</Note>
