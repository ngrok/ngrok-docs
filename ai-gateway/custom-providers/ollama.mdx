...
[Ollama](https://ollama.ai) runs open-source large language models locally. Connect it to the AI Gateway as a [custom provider](/ai-gateway/concepts/custom-providers).
...
- An [access key](/ai-gateway/concepts/api-keys) from [app.ngrok.ai](https://app.ngrok.ai)
...
<Steps>
  ...
  <Step title="Expose Ollama with ngrok">
    Create an [internal endpoint](/ai-gateway/concepts/custom-providers#self-hosted-on-a-local-network) with the [ngrok agent](/agent/):
    ...
    <Note>
    Internal endpoints (`.internal` domains) are private to your ngrok account—not reachable from the public internet. Use the same ngrok account here and in the AI Gateway, otherwise the gateway can't reach the endpoint.
    </Note>
  </Step>

  <Step title="Create the custom provider">
    See [Create a custom provider](/ai-gateway/concepts/custom-providers#create-a-custom-provider). Use provider ID `ollama`, base URL `https://ollama.internal`, and your model IDs (for example `llama3.2`).
...
- [Custom Providers](/ai-gateway/concepts/custom-providers): URL requirements and local networking
