...
2. **The gateway selects which model(s) to try** based on your configuration
...
on_http_request:
  - type: ai-gateway
    config:
      ...
      model_selection:
        strategy:
          # Prefer your self-hosted model
          - "ai.models.filter(m, m.provider_id == 'ollama')"
          # Fall back to cheapest cloud option
          - "ai.models.sortBy(m, m.pricing.input)"
```

Each strategy in the list is evaluated in order. The first one that returns models is used. This lets you define sophisticated routing logic:
...
When a request fails, the gateway automatically retries. This happens transparently - your app just sees a successful response (or a final error if all options are exhausted).
...
| `total_timeout` | 5m | Max time including all retries |
...
on_http_request:
  - type: ai-gateway
    config:
      ...
      total_timeout: "3m"          # But don't retry forever
      providers:
        - id: openai
          api_keys:
            - value: ${secrets.get('openai', 'api-key')}
