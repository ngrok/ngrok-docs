...
When `only_allow_configured_providers: true`, only providers explicitly listed in the `providers` array are allowed. Requests to any other provider are rejected, regardless of whether the SDK sends its own API key:
...
With this configuration, requests to providers not in the list (like `anthropic`) will fail with a clear error. API keys have no impact on this behavior—it's purely about which providers you've configured.
