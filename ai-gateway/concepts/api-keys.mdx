...
2. Send a request to the [Access Keys API](/api-reference/access-keys/create), authorized with that key:
...
Malformed or deleted access keys are rejected immediately. See [Error Codes](/ai-gateway/reference/error-codes).
...
| **Your provider keys** | Upstream credentials—see [Provider Keys](/ai-gateway/guides/attaching-provider-keys) |
