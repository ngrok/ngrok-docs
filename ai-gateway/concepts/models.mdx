...
Model names are not case-sensitive: `gpt-4o`, `GPT-4o`, and `Gpt-4O` all resolve to the same model. Provider names are matched exactly, so use the lowercase **Provider ID** from the [model catalog](/ai-gateway/reference/model-catalog).
