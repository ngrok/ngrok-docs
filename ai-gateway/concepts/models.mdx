...
All model, provider, and author names are matched **case-insensitively**. For example, `gpt-4o`, `GPT-4o`, and `Gpt-4O` all resolve to the same model. See the [Aliases reference](/ai-gateway/reference/model-catalog#aliases-reference) for a complete list of built-in aliases.
