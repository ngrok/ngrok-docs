...
<Note>
  Unknown models bypass catalog validation—the provider will return an error if the model doesn't exist.
  Unknown models also lack metadata (pricing, capabilities) for model selection strategies.
</Note>
...
<Note>
  ...
  You cannot use an alias that is already in use as an ID or alias by another model.
</Note>
