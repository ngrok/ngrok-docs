...
You can manage providers at [app.ngrok.ai](https://app.ngrok.ai). See [Configuring providers](/ai-gateway/guides/configuring-providers) for built-in and custom setup. The dashboard separates **built-in** catalog providers from **custom** providers you define.

## Built-in providers

Built-in providers are public AI APIs with a known model catalog: OpenAI, Anthropic, Google, Groq, and others listed in the [model catalog](/ai-gateway/reference/model-catalog). You can send requests with an [access key](/ai-gateway/concepts/api-keys). Your access key authenticates you to the gateway, not to the upstream provider.
...
For other built-in providers, [add a provider key](/ai-gateway/guides/attaching-provider-keys#add-a-provider-key) to bring your own key. Upstream costs go to your provider account; credits still cover the gateway processing fee. This is generally referred to as BYOK (bring your own key).
...
Custom providers are upstream endpoints you define—self-hosted Ollama or vLLM, a private deployment, or any OpenAI- or Anthropic-compatible API that isn't in the built-in catalog. See [Create a custom provider](/ai-gateway/concepts/custom-providers#create-a-custom-provider).

Custom providers require a provider key when the upstream needs authentication. See [Custom providers](/ai-gateway/concepts/custom-providers) for setup.
