...
- Automatic retries and metrics are still enabled
...
Custom providers must expose an OpenAI-compatible API. See the [Custom Providers guide](/ai-gateway/custom-providers) for detailed setup instructions.

## Provider Selection

When a request arrives, the gateway determines which provider to use:

1. **Explicit provider** - If the model name includes a provider prefix (e.g., `openai:gpt-4o`), that provider is used
