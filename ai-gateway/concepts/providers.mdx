---
title: Providers
description: Understanding AI providers in the AI Gateway.
---

Providers are the AI services that host and serve models. The AI Gateway routes your requests to providers and handles authentication, failover, and observability. Some providers support [ngrok-managed keys](/ai-gateway/concepts/api-keys)—you don't need your own accounts for these.

## Built-in providers

The AI Gateway includes built-in support for these providers:

| Provider | Description | ngrok-managed Keys |
|----------|-------------|-------------------|
| **OpenAI** | GPT models, o1/o3 reasoning models | ✅ |
| **Anthropic** | Claude models (supports both OpenAI and Anthropic Claude API formats) | ✅ |
| **Groq** | LPU-accelerated open-source models (Llama, Mixtral) | ❌ |
| **Google** | Gemini models | ❌ |
| **DeepSeek** | DeepSeek Chat and Reasoner models | ❌ |
| **OpenRouter** | Multi-provider routing service | ❌ |
| **Hyperbolic** | Open-source model hosting | ❌ |
| **InceptionLabs** | Diffusion-based language models | ❌ |
| **Inference.net** | Distributed inference network | ❌ |

Providers with ngrok-managed keys work with [AI Gateway API Keys](/ai-gateway/concepts/api-keys)—no provider accounts needed. For other providers, use [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview).

For the complete list of models available from each provider, see the [Model Catalog](/ai-gateway/reference/model-catalog).

## Operating modes

### ngrok-managed mode (recommended)

When you use an [AI Gateway API Key](/ai-gateway/concepts/api-keys), the gateway automatically authenticates with supported providers using ngrok's own keys:

```yaml
on_http_request:
  - type: ai-gateway
    config: {}  # No provider configuration needed
```

Your application authenticates with the AI Gateway API Key, and ngrok handles provider authentication automatically. No secrets or provider configuration required.

### Passthrough mode (default)

When no providers are configured and the client does not send an AI Gateway API Key, the gateway operates in **passthrough mode**:

- All built-in providers are available
- Your SDK's API key is forwarded directly to the provider
- Failover across models and providers is still enabled
- Metrics and observability are still enabled

```yaml
on_http_request:
  - type: ai-gateway
    config: {}  # No configuration needed
```

The gateway distinguishes ngrok-managed mode from passthrough mode by the key format: `ng-*-g1-*` keys trigger managed mode; any other value is treated as a provider key and forwarded as-is.

### BYOK mode

When you [bring your own provider keys](/ai-gateway/bring-your-own-keys/overview), you configure them explicitly in your Traffic Policy:

- Specify which providers are allowed
- Manage API keys centrally
- Add custom metadata for tracking
- Configure custom or self-hosted providers

```yaml
on_http_request:
  - type: ai-gateway
    config:
      providers:
        - id: "openai"
          api_keys:
            - value: ${secrets.get('openai', 'key')}
```

## Custom providers

You can add any OpenAI or Anthropic Claude API compatible endpoint as a custom provider:

```yaml
providers:
  - id: "my-ollama"
    base_url: "https://ollama.internal.company.com"
    models:
      - id: "llama3"
      - id: "mistral"
```

Custom providers must expose a supported API. See the [Custom Providers guide](/ai-gateway/custom-providers) for detailed setup instructions.

## Provider selection

When a request arrives, the gateway determines which provider to use:

1. **Explicit provider** - If the model name includes a provider prefix (for example, `openai:gpt-4o`), that provider is used
2. **Catalog lookup** - The gateway looks up the model in its catalog to find the default provider
3. **Filtering the model catalog** - The gateway ensures the provider supports the API format the request is using
4. **Selection strategy** - If configured, [model selection strategies](/ai-gateway/guides/model-selection-strategies) can override the default

## Next steps

- [AI Gateway API Keys](/ai-gateway/concepts/api-keys) - Use ngrok-managed keys for the simplest setup
- [Models](/ai-gateway/concepts/models) - Understanding model naming and request formats
- [Configuring Providers](/ai-gateway/bring-your-own-keys/configuring-providers) - Detailed configuration guide

