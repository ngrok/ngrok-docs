...
AI Gateway credits are a prepaid balance you buy in [app.ngrok.ai](https://app.ngrok.ai). Every request through `https://gateway.ngrok.ai` spends credits.
...
You don't need a specific ngrok subscription plan to use the AI Gateway or purchase credits. Sign in at [app.ngrok.ai](https://app.ngrok.ai), buy credits, and create an [access key](/ai-gateway/concepts/api-keys).
...
If you want ngrok platform features alongside the AI Gateway—team members, or [internal endpoints](/ai-gateway/concepts/custom-providers) to reach self-hosted models—you manage those through [dashboard.ngrok.com](https://dashboard.ngrok.com). That usually means a ngrok plan such as [Pay-as-you-go](https://ngrok.com/pricing). Using the gateway with credits alone does not.

## Purchasing credits

Open the [Credits page](https://app.ngrok.ai/credits) and purchase at least USD $5.00.
...
|---------|-----------------|------------------|
| Built-in OpenAI or Anthropic (ngrok.ai inference) | Processing fee and upstream model cost | — |
...
Every successful request through the gateway incurs a processing fee at **$0.05 per million tokens** (USD). The fee is based on the tokens in your request and response. It applies to all traffic—ngrok.ai inference, your own [provider keys](/ai-gateway/guides/attaching-provider-keys), and [custom providers](/ai-gateway/concepts/custom-providers)—and does not vary by model or provider.
...
- [Access Keys](/ai-gateway/concepts/api-keys): Authenticate requests to the gateway
