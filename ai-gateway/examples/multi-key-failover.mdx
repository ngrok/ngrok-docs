...
Keys are tried **in the order they're listed**.
...
<Note>For cross-provider failover, clients must specify multiple models in the request or use a model selection strategy. Providers are tried in alphabetical order by default; use `model_selection.strategy` to specify a custom order.</Note>
