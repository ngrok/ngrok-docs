...
<Note>
  ...
  (`http_request_complete.v0`). AI-specific log fields (provider selection,
  retry tracking, token usage) are not currently available but are on our
  roadmap.
</Note>
...
<Steps>
  <Step title="Open Log Exporting">
    Go to [Log Exporting](https://dashboard.ngrok.com/log-exporting) in the
    ngrok Dashboard
  </Step>
  <Step title="Create a New Log Export">Click to create a new Log Export</Step>
  <Step title="Select Log Sources">
    Choose which log sources to capture (for example, `http_request_complete.v0`
    for HTTP traffic)
  </Step>
