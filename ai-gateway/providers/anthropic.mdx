...
This guide walks through using [Anthropic](https://anthropic.com) Claude models with the AI Gateway. Anthropic is a **built-in provider**, so ngrok.ai offers managed inference for it—use Claude models with no Anthropic account of your own, or bring your own key.

## Use ngrok.ai inference (no Anthropic account)

Use Claude models without your own Anthropic account—ngrok.ai inference supplies the upstream credentials and bills the model cost through credits.

<Steps>
  <Step title="Create an access key and add credits">
    Follow the [quickstart](/ai-gateway/quickstart) to create an [access key](/ai-gateway/concepts/api-keys) and purchase [credits](/ai-gateway/concepts/credits) in [app.ngrok.ai](https://app.ngrok.ai).
  </Step>
...
<Steps>
  <Step title="Create an access key">
    Follow the [quickstart](/ai-gateway/quickstart) to create an [access key](/ai-gateway/concepts/api-keys) in [app.ngrok.ai](https://app.ngrok.ai) if you don't have one yet.
  </Step>

  <Step title="Add your Anthropic key">
    [Add a provider key](/ai-gateway/guides/attaching-provider-keys#add-a-provider-key) for Anthropic. We store it encrypted, so it never has to live in your application or get shared around—and you can reuse it across as many access keys as you need.
  </Step>
...
See the [model catalog](/ai-gateway/reference/model-catalog#anthropic).

## Next steps

- [Access Keys](/ai-gateway/concepts/api-keys)
