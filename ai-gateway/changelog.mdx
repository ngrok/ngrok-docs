...
<Update label="2026-09-02" description="Response headers" tags={["General Availability"]}>
  ...
  All three are exposed through CORS, so browser clients can read them. They are ngrok's own headers, so managed-key and Bring Your Own Keys callers see the same values. See [Response headers](/ai-gateway/how-it-works#response-headers).

  The native Gemini `generateContent` and `streamGenerateContent` surface returns these headers too, where it previously returned only `Content-Type`. Bodies on that surface are still forwarded byte-for-byte.
</Update>
