...
Groq provides ultra-fast AI inference powered by their custom LPU (Language Processing Unit) hardware, offering the fastest token generation speeds for large language models.
...
All model, provider, and author names are matched **case-insensitively**. For example, `gpt-4o`, `GPT-4o`, and `Gpt-4O` all resolve to the same model.
...
Authors represent the organizations that create models. You can use author aliases in the `author/model` format.
