...
Explicitly specify a provider with the `provider:model` format (for example, `openai:gpt-4o`).

## Built-in providers

The catalog lists **built-in providers**—public AI APIs with known models and routing metadata. Manage them on the [Providers page](https://app.ngrok.ai/providers).

Send traffic with an [access key](/ai-gateway/concepts/api-keys). Your access key authenticates you to the gateway, not to the upstream provider.
...
For other built-in providers below, add a provider key in app.ngrok.ai to bring your own key. Upstream costs go to your provider account; credits still cover the gateway processing fee. Other products sometimes call this BYOK (bring your own key).

The **Provider key required** column marks whether you must add a provider key before the gateway can route traffic to that built-in provider.
