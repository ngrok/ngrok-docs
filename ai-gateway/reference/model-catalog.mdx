...
3. Routes the request to the appropriate provider endpoint—for [ngrok-managed providers](#providers-with-ngrok-managed-keys), ngrok injects the API key automatically; for [other providers](#providers-requiring-byok), you must have [configured your own keys](/ai-gateway/bring-your-own-keys/overview)
...
## Providers with ngrok-managed keys

These providers are available when using [AI Gateway API Keys](/ai-gateway/concepts/api-keys). ngrok manages the provider keys—no provider accounts needed.

<Note>
This list may grow over time. For providers not listed here, use [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview).
</Note>
...
| `gpt-4o-audio-preview` | GPT-4o Audio Preview | 128,000 | 16,384 | text, image, audio |
| `gpt-4o-mini-audio-preview` | GPT-4o Mini Audio Preview | 128,000 | 16,384 | text, image, audio |
...
## Providers requiring BYOK

These providers require you to [bring your own API keys](/ai-gateway/bring-your-own-keys/overview). They are not available with ngrok-managed keys.
