...
<ConfigField title="on_error" type="enum" defaultValue="halt">
  <p>Behavior when all retry attempts are exhausted.</p>
...
<ConfigField title="total_timeout" type="string" defaultValue="5m">
  <p>Maximum total time for all retry attempts across all providers and keys. Must be specified as a duration string (e.g., <code>"2m"</code>, <code>"90s"</code>).</p>
...
<ConfigField title="per_request_timeout" type="string" defaultValue="30s">
  <p>Timeout for a single request to a provider. Must be specified as a duration string (e.g., <code>"45s"</code>, <code>"1m"</code>).</p>
...
<ConfigField title="model_selection" type="object">
  <p>Strategy for selecting models using CEL expressions. When not specified, models are selected based on client request and availability.</p>
...
<ConfigField title="providers[].models[].input_modalities" type="array of strings">
  <p>Input types supported by the model (e.g., "text", "image", "audio").</p>
</ConfigField>
...
<ConfigField title="providers[].models[].supported_features" type="array of strings">
  <p>Features supported by the model (e.g., "tool-calling", "coding").</p>
</ConfigField>
