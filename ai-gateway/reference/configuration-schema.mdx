...
<Note>
When using [AI Gateway API Keys](/ai-gateway/concepts/api-keys), no provider configuration is needed — ngrok manages provider keys automatically. The `providers` and `api_keys` fields below are only required for [Bring Your Own Keys](/ai-gateway/bring-your-own-keys/overview) setups.
</Note>
