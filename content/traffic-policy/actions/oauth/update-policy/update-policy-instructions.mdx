import Email from "@content/traffic-policy/actions/oauth/examples/restrict-by-email-example.mdx";
import Name from "@content/traffic-policy/actions/oauth/examples/restrict-by-name-example.mdx";
import ShowScopesTip from "@content/traffic-policy/actions/oauth/update-policy/show-scopes-tip.mdx";

1. Access the [ngrok Dashboard Endpoints page](https://dashboard.ngrok.com/endpoints?sortBy=createdAt&orderBy=desc) and locate an existing endpoint you'd like to add this to or create a new one.
2. In your traffic policy, add the following configuration:

{props.policySnippet}

{props.showScopesTip && <ShowScopesTip />}

3. Click **Save** to validate and update your traffic policy.

#### Configure access control

Optionally, configure access control to your service by only allowing specific users or domains. For example:

{ props.showNameSnippet ? <Name /> : <Email />}
