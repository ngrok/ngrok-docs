...
Next, install the Operator 3 times, specifying different credentials and a different `ingressClass` for each to watch
...
```
TODO: Do you also need to set the controllername? Guessing no and it uses the release name as a part of it hopefully, but need to confirm.
...
In the previous example, `ingressClass` was used as a means of separating which resources an Operator watches cluster wide. Another common setup would be to install the Operator in a particular namespace and have it watch for resources (including `CRDs`) within just that namespace.
...
Next, install the Operator in 2 different namespaces setting `ingress.watchNamespace` on each to scope it to just handle resources in its own namespace.
