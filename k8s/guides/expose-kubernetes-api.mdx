...
**HTTPS endpoints** terminate TLS at ngrok. The `Authorization: Bearer <token>` header passes through transparently to the Kubernetes API server, so any token-based strategy works.
