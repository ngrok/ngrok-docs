...
<Tip>Kubernetes events usually expire after an hour, so you might want to restart the Operator to re-create the events while troubleshooting if the events happened more than an hour ago.</Tip>
...
The ngrok `IngressClass` is required to use `Ingress` resources.
Check that your `Ingress` uses the ngrok `IngressClass` like the following example.
...
The ngrok `GatewayClass` is required to use Gateway API resources.
If you do not already have it installed, you can create it with:
