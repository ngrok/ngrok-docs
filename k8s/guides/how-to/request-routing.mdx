---
title: Controlling Traffic Flow with Request Routing
sidebarTitle: Request Routing
---
...
<Tip>
	You can also include traffic policy configuration on your internal
	AgentEndpoint resources that will run after the CloudEndpoint forwards the
	request to them
</Tip>
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.url.path == '/example'"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
`
	</Tab>
...
namespace: default
spec:
hostnames: - example-hostname.ngrok.io
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.url.path.startsWith('/example')"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
`
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.method == 'POST'"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
```	</Tab>
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.url.path.startsWith('/')"
...
name: example-gateway
namespace: default
rules: - matches: - headers: - type: Exact
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.headers.exists_one(x, x == 'x-request-header') && req.headers['x-request-header'].join(',').matches('x-value-.\*')"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
```	</Tab>
...
name: example-gateway
namespace: default
rules: - matches: - headers: - type: RegularExpression
name: x-request-header
value: x-value-.\*
backendRefs: - group: ""
kind: Service
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.url.query_params.exists_one(x, x == 'my-query-param') && req.url.query_params['my-query-param'].join(',') == 'foo'"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
```	</Tab>
...
name: example-gateway
namespace: default
rules: - matches: - queryParams: - type: Exact
name: my-query-param
value: foo
backendRefs: - group: ""
kind: Service
...
trafficPolicy:
policy:
on_http_request: - expressions: - "req.url.query_params.exists_one(x, x == 'my-query-param') && req.url.query_params['my-query-param'].join(',').matches('value-.\*')"
actions: - type: forward-internal
config:
url: http://example-service.internal:80
```	</Tab>
...
name: example-gateway
namespace: default
rules: - matches: - queryParams: - type: RegularExpression
name: my-query-param
value: value-.\*
backendRefs: - group: ""
kind: Service
