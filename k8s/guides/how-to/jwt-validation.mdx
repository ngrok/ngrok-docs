...
🔒 Ensure only authenticated users can access your services. <br /> ⚡ Reduce backend load by rejecting unauthorized requests early. <br /> 🛡 Protect APIs from unauthorized access without modifying your upstream services. <br />
...
However, simply including a JWT in a request does not improve security.
It must be verified to ensure:
...
The following Traffic Policy configuration is an example configuration of the jwt-validation action.
For a more real-world example, check out the [Auth0](/integrations/jwt-validation/auth0) guide.
