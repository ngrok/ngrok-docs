...
| `conn.k8s.pod.metadata.annotations` | A map of pod annotations prefixed with `ngrok-`. See [Annotations](#annotations) below. | 1024 bytes |
...
| `conn.k8s.pod.metadata.error_message` | A human-readable error message providing additional detail when `conn.k8s.pod.error_code` is set. | N/A |
