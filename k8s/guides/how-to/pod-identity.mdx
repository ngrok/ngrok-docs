...
| `conn.k8s.pod.name` | The name of the originating pod. | 255 bytes |
| `conn.k8s.pod.namespace` | The namespace the originating pod belongs to. | 63 bytes |
| `conn.k8s.pod.annotations` | A map of pod annotations prefixed with `ngrok-`. See [Annotations](#annotations) below. | 1024 bytes |
| `conn.k8s.pod.error_code` | An error code set when pod identity could not be resolved. See [Handling errors](#handling-errors) below. | N/A |
| `conn.k8s.pod.error_message` | A human-readable error message providing additional detail when `conn.k8s.pod.error_code` is set. | N/A |
...
Only annotations prefixed with `ngrok-` are included in `conn.k8s.pod.annotations`. Annotations without this prefix are not surfaced. The combined size of all included annotations is subject to a 1024-byte cumulative limit. If this limit is exceeded, `conn.k8s.pod.error_code` will be set to `ERR_NGROK_28000` rather than a partial annotation map being returned.

To use an annotation in a Traffic Policy expression, add the `ngrok-` prefix to it in your pod spec:

```yaml
metadata:
  annotations:
    ngrok-tenant-id: "tenant-abc"
    ngrok-environment: "production"
```

You can then reference the annotation in a Traffic Policy expression:

```
conn.k8s.pod.annotations["ngrok-tenant-id"] == "tenant-abc"
```
...
on_tcp_connect:
  - expressions:
      - conn.k8s.pod.error_code != ""
    actions:
      - type: deny
  - expressions:
      - conn.k8s.pod.namespace != "my-namespace"
    actions:
      - type: deny
```
...
on_tcp_connect:
  - expressions:
      - conn.k8s.pod.error_code == "" && conn.k8s.pod.namespace != "my-namespace"
    actions:
      - type: deny
```
      ...
      on_tcp_connect:
        - expressions:
          ...
          actions:
            - type: deny
        - expressions:
            - conn.k8s.pod.namespace != "payments"
          actions:
            - type: deny
```
   </Tab>
      ...
      on_tcp_connect:
        - expressions:
          ...
          actions:
            - type: deny
        - expressions:
            - !(conn.k8s.pod.name in ["worker-a", "worker-b"])
          actions:
            - type: deny
```
      ...
      on_tcp_connect:
        - expressions:
          ...
          actions:
            - type: deny
        - expressions:
            - conn.k8s.pod.annotations["ngrok-environment"] != "production"
          actions:
            - type: deny
```
      ...
      on_tcp_connect:
        - expressions:
          ...
          actions:
            - type: deny
        - expressions:
            - conn.k8s.pod.namespace != "tenant1"
          actions:
            - type: deny
        - actions:
