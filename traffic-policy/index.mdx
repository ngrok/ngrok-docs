...
A Traffic Policy document is a YAML (or JSON) object. The top-level keys are **phase** names — `on_tcp_connect`, `on_http_request`, and `on_http_response`. Each phase holds an ordered list of **rules**, and each rule contains an optional list of **expressions** and a required list of **actions**.

```yaml
on_tcp_connect:           # Phase: runs when a TCP connection is established
...
on_http_request:          # Phase: runs when an HTTP request is received
  - expressions:
    ...
    actions:
      - type: oauth
        config:           # optional — action-specific configuration
          provider:
            name: google
  - actions:              # a rule with no expressions matches all traffic
      - type: add-headers
        config:
          headers:
            x-powered-by: "ngrok"

on_http_response:         # Phase: runs after the upstream service responds
  - actions:
      - type: add-headers
        config:
          headers:
...
For a detailed explanation of each field, see [Phase Rules](/traffic-policy/concepts/phase-rules).
