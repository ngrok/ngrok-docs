...
With Traffic Policy, you can block unwanted requests to your endpoints.
...
```json policy.json
{
  "on_http_request": [
    {
      "expressions": ["!req.url.path.contains('/robots.txt')"],
      "actions": [
        {
          "type": "forward-internal",
          "config": {
...
```json policy.json
{
  "on_http_request": [
    {
      "name": "Block specific bots by user agent",
      "expressions": ["req.user_agent.name in ['ChatGPT-User', 'GPTBot', 'OAI-SearchBot']"],
      "actions": [
        {
          "type": "deny",
          "config": {
...
Sometimes you may need to block requests originating from one or more countries to remain compliant with data regulations or sanctions.
This rule blocks requests based on the origin country using [ISO country codes](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes) with the following steps:
...
```json policy.json
{
  "on_http_request": [
    {
      "expressions": ["conn.geo.country_code in ['<COUNTRY-01>', '<COUNTRY-02>']"],
      "name": "Block traffic from unwanted countries",
      "actions": [
        {
          "type": "custom-response",
...
```json policy.json
{
  "on_http_request": [
    {
      "name": "Block large POST/PUT requests.",
      "expressions": ["req.method == 'POST' || req.method == 'PUT'", "req.content_length >= 1000"],
      "actions": [
        {
          "type": "custom-response",
          "config": {
...
In this example, the Algolia web crawler is exempted from any rate limiting configured on your site.
See [the IP Intelligence docs](/traffic-policy/variables/ip-intel/#ip-categories) for other bots and crawlers that are available.
