...
With Traffic Policy, we automatically parse out useful information from the request's `User-Agent` header and make them [available as variables](/traffic-policy/variables/req/#requser-agentname). You can use them to deliver tailored content to users based on their browser and device, or to customize or block for SEO crawlers and bots.
