...
- [**Phase Rules:**](/traffic-policy/how-it-works#phase-rules) The set of conditions and actions applied during specific phases to filter, manipulate, or manage traffic.
- [**Expressions:**](/traffic-policy/how-it-works#expressions) Conditions used in Phase Rules to determine when an action should be applied.
- [**Actions:**](/traffic-policy/how-it-works#actions) Logic that is triggered when a Phase Rule condition is met.
- [**CEL Interpolation:**](/traffic-policy/how-it-works#cel-interpolation) A way to dynamically evaluate variables and macros inside configuration values, such as URLs or headers.
