...
The valid formatting clauses are `%s` (string), `%d` (integer), `%f` (double with fixed-point precision), 
`%e` (double in scientific notation), `%b` (binary integer or boolean), `%x` (lowercase hex), `%X` (uppercase hex), and `%o` (octal).
