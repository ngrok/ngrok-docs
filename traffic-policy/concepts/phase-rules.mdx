...
A Traffic Policy document is a YAML (or JSON) object whose top-level keys are [phase](/traffic-policy/concepts/phases) names. Each phase contains a list of **rules**. Every rule may optionally carry a human-readable name and one or more CEL expressions, plus the list of actions that run when all expressions match.

```yaml
# ┌─ Phase names are the top-level keys.
# │  Include only the phases you need.
# │
...
on_http_request:          # runs when an HTTP request is received
  - name: "rule name"
    expressions:
      - "expression"
    actions:
...
on_http_response:         # runs after the upstream service responds
  - name: "rule name"
    expressions:
      - "expression"
    actions:
...
A policy only needs to include the phases it uses. Each phase holds an ordered list of rules, and each rule requires at least one action.
...
on_http_request:
  ...
  # Rule 3: no expressions — matches all remaining traffic
  - actions:
