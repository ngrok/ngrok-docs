...
import ActionVariablesDescription from "/snippets/traffic-policy/common/action-variables-description.mdx"; import NonTerminatingExplanation from "/snippets/traffic-policy/common/non-terminating-explanation.mdx"; import { ConfigField } from "/snippets/ConfigTable.jsx"; import { ConfigChildren } from "/snippets/ConfigChildren.jsx";
...
The following [Traffic Policy](/traffic-policy/) configuration is an example configuration of the `jwt-validation` action.
For a more real-world example, check out the [Auth0 guide](/integrations/jwt-validation/auth0/).
...
In this example, a request is sent to the API with a valid JWT token in the `Authorization` header with the `Bearer` prefix and getting back a `200 OK` response.
...
<ConfigField title="actions.ngrok.jwt_validation.tokens" type="array of objects">
  ...
  <Expandable label="child properties">
    ...
    <ConfigField title="tokens[i].payload" type="object">
      The payload portion of the JWT, parsed into a key-value map representing the claims.
    </ConfigField>

    <ConfigField title="tokens[i].signature" type="string">
      The signature portion of the JWT, encoded in base64, used for verifying token integrity.
    </ConfigField>
