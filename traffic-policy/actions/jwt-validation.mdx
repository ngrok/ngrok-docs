...
<ConfigField title="http" type="object" required={true}>
  ...
  Name of the header or body field where the JWT is expected (Example: <code>"Authorization"</code>).
...
<ConfigField title="actions.ngrok.jwt_validation.tokens" type="array of objects">
  ...
  Indicates whether the token passed verification.
  Returns{" "} <code>true</code> if verified successfully.
