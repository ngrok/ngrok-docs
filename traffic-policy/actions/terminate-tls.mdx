...
<Note>  Note that this generates a self-signed certificate, so if you would like to use a custom certificate that is trusted by modern operating systems and browsers you will need to use a trusted CA (for example, LetsEncrypt).
</Note>
...
<Note>  Replace the contents of `server_private_key` with the contents of `server.key` and `server_certificate` with `server.crt` respectively in the Traffic Policy to enable your custom certificate on your endpoint and `ca.crt` to trust the certificate when using `curl`.
</Note>
...
<Note>  These variables are also available on the `conn` object (see [HTTP Variables](/traffic-policy/variables) for examples).
</Note>
