...
<ConfigField
  title="min_version" type="string" required={false} defaultValue="1.2" > {" "} The minimum TLS version to support.
  Must be one of `1.2` or `1.3`.
</ConfigField>

<ConfigField
  title="max_version" type="string" required={false} defaultValue="1.3" > {" "} The maximum TLS version to support.
  Must be one of `1.2` or `1.3`.
</ConfigField>

<ConfigField
  title="server_private_key" type="string" required={false} cel={true} > {" "} The PEM-encoded private key for the server if using a custom certificate (must be specified with `server_certificate`).
</ConfigField>

<ConfigField
  title="server_certificate" type="string" required={false} cel={true} > {" "} The PEM-encoded certificate for the server if using a custom certificate (must be specified with `server_private_key`).
</ConfigField>

<ConfigField
  title="mutual_tls_certificate_authorities" type="array of strings" required={false} cel={true} > {" "} A list of PEM-encoded Certificate Authority certificates and/or Certificate Authority IDs that are trusted for mutual TLS authentication.
</ConfigField>
