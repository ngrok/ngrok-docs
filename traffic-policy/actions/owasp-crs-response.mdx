...
The Response action protects against data leaks or vulnerabilities in your upstream service's responses (e.g., exposed SQL error messages, sensitive data). The Request action protects against malicious client requests (e.g., SQL injection, XSS attacks).
