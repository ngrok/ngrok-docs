...
The `owasp-crs-response` action enables rule processing on HTTP responses from
your upstream service. To use rule processing to block malicious HTTP
_requests_, enable the [OWASP CRS Request
action](/traffic-policy/actions/owasp-crs-request/).

<Tip>
**Use both actions for comprehensive protection.** The Response action protects against data leaks or vulnerabilities in your upstream service's responses (e.g., exposed SQL error messages, sensitive data), while the Request action protects against malicious client requests (e.g., SQL injection, XSS attacks). Together, they provide defense-in-depth security for both incoming and outgoing traffic.
</Tip>
