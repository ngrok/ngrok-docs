...
If the forwarding is successful, the response from the upstream for the private endpoint will be sent back to the client making the original request. No further actions in the Traffic Policy will be executed and no traffic will be sent to the upstream for the public endpoint.
