...
To match requests that begin with `/products`, use [CEL Interpolation](/traffic-policy/how-it-works#cel-interpolation) and write `^${req.url.authority}/products`, or use an expression like `req.url.path.startsWith('/products')` in the phase rule's `expressions` field.
...
policy rule (no expression) by leveraging [CEL Interpolation](/traffic-policy/how-it-works#cel-interpolation).
...
this by leveraging [CEL Interpolation](/traffic-policy/how-it-works#cel-interpolation) to replace the
