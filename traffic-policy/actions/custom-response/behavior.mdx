...
immediately returns the configured response to the caller. Keep in min that because this action bypasses the upstream server, request bodies wil not be forwarded and will not be visible within the ngrok [Traffic Inspector](/obs/traffic-inspection/).
