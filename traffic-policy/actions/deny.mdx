...
import ActionVariablesDescription from "/snippets/traffic-policy/common/action-variables-description.mdx"; import TerminatingExplanation from "/snippets/traffic-policy/common/terminating-explanation.mdx";
...
<Tabs>
  ...
  <Tab title="on_http_request" label="on_http_request">
    ...
    To send back a response body, use the [`custom-response`](/traffic-policy/actions/custom-response) action.
...
<Tabs>
  <Tab title="on_tcp_connect" label="on_tcp_connect">
    ...
    The following [Traffic Policy](/traffic-policy/) configuration will deny all inbound traffic on your endpoint.
    ...
    In this example, a connection attempt to `5.tcp.ngrok.io:22984` using the `telnet` command and ngrok immediately closes the connection.
  ...
  <Tab title="on_http_request" label="on_http_request">
    ...
    The following [Traffic Policy](/traffic-policy/) configuration will deny all inbound traffic with a `404` on your endpoint.
    ...
    In this example, a connection attempt to `example.ngrok.app` using the `curl` command and ngrok immediately closes the connection with a `404` status code.
