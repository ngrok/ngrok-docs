...
<ConfigField title="enforce" type="boolean" required={true}>
	Default true. If false, continue to the next action even if the IP is not
	permitted.
</ConfigField>
...
<ConfigField title="ip_policies" type="array of refs" required={false}>
	List of [IP Policy](/api-reference/ippolicies/list) identifiers to be checked
	if the source IP is allowed access.
</ConfigField>
...
<ConfigField title="actions.ngrok.restrict_ips.matched_cidr" type="string">
	The CIDR block that matched the incoming client's IP address. If no match was
	found, this value will be empty.
</ConfigField>
