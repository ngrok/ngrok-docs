...
You can define rules using either **Allow and Deny lists**, or **Reference IDs** to existing ngrok [IP Policies](/api/resources/ip-policies/).
...
<ConfigField title="ip_policies" type="array of refs" required={false}>
  List of [IP Policy](/api/resources/ip-policies/) identifiers to be checked if the source IP is allowed access.
</ConfigField>
...
- The CIDRs belonging to the ngrok [IP Policies](/api/resources/ip-policies/)
...
action with the `restrict-ips` action and [IP Policies](/api/resources/ip-policies/).
...
[IP Policy](/api/resources/ip-policies/) `ipp_1yjqdrIBwgciY2I9zH2EelgBbJF`
