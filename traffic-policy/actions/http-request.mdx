...
<Note>
	External requests are supported but disabled by default. [Contact ngrok
	support](https://ngrok.com/support) to enable external access.
</Note>
...
<ConfigField title="actions.ngrok.http_request.error.code" type="string">
	The ngrok error code for an error that occurred during the invocation of an
	action.
</ConfigField>
