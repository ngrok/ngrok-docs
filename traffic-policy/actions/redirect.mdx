...
This action is different from the [URL Rewrite action](/traffic-policy/actions/rewrite/), which changes the request internally while keeping the client's URL unchanged. Redirects, by contrast, instruct the client to issue a new request to the new URL.
