...
Traffic Policy actions can be triggered at different [phases](/traffic-policy/concepts/phases) of the lifecycle of a request to your endpoint. Here are the supported phases for this action.
...
- **`on_http_request` phase**: Adds the specified headers to incoming http requests before reaching the upstream server.
- **`on_http_response` phase**: The specified headers are added to the http response from the upstream server.
