...
import BasicAuthSshExample from "/snippets/examples/ssh/http-basic-auth.mdx"; import NonLocalSshExample from "/snippets/examples/ssh/http-nonlocal.mdx"; import OAuthSshExample from "/snippets/examples/ssh/http-oauth-authn.mdx"; import RandomHttpSshExample from "/snippets/examples/ssh/http-random.mdx"; import StaticDomainSshExample from "/snippets/examples/ssh/http-static-domain.mdx"; import FixedTcpSshExample from "/snippets/examples/ssh/tcp-fixed.mdx"; import RandomTcpSshExample from "/snippets/examples/ssh/tcp-random.mdx"; import RandomTlsSshExample from "/snippets/examples/ssh/tls-terminate-at-upstream.mdx";
...
Normally you will connect to ngrok's closest point of presence via the [Global Load Balancer](/universal-gateway/global-load-balancer/), but you can also explicitly choose a region.
...
You'll first need to upload yours to the [SSH Public Keys page on your ngrok dashboard](https://dashboard.ngrok.com/tunnels/ssh-keys).
...
You may wish to consult `man ssh`, and the section devoted to the `-R` option for additional details. ngrok uses additional command line options to implement features that are not otherwise available via the `-R` syntax.
