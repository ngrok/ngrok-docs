...
# forward TCP traffic (for example, ssh) with an assigned public address
ngrok tcp 22
...
| `--oauth`                     | enforce authentication oauth provider on tunnel endpoint, for example, 'google'                      |
...
| `--oidc`                      | oidc issuer url, for example, https://accounts.google.com                                            |
...
| `--traffic-policy-file`       | path to traffic policy configuration YAML or JSON file, for example, './path/to/policy.yml'          |
| `--traffic-policy-url`        | URL to traffic policy configuration YAML or JSON file, for example, 'https://example.com/policy.yml' |
...
| `--verify-webhook`            | validate webhooks are signed by this provider, for example, 'slack'                                  |
...
Do not run this command if you installed ngrok with a package manager (such as
