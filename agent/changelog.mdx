...
- Deprecated flags that are made obsolete by Traffic Policies.
  Such flags can still be used, but are not shown in `--help` output and will display deprecation warnings if used.
...
To workaround this, you may specify an explicit http URL if you need the old behavior: `ngrok http http://localhost:443`.
