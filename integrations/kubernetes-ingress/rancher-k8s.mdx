...
- The [ngrok Kubernetes Operator](/k8s/) installed on
  your cluster. While you _can_ install the Operator via Rancher directly, this guide
  recommends using the official Helm chart.
...
1. The **Server URL** field will default to `https://localhost:444`, but your worker nodes won't be able to connect to
   ...
   ![Configure the Rancher installation URL](../img/rancher/img/rancher_install-url.png)

   When the Rancher dashboard loads, Rancher should have already deployed a single K3s-based cluster named `local`—click the cluster's name to explore. Rancher recommends that its server management and your workloads run on separate clusters, which is what you'll do next.
...
1. Create a new [RKE2](https://docs.rke2.io/) cluster by clicking **Create** in your Rancher dashboard home, then
   ...
   **NGINX Ingress**, as you'll add ngrok-based ingress in the next step. Click **Create** to initialize the cluster.
   ...
   ![Configure the RKE2 cluster](../img/rancher/img/rancher_cluster-config.png)

1. Register your Linux nodes with your RKE2 cluster. Leave the **Node Role** options at their defaults, and under the
   **Registration Command** heading and command example, click the **Insecure** checkbox.

   ![Configure the agent registration command](../img/rancher/img/rancher_cluster-agent.png)

   Once you copy-paste the command into your Linux node and execute it, your new cluster will begin bootstrapping the
   node. When Rancher finishes bootstrapping your nodes, you can navigate to the **Cluster Dashboard** for your RKE2
...
1. Set up `kubectl` to manage your RKE2 cluster. At the top of the Cluster Dashboard, click the **Copy KubeConfig to
   ...
   ![Copy the RKE2 cluster's configuration to your clipboard](../img/rancher/img/rancher_cluster-kubeconfig.png)
...
1. Create a new Kubernetes manifest (`tinyllama.yaml`) with the below contents. This manifest defines the [tinyllama](https://github.com/ngrok-samples/tinyllama) demo LLM application
...
1. Access your tinyllama demo app by navigating to your ngrok subdomain, for example, `https://one-two-three.ngrok.app`.
