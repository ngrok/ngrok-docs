...
<Note>
	Another viable option is to launch a single Linux virtual machine on your
	local workstation or with a cloud provider to host a K3s cluster for
	[installing Rancher with
	Helm](https://ranchermanager.docs.rancher.com/getting-started/quick-start-guides/deploy-rancher-manager/helm-cli).
	If you choose that option, you can skip ahead to [2. Install the ngrok Ingress
	Controller](#install-a-sample-application) once you’ve finalized your K3s
	cluster.
</Note>
...
1. Access your 2048 demo app by navigating to your ngrok subdomain, for example, `https://one-two-three.ngrok.app`.
