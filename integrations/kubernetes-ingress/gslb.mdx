...
- Edit the `ngrok.yml` [ngrok agent configuration file](/gateway/agent/config/v3/) with your `<YOUR_NGROK_AUTHTOKEN>` (find it in the [ngrok dashboard](https://dashboard.ngrok.com/get-started/your-authtoken)); it's different from the API key you created in the first step.
  All three VMs use the same `url` value, and ngrok automatically routes each incoming request to the nearest connected agent.
...
To route traffic by explicit weight across specific regions, such as for A/B tests or shifting capacity, update each VM's `ngrok.yml` to use a unique per-region internal domain instead of the shared one:
...
- Spread the load further by deploying additional VMs in more regions and connecting each agent to `https://gslb-demo.internal`. ngrok automatically includes them in load balancing.
...
- [Introducing Always-On Global Server Load Balancing](https://ngrok.com/blog-post/gslb-global-server-load-balancing)
