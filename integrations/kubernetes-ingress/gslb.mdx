...
<Tip>
  **Custom domains within ngrok:** Using CNAMEs, ngrok can host an endpoint on
  your domain while also managing the complete TLS certificate lifecycle on your
  behalf. If you'd prefer to use a custom domain rather than an ngrok-managed
  one, [follow the guide](/universal-gateway/custom-domains/) to set up your DNS
  and replace your custom domain in the `export` command above.
</Tip>
...
<Tip>
  Make sure you grab the right ID—the command starts with two ID values
  (`edge_id` and `id`). Be sure to use `id` with the value that starts with
  `edghtsrt_...`.
</Tip>
