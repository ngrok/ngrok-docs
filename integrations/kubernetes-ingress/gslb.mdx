...
You'll need to run the API request to the [`/backends/tunnel_group` endpoint](/api/resources/tunnel-group-backends/) for **each** of your VMs, changing the `<LOCATION_NAME>` with the name of the city or region of the data center where they're deployed (for example, `nyc`, `sydney`, and `frankfurt`).
