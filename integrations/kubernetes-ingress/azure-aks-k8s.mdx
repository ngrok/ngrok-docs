...
<Tip>  If you prefer the CLI, save the YAML below to a `.yaml` file on your local workstation and deploy with `kubectl apply -f ...`.
...
<Tip>  Edit line `9` of the YAML below (the `NGROK_DOMAIN` variable) with the ngrok subdomain you created earlier.
