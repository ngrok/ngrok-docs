...
Next, you'll configure and deploy the [ngrok Kubernetes Operator](https://github.com/ngrok/ngrok-operator) to expose your demo app to the public internet through ngrok.
...
Now that your demo app is publicly accessible through ngrok, you can quickly layer on [additional capabilities](/gateway/endpoints/http/), like authentication, without configuring and deploying complex infrastructure.
...
You can also apply the policy to just a specific backend or as the default backend for an `Ingress`—see the doc on using the [Operator with Ingresses](/gateway/k8s/guides/using-ingresses/#using-ngroktrafficpolicy-with-ingress).
...
For next steps, explore the [Kubernetes docs](/gateway/k8s/) for more details on how the Operator works, different ways you can integrate ngrok with an existing production cluster, or use more advanced features like [bindings](/gateway/k8s/guides/bindings/) or [endpoint pooling](/gateway/k8s/guides/pooling/).
