...
<Tip>
    If you have a free ngrok account, you can only have one ngrok agent active
    at a time; set the `replicaCount` parameter to `1` so the Operator works
    correctly.
...
<Tip>
    **Troubleshooting:** A common error is that certain APIs must be activated
    for your project (for example, GCP requires the Compute Engine API and an
    enabled billing account). After fixing errors, wait a few minutes for
    settings to propagate to Palette.
