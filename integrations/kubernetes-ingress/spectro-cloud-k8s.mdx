...
1. Click **Profiles** in the navigation, then **Add Cluster Profile**. In the **Basic Information** tab, give your profile a name, like `ngrokker`, and a version. For this guide, leave the **Type** as the default **Full**, then click **Next**.
   ...
   ![Add a cluster profile name and version](../img/spectro-cloud/img/add-profile.png)

1. In the **Cloud Type** tab, choose your provider.

   ![Choose a Kubernetes provider for your profile](../img/spectro-cloud/img/choose-provider.png)

1. In the **Profile Layers** tab, click your base OS pack—in most cases, there will be a single option, like `Container-Optimized OS` or `Linux`. This is where you would add customizations or additional manifests, but you can leave the settings at their default and click **Next layer**.

   ![Select packs and options](../img/spectro-cloud/img/select-pack.png)
...
1. Palette now shows you the layers of this new cluster profile. You can also add new Packs, manifests, or Helm charts, but to save your progress for now, click **Next**, then **Finish Configuration**.
   ...
   ![Finalize your cluster profile](../img/spectro-cloud/img/finalize-profile.png)
...
1. Click **Add New Pack**, then either search `ngrok`, or scroll down to the **Ingress** section, and click the **ngrok Kubernetes Operator** icon.

   ![Add the ngrok Kubernetes Operator pack](../img/spectro-cloud/img/add-ngrok-controller.png)
...
1. Scroll down in the default YAML supplied by the pack to the following configuration.
   ...
   ![Add ngrok secrets to the Palette profile](../img/spectro-cloud/img/add-ngrok-secrets.png)
<Tip>
...
1. Create a new cluster profile, name it `tinyllama`, and click the **Add-on** profile type.
...
1. Add another manifest to create a Kubernetes ingress service, which will inform the ngrok Kubernetes Operator to create a new Edge for your app. Name it `tinyllama-ingress` and create an `ingress` manifest with the following YAML content, replacing the `<NGROK_DOMAIN>` variable with the subdomain you created, which should look like `one-two-three.ngrok.app`.
   ...
   ![Finish creating 2048 profile](../img/spectro-cloud/img/finish-2048-profile.png)
...
1. Find the cluster profile you created in Step 1 and click **Next**.
   ...
   ![Choose your cluster profile](../img/spectro-cloud/img/choose-profile.png)

1. Add your additional profiles by clicking on the **+** next to **Addon layers**. Choose the ngrok Kubernetes Operator profile, then repeat for the tinyllama profile.
...
1. Next, create your node pool. Here, you can change the number of nodes in the pool and specify the instance type you'd like to use.
   ...
   ![Selecting the cluster's node pool](../img/spectro-cloud/img/node-pool.png)

1. Click **Finish Configuration** to create your cluster.

   ![The final visualization of the cluster with all profiles](../img/spectro-cloud/img/final-cluster-layers.png)
...
1. Once Palette reports your cluster as `Running`, you can open your browser and navigate to your `NGROK_DOMAIN` to see your example app as managed, deployed, and publicly networked via, respectively, Palette, Kubernetes, and ngrok.
   ...
   ![Viewing tinyllama](../img/spectro-cloud/img/final-2048-game.png)
