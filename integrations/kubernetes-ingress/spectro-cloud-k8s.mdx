...
<Steps titleSize="h2">
  <Step title="Create a cluster profile in Palette">
    ...
    If you already have a cluster profile, skip to the [Create your cluster with Palette](#create-your-cluster-with-palette) step.
    If you already have a functioning cluster in Palette, skip to the [Create an add-on cluster profile for an example app](#create-an-add-on-cluster-profile-for-an-example-app) step.
  ...
  <Step title="Create an add-on cluster profile for the ngrok Kubernetes Operator">
  ...
  <Step title="Create an add-on cluster profile for an example app">
  ...
  <Step title="Create your cluster with Palette">
...
Read the [Kubernetes Operator docs](/gateway/k8s/) for more details, including [how it works](/gateway/k8s/how-it-works/) and details on how to [apply an `NgrokTrafficPolicy` resource](/gateway/k8s/guides/using-ingresses/#using-ngroktrafficpolicy-with-ingress) to your `Ingress` to start using the [Traffic Policy system](/traffic-policy).
