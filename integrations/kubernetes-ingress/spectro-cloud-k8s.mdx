...
1. Scroll down in the default YAML supplied by the pack to the following configuration.
   ...
   <Tip>
   	**If you have a free ngrok account, you can only have one ngrok agent
   	active at a time. To ensure the ngrok Kubernetes Operator functions
   	properly in production, set the `replicaCount` parameter to `1`.**
   </Tip>
...
1. Choose the **Project** you'd like to create your new cluster within and pick a region.
   ...
   <Tip>
   	**A common error at this stage is you still need to activate certain APIs
   	for your project. For example, a project in GCP requires the Compute Engine
   	API, which in turn requires an enabled billing account. Once you fix these
   	errors, give the settings a few minutes to propagate to Palette, before
   	moving forward.**
   </Tip>
...
1. Once Palette reports your cluster as `Running`, you can open your browser and navigate to your `NGROK_DOMAIN` to see your example app as managed, deployed, and publicly networked via, respectively, Palette, Kubernetes, and ngrok.
   ...
   ![Viewing the final 2048 game](../img/spectro-cloud/img/final-2048-game.png)
