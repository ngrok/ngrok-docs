...
<Note>  These steps follow the Linkerd getting-started documentation for installing the service mesh.
...
<Tip>  Edit line `9` of the manifest below (the `NGROK_DOMAIN` variable) with your ngrok subdomain (for example, `one-two-three.ngrok.app`).
...
<Tip>  **Troubleshooting:** If you see an error when applying the manifest, double-check that you've updated the `NGROK_DOMAIN` value and try again.
