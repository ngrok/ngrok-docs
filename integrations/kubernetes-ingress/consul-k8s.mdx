...
For this guide, we'll need access to a remote or local Kubernetes cluster with Consul installed. If you have an existing cluster with Consul set up, you can skip this step and proceed to [2. Install the ngrok Kubernetes Operator](#install-the-ngrok-ingress-controller).
...
<Note>
	**These steps are based on Consul's Tutorial [Consul Service Discovery and
	Service Mesh on
