...
## Enable extra features of ngrok 

To demonstrate ingress configuration and [OAuth support](/gateway/traffic-policy/actions/oauth/), you can launch an observability stack (for example, Prometheus, Grafana, and Alertmanager) on your cluster.
...
Because ngrok handles ingress and middleware execution, and thanks to DigitalOcean's `doctl` tool, you can quickly deploy new clusters, apps, and helpful services without leaving your CLI.
...
An extensive explainer is available for [how the Operator works](/gateway/k8s/how-it-works/), plus additional [documentation](/gateway/k8s/), which also includes details on using the Kubernetes Gateway API.
