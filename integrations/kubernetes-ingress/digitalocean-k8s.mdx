...
Your Bookinfo app is running, but you have no means of accessing it from your local workstation—or anywhere else, for that matter.
You _could_ use `kubectl port-forward...`, but that's a fragile method of accessing your new app, and isn't appropriate for production use.
...
- Edit your `bookinfo-ingress.yaml` file again to add OAuth, leaving the previous configurations untouched. Note the new `annotations` field and the `NgrokTrafficPolicy` CR.
