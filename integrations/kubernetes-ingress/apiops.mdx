...
- A Kubernetes cluster. For a local cluster, use [minikube](https://minikube.sigs.k8s.io/docs/start/); this guide refers to your cluster as `<YOUR-CLUSTER>`.
- [Argo CD](https://argo-cd.readthedocs.io/en/stable/getting_started/) installed on your cluster, with the `argocd` CLI installed and logged in on your local workstation.
...
- A reserved domain from the ngrok [dashboard](https://dashboard.ngrok.com/domains) or [API](/api-reference/reserveddomains/list). You can use an ngrok subdomain or a custom branded domain like `https://api.example.com`; this guide refers to it as `<NGROK_DOMAIN>`.

## Deploy Argo CD

This guide uses Argo CD to sync your Git repository to your cluster. If you don't have it yet, follow Argo CD's [getting started guide](https://argo-cd.readthedocs.io/en/stable/getting_started/) to install it on your cluster and log in with the `argocd` CLI.

Confirm Argo CD is running before continuing:
...
If you have an existing API and GitOps configuration, you can [skip to Deploy your demo API with Argo CD](#deploy-your-demo-api-with-argo-cd) and adapt the Argo CD CLI commands to your Git repository.

1. Reserve a static domain from the ngrok [dashboard](https://dashboard.ngrok.com/domains) or [API](/api-reference/reserveddomains/list).
   This static domain (for example, `example.ngrok.app`) is your `NGROK_DOMAIN` for the remainder of this guide.
...
The [Traffic Policy module](/gateway/traffic-policy/) can be used alongside the Kubernetes Gateway API.
...
The project includes a basic example in [`traffic-policy.yaml`](https://github.com/ngrok-samples/apiops-demo/blob/main/traffic-policy.yaml).
The steps below add rate limiting with the [`rate-limit` action](/gateway/traffic-policy/actions/rate-limit), a common first step in protecting an API from abuse.
...
3. Add, commit, and push this change to your Git repository.
   ...
   Once the ngrok Kubernetes Operator picks up those changes, it will push them, and you can verify them directly in your [ngrok dashboard](https://dashboard.ngrok.com/endpoints).
...
You can use this combination to deploy real-world APIs behind a production-grade gateway without complex configuration or extra external infrastructure.
...
For more Traffic Policy options, see the [Auth0 JWT integration](/integrations/jwt-validation/auth0/) and the [Traffic Policy docs](/gateway/traffic-policy/) for the full list of actions and syntax.
