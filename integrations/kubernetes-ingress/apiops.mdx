...
- A Kubernetes cluster.
  For a local cluster, use [minikube](https://minikube.sigs.k8s.io/docs/start/); this guide refers to your cluster as `<YOUR-CLUSTER>`.
...
- A reserved domain from the ngrok [dashboard](https://dashboard.ngrok.com/domains) or [API](/api-reference/reserveddomains/list).
  You can use an ngrok subdomain or a custom branded domain like `https://api.example.com`; this guide refers to it as `<NGROK_DOMAIN>`.

## Deploy Argo CD

This guide uses Argo CD to sync your Git repository to your cluster.
If you don't have it yet, follow Argo CD's [getting started guide](https://argo-cd.readthedocs.io/en/stable/getting_started/) to install it on your cluster and log in with the `argocd` CLI.
