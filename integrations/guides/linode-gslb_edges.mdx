...
1. Three Ubuntu 24.04 virtual machines (VMs) in three globally distributed regions (for example, Atlanta, Osaka, and Frankfurt):
...
You'll need to run the API request to the [`/backends/tunnel_group` endpoint](/api-reference/tunnelgroupbackends/list) for **each** of your VMs, changing the `<LOCATION_NAME>` with the name of the city or region of the data center where they're deployed (for example, `atlanta`, `osaka`, and `frankfurt`).
...
Create a Weighted Backend at the [`/backends/weighted` endpoint](/api-reference/weightedbackends/list) using your Tunnel Group backend `id`s exported from the previous step. The `curl` command below creates equal weighting for your backends, but you can use integers between `0-10000` to configure precise proportional weights.
...
You can now create an catch-all Edge Route at `/` using your Weighted Backend using the [`/edges/https/{EDGE_ID}/routes` endpoint](/api-reference/edgeshttpsroutes/get).
...
![An example of an ngrok domain and Edge with no matching tunnels](../img/linode/img/edge-no-tunnels.png)
...
![alt text](../img/linode/img/edge-dashboard-no-tunnels.png)
...
![The ngrok dashboard showing online, weighted tunnels](../img/linode/img/edge-weighted-active.png)
...
- Using `curl` with the ngrok API's [`/edges/https/{edge_id}/routes/{id}/policy` endpoint](/api-reference/edgeroutetrafficpolicymodule/get). For this, you will need find the `id` of the catch-all Route you created in [step 4](#create-weighted-backend).
