...
For ngrok to successfully send events into CloudWatch, a log group ARN is required. To
...
To send the events to CloudWatch, assign an Event Destination to the Log Export.
...
3. **Create IAM Role** - An IAM role is required to allow ngrok to stream logs into
   ...
   **API** or **CLI Script**, create the IAM role and provide the role ARN. For the
   fastest integration or proof of concept, use the **CLI Script**.
<Note>
