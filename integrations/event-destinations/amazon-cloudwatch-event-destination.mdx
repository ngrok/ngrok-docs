...
<Steps titleSize="h2">
  <Step title="Obtain CloudWatch log group ARN">
    For ngrok to send events to CloudWatch, a log group ARN is required.
    To create or retrieve the ARN, see the [AWS CloudWatch documentation for working with log groups and streams](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/Working-with-log-groups-and-streams.html).
    You may also want to reference the [AWS IAM documentation on using external IDs with cross-account roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html).
  </Step>

  <Step title="Create a log export">
