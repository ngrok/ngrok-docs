...
description: Use Auth0 OpenID Connect (OIDC) to secure access to ngrok endpoints
---
...
1. Enter `https://idp.ngrok.com/oauth2/callback` in the **Allowed Callback URLs** field, and enter the URL provided by the ngrok agent to expose your application to the internet in the **Application Login URI** field (that is, `https://myexample.ngrok.app`).
   ![Auth0 configuration](../img/auth0/img/ngrok_url_configuration_auth0.png)
...
1. Enter the following command to launch an ngrok tunnel with Auth0 SSO:
   ...
   **Note**: Replace the following with values:
   - Auth0_OAUTH_URL: The domain value you copied from Auth0, in the form of an URL (that is, `https://dev-abcd1234.us.auth0.com/`).
...
1. Click **Begin setup** and enter the following values into the fields:
   ![Auth0 config in ngrok](../img/auth0/img/auth0-1.png)
   - **Issuer URL**: The domain value you copied from Auth0, in the form of a URL (that is, `https://dev-abcd1234.us.auth0.com/`).
...
1. Click the **copy icon** next to the tunnel command.
    ![tunnel config](../img/auth0/img/auth0-2.png)
...
1. To confirm that the tunnel is connected to your edge:
    ...
    - Refresh the test edge page. Under traffic, you will see the message _You have 1 tunnel online. Start additional tunnels to begin load balancing._
      ![tunnel confirmed](../img/auth0/img/auth0-3.png)

1. In the test edge, copy the **endpoint URL**. (You use this URL to test the Auth0 Authentication)
    ![tunnel url](../img/auth0/img/auth0-4.png)

## 3. Test the integration 

1. In your browser, launch an incognito window.

1. Access your ngrok tunnel (that is, `https://auth0-sso-test.ngrok.app` or using the copied endpoint URL).
