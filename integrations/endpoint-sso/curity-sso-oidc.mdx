...
description: How to secure access to your ngrok endpoints with Curity Identity Server SSO (OpenID Connect)
---
...
1. #### Add an Authenticator
   ...
   1. Under **HTML Form Settings**, select the `default-account-manager` as account manager and `default-credential-manager` as credential manager.
      ![img/username-03.jpg](../img/curity/img/username-03.jpg)

2. #### Commit the changes
   1. Commit the changes via the **Changes** menu.

      ![img/username-04.jpg](../img/curity/img/username-04.jpg)
...
2. #### Add Capabilities
   ...
   1. Select the **Code Flow** capability and click **Next**.
      ...
      ![create-client-03.jpg](../img/curity/img/create-client-03.jpg)

   1. Enter `http://localhost/callback` as a **Redirect URI** and click **Add**.
      ![config-callback.jpg](../img/curity/img/config-callback.jpg)
   1. Click **Next**.
   1. On the **Client Authentication** screen, select **secret**.
   1. Click **Generate** to generate a new secret.
      ![config-client-authentication.jpg](../img/curity/img/config-client-authentication.jpg)
   ...
   1. Select `openid` from the list of scopes in the dropdown menu.
      ![config-openid-scope.jpg](../img/curity/img/config-openid-scope.jpg)
...
3. #### Expose the metadata url
   ...
   1. Scroll down to the **OpenID Connect** section and enable the `Expose Metadata` toggle.
      ![expose-metadata.png](../img/curity/img/expose-metadata.png)
...
> **Note:** This tutorial assumes you have an app running locally (that is, on localhost:3000) with the ngrok client installed.

1. Launch a terminal
1. Enter the following command to launch an ngrok tunnel with Curity Identity Server. Replace `<curity_url>` with your Curity issuer address (that is, https://acme.com/oauth/v2/oauth-anonymous) and the `<curity_client_id>` and `<curity_client_secret>` with the respective values copied from the ngrok app registered at Curity:
...
1. [Skip to 4. ](#test-sso) Test the integration
...
1. Click **Begin setup** and enter the following:
   ...
   ![Curity config in ngrok](../img/curity/img/curity-1.png)
...
This step assumes you have an app running locally (that is, on localhost:3000) with the ngrok client installed.
</Tip>

1. Click **Start a tunnel**.
1. Click the **copy icon** next to the tunnel command.

    ![tunnel config](../img/curity/img/curity-2.png)
...
1. To confirm that the tunnel is connected to your edge:
    ...
    ![tunnel confirmed](../img/curity/img/curity-3.png)

1. In the test edge, copy the **endpoint URL**. (you will use this URL to test the Curity Authentication)
    ![tunnel url](../img/curity/img/curity-4.png)

## 4. Test the integration 

1. In your browser, launch an incognito window.
1. Access your ngrok tunnel (that is, https://curity-sso-test.ngrok.app or using a copied URL).
