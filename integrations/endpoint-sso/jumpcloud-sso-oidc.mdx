...
1. Click the **SSO** tab, enter `https://idp.ngrok.com/oauth2/callback` in the **Redirect URIs** field, select **Client Secret Basic** as the **Client Authentication Type**, and enter the URL provided by the ngrok agent to expose your application to the internet in the **Login URL** (that is, `https://myexample.ngrok.app`).
   ![JumpCloud configuration](../img/jumpcloud/ngrok_url_configuration_jumpcloud.png)
...
1. Enter the following command to launch an ngrok tunnel with JumpCloud SSO:
   ...
   **Note**: Replace the following with values:
   - JUMPCLOUD_OAUTH_URL: Your JumpCloud OIDC base URL (that is, `https://oauth.id.jumpcloud.com/`).
...
1. Click **Begin setup** and enter the following values into the fields:
   ![JumpCloud config in ngrok](../img/jumpcloud/jumpcloud-1.png)
   - **Issuer URL**: Your JumpCloud OIDC base URL (that is, `https://oauth.id.jumpcloud.com/`).
...
1. Click the **copy icon** next to the tunnel command.
    ![tunnel config](../img/jumpcloud/jumpcloud-2.png)
...
1. To confirm that the tunnel is connected to your edge:
    ...
    - Refresh the test edge page. Under traffic, You will see the message _You have 1 tunnel online. Start additional tunnels to begin load balancing._
      ![tunnel confirmed](../img/jumpcloud/jumpcloud-3.png)

1. In the test edge, copy the **endpoint URL**. (You use this URL to test the JumpCloud Authentication)
    ![tunnel url](../img/jumpcloud/jumpcloud-4.png)

## 3. Test the integration 

1. In your browser, launch an incognito window.

1. Access your ngrok tunnel (for example, `https://jumpcloud-sso-test.ngrok.app` or using the copied endpoint URL).
