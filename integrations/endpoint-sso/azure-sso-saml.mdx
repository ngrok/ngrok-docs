...
To have ngrok enforce single sign-on (SSO) using SAML with Azure Active Directory B2C (Azure AD B2C):

1. [Create an ngrok Edge](#1-create-an-ngrok-edge)
2. [Configure Azure AD B2C custom policies](#2-configure-azure-ad-b2c-custom-policies)
3. [Create an Azure AD B2C SAML application](#3-create-an-azure-ad-b2c-saml-application)
4. [Update the ngrok Edge with the IdP metadata](#4-update-the-ngrok-edge-with-the-idp-metadata)
5. [Test your integration](#5-test-the-integration)
6. [Bonus: Update your Azure AD B2C custom policies to support the password reset flow](#bonus-update-your-azure-ad-b2c-custom-policies-to-support-the-password-reset-flow)
</Tip>
...
2. Some hints:
   - For a SAML application, you need to configure custom policies as it is not possible to use the user flows, as noted elsewhere in the [Azure docs](https://learn.microsoft.com/en-us/azure/active-directory-b2c/saml-service-provider?tabs=macos&pivots=b2c-user-flow#register-your-saml-application-in-azure-ad-b2c).
   - A couple resources to make the process of following these steps easier: you can follow along the docs with the templates shared in [Azure AD B2C custom policy starterpack](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack), or you can use the [IEF Setup App](https://b2ciefsetupapp.azurewebsites.net) to automate these steps.
...
2. Some hints:
   ...
   You can either update the files in the starterpack repo, or you can download the XML files for your existing policies from the [Identity Experience Framework page](https://portal.azure.com/#view/Microsoft_AAD_B2CAdmin/CustomPoliciesMenuBlade/~/overview/tenantId/ngroksaml.onmicrosoft.com).
...
1. Back in the ngrok dashboard for your Edge's SAML configuration, copy and paste the XML found at the IdP Saml Metadata URL in the Identity Provider section, replacing the previously set placeholder value.
...
9. After logging in, you should be able to see the application.
...
Here is [sample code from Azure AD B2C](https://github.com/azure-ad-b2c/samples/tree/master/policies/embedded-password-reset) on how to configure this.
