...
1. Follow [GitHub's documentation for creating an OAuth app](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app) until the final step of submitting the registration form.
2. Set the **Authorization callback URL** to `https://idp.ngrok.com/oauth2/callback`.
3. Submit the form.
4. From the application overview, copy your **Client ID**. Under **Client secrets**, select **Generate a new client secret**, then copy the secret immediately, since GitHub displays its full value only once. Save both for use in ngrok.
