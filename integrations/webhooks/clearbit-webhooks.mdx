...
In Clearbit's API settings, set the following:

- **Webhook URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).

Save your account settings so Clearbit sends webhook responses to your app.

### Run webhooks with Clearbit and ngrok

Clearbit returns a webhook response when you make a lookup against its API.
Trigger one by running a person lookup in a terminal:
...
Replace `EMAIL_ADDRESS` with an email in your Clearbit organization and `TOKEN` with your Clearbit secret key.

Confirm your app receives the response and logs its headers and body in the terminal.
