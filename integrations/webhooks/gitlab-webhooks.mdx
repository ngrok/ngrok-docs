...
<Steps>
  ...
  <Step title="Configure GitLab to send webhooks">
...
=======
Follow [GitLab's documentation for creating a webhook](https://docs.gitlab.com/user/project/integrations/webhooks/), and set the following:

- **URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Trigger**: choose which events trigger the webhook (for this example, **Push events**).
- **Secret token**: set a token and note it for the [verification step](#secure-webhook-requests) below.
>>>>>>> origin/main
...
=======
After you add the webhook, GitLab sends POST requests to your app when the events you chose occur.
The request body varies by event.

To trigger a call, push a commit to the repository, or use the webhook's test option to send a sample push event.
Then confirm your localhost app logs the request headers and body in the terminal.

You can also review the webhook's recent events in GitLab to compare what GitLab sent with what your app received.
