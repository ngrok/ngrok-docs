...
1. [Configure Linear webhooks with your ngrok URL.](#setup-webhook)
...
- **Develop and test Linear webhooks locally**, eliminating the time in deploying your development code to a public environment and setting it up in HTTPS.
- **Inspect and troubleshoot requests from Linear** in real-time via the inspection UI and API.
- **Modify and Replay Linear Webhook requests** with a single click and without spending time reproducing events manually in your Linear workspace.
...
Linear can trigger webhook calls to external applications whenever events happen in your Linear workspace. To register for such events, follow the instructions below:
...
1. In the **Webhooks** section, click on **Create Webhook**.
   ![URL](../img/linear/img/ngrok_url_configuration_linear.png)

1. In the **Label** field, provide a descriptive name for the webhook (e.g., "ngrok Integration Webhook").
...
1. Click on the webhook you've just created.

1. Review the webhook details and recent delivery attempts.
   ![Recent Events](../img/linear/img/ngrok_introspection_linear_webhooks.png)
