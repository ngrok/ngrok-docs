...
<Steps>
  ...
  <Step title="Configure MongoDB to send webhooks">
...
=======
Follow [MongoDB Atlas's documentation for configuring alert settings](https://www.mongodb.com/docs/atlas/configure-alerts/), add a webhook notification, and set the following:

- **Webhook URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Webhook Secret**: enter a value (for example, `12345`) and note it for the [verification step](#secure-webhook-requests) below.
- **Alert condition**: choose which event triggers the webhook (for this example, when a user joins the project).
>>>>>>> origin/main
...
MongoDB sends different request body contents depending on the event.
To trigger a call, cause the event you selected, such as inviting a new user to your project.
