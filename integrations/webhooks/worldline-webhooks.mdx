...
<Steps>
  ...
  <Step title="Configure Worldline to send webhooks">
...
=======
Follow [Worldline's documentation for creating a webhook endpoint](https://docs.direct.worldline-solutions.com/en/integration/api-developer-guide/webhooks), and set the following:

- **Endpoint URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).

When you create the webhook, Worldline generates a **Secret Webhook Key**.
Note it for the [verification step](#secure-webhook-requests) below.
>>>>>>> origin/main
...
=======
Worldline sends different request body contents depending on the event.
To trigger a test, generate a payment link, open it, and start or cancel a payment.
Confirm your localhost app receives the POST and logs both the headers and body in the terminal.
...
- Use the **Secret Webhook Key** you noted when creating the webhook (see the **Configure Worldline to send webhooks** step).
