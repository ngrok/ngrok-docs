...
<Note>
  This integration requires an ngrok Pay-as-you-go plan because Meta validates your ngrok domain and certificate.
</Note>
...
<Steps>
  ...
  <Step title="Configure WhatsApp to send webhooks">
    ...
    - In **Edit webhook's callback URL**, enter your ngrok URL with `/webhooks` (for example, `https://myexample.ngrok.app/webhooks`), enter a **Verify token** (for example, `12345`), and click **Verify and save**.
...
=======
Follow [Meta's documentation for setting up WhatsApp Cloud API webhooks](https://developers.facebook.com/docs/whatsapp/cloud-api/guides/set-up-webhooks), and set the following:

- **Callback URL**: your ngrok URL with your app's path (for example, `https://myexample.ngrok.app/webhooks`).
- **Verify token**: a value of your choosing (for example, `12345`) that your app checks during the verification handshake.
- **Webhook fields**: subscribe to **messages** and any other fields you need.

After you save, confirm your app receives the verification request and logs `WEBHOOK_VERIFIED`.
>>>>>>> origin/main
...
=======
Send a message to your WhatsApp business number to trigger the webhook.
Confirm your localhost app receives the message and logs both the headers and body in the terminal.
