...
<Steps>
  ...
  <Step title="Configure VMware Workspace ONE to send webhooks">

    Follow [VMware's documentation for configuring Event Notifications in Workspace ONE UEM](https://docs.vmware.com/en/VMware-Workspace-ONE-UEM/2206/System_Settings_On_Prem/GUID-AWT-SYSTEM-ADVANCED-API-NOTIF.html), and set the following when you add a notification rule:

    - **Target URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
    - **Format**: `JSON`.
    - **Events**: enable the events you want to send to your app.

    ### Run webhooks with VMware Workspace ONE and ngrok

    Trigger one of the events you enabled, such as enrolling a device.
...
- Trigger an event again (for example, device enrollment) to send a request.
