...
<Steps titleSize="h2">
  ...
  <Step title="Configure Brex to send webhooks">

    Brex registers webhooks through its API.
    Follow [Brex's Webhooks API documentation](https://developer.brex.com/openapi/webhooks_api/) to create a webhook, using an API token with write access to the resources you want to receive events for (this example uses **Users**).
    ...
    To trigger a webhook call, invite a new user to your Brex account.
