...
Okta sends a one-time verification request to your URL before it can deliver events. Complete the verification so Okta can reach your localhost through ngrok.

### Run webhooks with Okta and ngrok

Okta sends different request body contents depending on the event. You can send a test delivery from the event hook, or trigger a real event by signing out of the Okta console and signing in again.
