...
<Steps>
  ...
  <Step title="Configure Sonatype Nexus to send webhooks">
...
=======
Follow [Sonatype's documentation for configuring a webhook capability](https://help.sonatype.com/en/configure-webhook.html), and set the following:

- **Capability type**: choose **Webhook: Global** or **Webhook: Repository** (for this example, **Webhook: Global**).
- **URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Events**: select the event categories you want to receive (for this example, **repository** events).
- **Secret Key**: set a secret and note it for the [verification step](#secure-webhook-requests) below.
>>>>>>> origin/main
...
=======
Sonatype Nexus sends a different request body depending on the event.
To trigger a call to your app, create a repository or perform another action that matches the events you selected.
