...
<Steps>
  ...
  <Step title="Configure Pinwheel to send webhooks">
...
=======
Pinwheel registers webhook endpoints through its API rather than a dashboard form.
Follow [Pinwheel's documentation for creating a webhook](https://docs.pinwheelapi.com/public/docs/webhooks-1) using your API secret and server URL from the Pinwheel Developer Portal, and set the following:

- **url**: your ngrok URL.
- **enabled_events**: the events you want to subscribe to (for this example, `account.added`, `employment.added`, `identity.added`, and `income.added`).
- **status**: `active`.

The request looks like this, with `SERVER_URL`, `API_SECRET`, and `NGROK_URL` replaced by your values:
...
Confirm the response JSON has a `status` attribute set to `active`.
>>>>>>> origin/main
...
=======
After you create the webhook, Pinwheel sends requests to your app when the events you subscribed to occur.

Because this example subscribes to `account.added`, trigger calls by having users log into their payroll accounts through the Pinwheel API.
