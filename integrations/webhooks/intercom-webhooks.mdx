...
<Steps>
  ...
  <Step title="Configure Intercom to send webhooks">
...
=======
Webhook subscriptions in Intercom belong to an app, so create an internal integration app in your [Developer Hub](https://developers.intercom.com/) first.

Follow [Intercom's documentation for setting up webhooks](https://developers.intercom.com/docs/webhooks/setting-up-webhooks), and set the following:

- **Your request endpoint URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Webhook topics**: choose the topics that trigger the webhook (for this example, **contact.user.created**).

After you save, Intercom sends a test request to your app.
Confirm your localhost app receives it in the terminal.
You can send another test request from Intercom's webhook settings.
>>>>>>> origin/main
...
=======
With **contact.user.created** selected, create a user in Intercom to trigger a call.

Confirm your localhost app receives the event and logs both headers and body in the terminal.

<Tip>
  Intercom sends different request body contents depending on the topics you selected when registering the webhook.
</Tip>
