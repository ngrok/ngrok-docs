...
<Steps>
  ...
  <Step title="Configure Buildkite to send webhooks">
...
=======
Follow [Buildkite's documentation for adding a webhook notification](https://buildkite.com/docs/apis/webhooks/pipelines), and set the following:

- **Webhook URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Events**: choose which events trigger the webhook (for this example, subscribe to all events across all pipelines).
>>>>>>> origin/main
...
=======
Buildkite sends a different request body for each event type.
Create a new build on one of your pipelines to trigger new calls to your app.

Confirm your app receives the event and logs its headers and body in the terminal.
You can also review the delivery in your webhook's request log.
