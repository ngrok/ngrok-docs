...
- An app that handles POST requests (for example, the [Twilio SMS webhook sample on GitHub](https://github.com/thomas-ngrok/ngrok-example-twilio-sms-webhook) or your own), running on port 3000 with a route such as `/sms`.
- A Twilio account.

<Steps titleSize="h2">
  <Step title="Start your app">

    Use an Express app that responds to Twilio webhooks, such as the [Twilio SMS webhook sample on GitHub](https://github.com/thomas-ngrok/ngrok-example-twilio-sms-webhook).
  ...
  <Step title="Expose your app with ngrok">
    ...
    - Copy the URL ngrok displays.
      Your app is now exposed at that URL for use with Twilio (use the full path to your webhook, for example `https://xxxx.ngrok.app/sms`).
  </Step>

  <Step title="Configure Twilio to send webhooks">

    Follow [Twilio's documentation for configuring a phone number's messaging webhook](https://www.twilio.com/docs/usage/webhooks/messaging-webhooks), and set the following for the number you want to use:

    - **A message comes in**: set the webhook to your ngrok URL plus your app's path (for example, `https://xxxx.ngrok.app/sms`).
    - **HTTP method**: `HTTP POST`.

    ### Run webhooks with Twilio and ngrok

    Send an SMS to your Twilio phone number to trigger the webhook.
    Your app receives the request and can respond with TwiML.
