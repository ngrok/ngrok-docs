...
<Steps>
  ...
  <Step title="Configure Jira to send webhooks">
...
=======
Sign in to Jira with an account that has **Jira Administrators** permission, then follow [Atlassian's documentation for managing webhooks](https://support.atlassian.com/jira-cloud-administration/docs/manage-webhooks/), and set the following:

- **Name**: a description for the webhook (for example, `ngrok Integration Webhook`).
- **URL**: your ngrok URL (for example, `https://1234-abcd-5e6f-7g8h-9i0j.ngrok.app`).
- **Events**: select the events that trigger the webhook.
- **JQL**: optionally, a query to limit scope (for example, `project in (ABC, XYZ)`).
...
=======
After you create the webhook, Jira sends a POST request to your app through ngrok when the selected events occur.
To trigger an event, create a new issue or update an existing issue in a project that matches your webhook scope.
