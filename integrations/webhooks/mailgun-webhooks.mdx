...
<Steps>
  ...
  <Step title="Configure Mailgun to send webhooks">
...
Mailgun can send a test request to your webhook URL.
Use it to confirm your localhost app receives the event and logs both headers and body in the terminal.
>>>>>>> origin/main
...
=======
Mailgun sends different request body contents depending on the event.
To trigger a real call, authorize a recipient, have them confirm by email, and then send an email to that recipient (for example, with the Mailgun API).
Replace the placeholders with values from your account:
