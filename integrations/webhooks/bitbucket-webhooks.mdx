...
<Steps>
  ...
  <Step title="Configure Bitbucket to send webhooks">
...
=======
Follow [Bitbucket's documentation for managing webhooks](https://support.atlassian.com/bitbucket-cloud/docs/manage-webhooks/), and set the following:

- **URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Triggers**: choose which events trigger the webhook (for this example, **Push**).
>>>>>>> origin/main
...
=======
<Note>
  The payload your app receives depends on the triggers you chose.
</Note>

To trigger a push event, commit a new file to the repository, either through Bitbucket or by pushing from a local clone:
...
Confirm your app receives the push event and logs the headers and body in the terminal.
...
<Note>
  IP restrictions require an ngrok Pay-as-you-go plan.
...
- In the [ngrok dashboard](https://dashboard.ngrok.com), go to **Identity & Access** and **IP Restrictions**.
