...
Alchemy sends different request bodies depending on the webhook type. Because this example uses **Address Activity**, sending or receiving tokens at the monitored address triggers a webhook.

To test without an on-chain transaction, send a test notification for your webhook from Alchemy. Confirm your app receives the event and logs the headers and body in the terminal.
