...
<Steps>
  ...
  <Step title="Configure Alchemy to send webhooks">
...
=======
Follow [Alchemy's documentation for creating an Address Activity webhook](https://www.alchemy.com/docs/reference/address-activity-webhook), and set the following:

- **Webhook URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Webhook type**: choose which events trigger the webhook (for this example, **Address Activity**, along with the chain, network, and address you want to monitor).
>>>>>>> origin/main
...
=======
Alchemy sends different request bodies depending on the webhook type.
Because this example uses **Address Activity**, sending or receiving tokens at the monitored address triggers a webhook.

To test without an on-chain transaction, send a test notification for your webhook from Alchemy.
Confirm your app receives the event and logs the headers and body in the terminal.
...
<Note>
  ...
  Alchemy does not allow editing webhook URLs, so it's recommended to create a [reserved domain](/gateway/domains/) on your ngrok account and run the agent with `--domain your-domain.ngrok.app`.
</Note>
