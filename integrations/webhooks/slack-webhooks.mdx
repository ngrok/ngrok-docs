...
<Steps>
  ...
  <Step title="Configure Slack to send webhooks">
...
=======
Slack sends webhook requests to your app through the Events API.
Follow [Slack's documentation for the Events API](https://docs.slack.dev/apis/events-api/), create a Slack app, enable event subscriptions, and set the following:

- **Request URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Events**: subscribe to the events you want to receive (for this example, `message.im`).

After you subscribe to events, install the app to your workspace.
>>>>>>> origin/main
...
Because you subscribed to `message.im` and installed the app, you can trigger webhooks by sending a direct message.
Message Slackbot with something like `Hello Slack bot` to send a request.
>>>>>>> origin/main
