...
<Steps>
  ...
  <Step title="Configure Box to send webhooks">
...
=======
Box V2 webhooks require a custom app that has the **Manage Webhooks** scope enabled and is authorized for your account.
With that app in place, follow [Box's documentation for creating a V2 webhook](https://developer.box.com/guides/webhooks/v2/create-v2/), and set the following:
...
To trigger a webhook call, upload a file to that folder in Box.
Confirm your app receives the **FILE.UPLOADED** event and logs the headers and body in the terminal.
