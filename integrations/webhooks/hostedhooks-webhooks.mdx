...
<Steps>
  ...
  <Step title="Configure HostedHooks to send webhooks">
...
=======
Follow [HostedHooks' documentation for creating a webhook endpoint](https://docs.hostedhooks.com/getting-started/webhooks), and set the following:

- **URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Version**: `1.0`.
- **Event**: subscribe the endpoint to the event type you want to receive (for this example, `myapp.event`).
>>>>>>> origin/main
...
=======
When you send a message to the HostedHooks message endpoint with an event type in the body, HostedHooks routes it to your app through your ngrok URL.

To trigger a call, post a message to your app's message endpoint (replace `APP_ID` with your app ID and `API_KEY` with your API key):
