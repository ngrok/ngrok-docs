...
<Steps>
  ...
  <Step title="Configure PagerDuty to send webhooks">
...
When you create the webhook, PagerDuty shows a signing secret once.
Copy it and note it for the [verification step](#secure-webhook-requests) below.
>>>>>>> origin/main
...
=======
PagerDuty sends different request body contents depending on the event.
You can send a test event from the webhook, or trigger a real event by creating a new incident on a service.
