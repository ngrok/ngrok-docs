...
1. On the right panel, select **POST** instead of **GET** in the **HTTP Method**, and enter the URL provided by the ngrok agent to expose your application to the internet in the field next to **POST** (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
   ![Plivo URL to Publish](../img/plivo/ngrok_url_configuration_plivo.png)
...
1. Click one of the logs and then scroll down to the \***\*Status Callbacks** section.
   ![Webhook Logs](../img/plivo/ngrok_logs_plivo.png)
