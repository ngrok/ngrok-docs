...
<Steps>
  ...
  <Step title="Configure Chargify to send webhooks">
...
=======
Follow [Chargify's documentation for adding a webhook endpoint](https://docs.maxio.com/hc/en-us/articles/24266136649869-Webhooks-Configuration-and-Testing), and set the following:

- **Webhook URL**: your ngrok URL (for example, `https://1a2b-3c4d-5e6f-7g8h-9i0j.ngrok.app`).
- **Events**: choose which events trigger the webhook (for this example, subscribe to all events).
>>>>>>> origin/main
...
=======
Chargify sends a different request body for each event type.
To trigger a call, send a test notification to your webhook from Chargify's webhook testing tools.

Confirm your app receives the event and logs its headers and body in the terminal.
You can also review the delivery in Chargify's webhook history.
