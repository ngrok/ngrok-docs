...
description: In this guide, you'll learn how to enable automated user provisioning for your ngrok account via SCIM with Okta.
---
...
1. Navigate to the **Settings**, **Account** menu in the left side of the page.
   ...
   ![ngrok Dashboard Settings](../img/okta/img/okta-scim-account-settings.png)
...
1. Go to the **User Provisioning / SCIM** section and toggle on the **Automated User Provisioning via SCIM 2.0** option to on. This will open a drawer and prompt you to create a new API key. It is recommended to use a unique key just for SCIM.
   ...
   ![ngrok User Provisioning](../img/okta/img/okta-scim-provisioning.png)
...
1. Give the API key a descriptive name and assign it either to an administrator user or a Service User. Service Users are ideal for this integration because they will not be deactivated when a user leaves the account.
   ...
   ![ngrok create new API key](../img/okta/img/okta-scim-add-api-key.png)

1. Save the API key somewhere safe or keep this window up while completing the next steps in Okta.

   ![ngrok new API key](../img/okta/img/okta-scim-new-api-key.png)

1. Go to the **Account Domain Controls** section and click the **Submit New Domain** button.

   ![ngrok Account Domain Controls](../img/okta/img/okta-scim-account-domain-controls.png)

1. Submit a request to ngrok support to allow the domain sync with SCIM provisioning in your account.

   ![ngrok Account Domain Controls request](../img/okta/img/okta-scim-account-domain-controls-request.png)

## Configuring Okta SCIM

This document assumes you have already set up [Okta for dashboard SSO](/integrations/dashboard-sso/okta-dashboard-sso-scim). Once you've completed those steps, you can continue here.
...
1. In the Okta admin console navigate to the to **Applications/Applications** menu on the left side.
   ...
   ![Okta Applications](../img/okta/img/okta-scim-okta-app.png)
...
1. Enable SCIM on the general tab in the ngrok application you created under the Provisioning section:
   ...
   ![Okta Application General](../img/okta/img/okta-scim-okta-general-settings.png)
...
1. The configuration should look like this:
   ...
   ![Okta Application Provisioning](../img/okta/img/okta-scim-okta-app-provisioning.png)
