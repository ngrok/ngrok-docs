...
You may want to consult the Salesforce documentation on [configuring an authentication provider using OpenID Connect](https://help.salesforce.com/s/articleView?id=xcloud.sso_provider_openid_connect.htm&type=5).
...
Finally, note your Client ID (Consumer Key) and Secret.
These can be found in **OAuth Settings > App Settings**.

## 2. Configure ngrok

Configure SSO for your ngrok account:
...
You should now be configured to log into your ngrok account using Salesforce.

By default, users can log into ngrok with their existing credentials as well as through Salesforce.
