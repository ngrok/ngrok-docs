...
When signing up for ngrok, you have the ability to utilize Google or Github SSO and/or by setting a password. Any or all of these methods can be used to create an ngrok account.
...
Note that if you did not set a password and only have an SSO method set, you won't be able to reset your password.
...
If you receive a `ERR_NGROK_4301` error, you'll need to try signing in with GitHub or Google SSO, as you didn't set a password for the account.
...
Click your username in the upper-left corner of the dashboard and selecting **User Settings**. 
...
Multi Factor Authentication (MFA) provides an extra layer of security for logging into your ngrok account.
Users may add an additional authentication factor to protect their ngrok logins.
At the moment, MFA only supports [TOTP](https://en.wikipedia.org/wiki/Time-based_one-time_password) as an additional factor.
...
You will also be provided a list of recovery codes, which you can copy and paste elsewhere, or to download directly.
...
You can enable or disable MFA at any time once logged into your account by clicking on your username in the upper left corner of your dashboard, and then **User Settings**.
