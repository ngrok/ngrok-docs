...
Use ngrok's [Traffic Inspector](/share-localhost/inspection) to monitor all API requests and responses in real time.
This is especially helpful for debugging integration issues when working with external consumers.

## Add authentication

Protect your shared API by adding authentication with a [Traffic Policy](/gateway/traffic-policy/).
You can require OAuth or other forms of authentication without changing your application code.
See the [authentication guide](/share-localhost/auth) for setup instructions.

## Next steps

- [Quickstart](/share-localhost/quickstart): get started with ngrok in minutes
- [Authentication](/share-localhost/auth): protect your shared API with authentication
- [Network security](/share-localhost/security): restrict access by IP or other criteria
