...
First, you'll need some sort of web service running on your machine. It should be available at `http://localhost:{any port}`. If you already have one, you can skip to Step 2. If not, we'll set one up using Python SimpleHTTPServer (ngrok actually has a [built in file server](/universal-gateway/http/#file-serving) but let's not worry about that now).
...
- Configure ngrok to use the same domain each time with [`--url`](/universal-gateway/http/#static-domains)
...
- Use [ngrok Edges](/universal-gateway/edges/) to dynamically reconfigure traffic to your ngrok agents
