...
First, you'll need some sort of web service running on your machine. It should be available at http://localhost:\[any port\]. If you already have one, you can skip to Step 2. If not, we'll set one up using Python SimpleHTTPServer (ngrok actually has a [built in file server](/docs/secure-tunnels#http-tunnels-file-urls) but let's not worry about that now).
...
By the way, if you have a paid plan and want to keep the same URL each time you restart, use the [`--subdomain`](/docs/ngrok-agent/ngrok#command-ngrok-http-flags) flag when starting the agent.
...
*   Configure ngrok to use the same domain each time with [`--subdomain`](/docs/secure-tunnels#http-tunnels-subdomain)
*   Tunnel other non-HTTP services such as SSH, RDP, or game servers using [TCP Tunnels](/docs/secure-tunnels#tcp-tunnels)
*   Bring your own [custom domains to ngrok](/docs/secure-tunnels#http-tunnels-custom-domains)
*   Add your [API key and automate via the `ngrok api` command](/docs/ngrok-agent/ngrok#command-ngrok-api).
*   Use [ngrok Cloud Edges](/docs/cloud-edge) to dynamically reconfigure traffic to your ngrok agents