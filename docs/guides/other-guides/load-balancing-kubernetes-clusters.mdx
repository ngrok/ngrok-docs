...
will proxy traffic to your `foo`, `bar`, and `baz` pods, respectively.
...
the `/foo` path to the pool you created at `https://foo.internal`.
	 - The pooled endpoints then proxy requests proxies the request to the `foo`
...
- [Deploy a multicloud API
	gateway](/docs/guides/other-guides/load-balancing-kubernetes-clusters/)
