...
- Works with [Traffic Policy](/docs/traffic-policy) for managing traffic
	identically across all your replicas from a single "front door" or enforcing
	different policies for each replica.
...
Make a few more requests to responses from the Go app alongside any existing
