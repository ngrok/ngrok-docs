...
- Works with the [agent CLI](/docs/agent),
  [SDKs](/docs/agent-sdks/) and our [Kubernetes Operator](/docs/k8s). You can even use both tools together.
...
endpoint](/docs//docs/universal-gateway/agent-endpoints/) on an ngrok URL (or a [custom domain](/docs/guides/other-guides/how-to-set-up-a-custom-domain/)!), and the `--pooling-enabled` flag on.
...
2. In [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector)—click on a request, then the **Response** tab, and look at **Ngrok-Agent-Ips** to see the IP for the responding agent changes from request to request with a similar frequency.
...
Here's an example of a Go app with the [ngrok Go SDK](https://github.com/ngrok/ngrok-go). Just as with the agent CLI examples in the previous step, this Go app starts up an agent endpoint at `$YOUR_DOMAIN.ngrok.app` with pooling enabled.
...
## Using Cloud Endpoints for custom traffic management
