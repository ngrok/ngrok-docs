...
that cloud provider's built-in solutions, like AWS Elastic Load Balancing, GCP
Cloud Load Balancing, or DigitalOcean Global Load Balancers. But once you start
deploying replicas of your services to multiple clouds, you need a load balancer
that's multicloud-ready.

[Endpoint Pooling](/docs/universal-gateway/endpoint-pooling) makes multicloud
load balancing simple as creating two endpoints with the same URL. ngrok then
automatically distributes traffic to replicas in multiple clouds, which means
you don't even need other load balancers in your stack at all. Plus, Endpoint
Pooling:

- Works with the [agent CLI](/docs/agent),
	[SDKs](/docs/agent-sdks/), and even our [Kubernetes Operator](/docs/k8s)—even
	in combination.
- Lets you migrate from one deployment strategy to another without hard
	cut-overs, like moving from VMs to Kubernetes.
- Works with [Traffic Policy](/docs/traffic-policy) for traffic management, even
	letting you set different policies for different endpoints in your pool.

## 1. Start with Agent Endpoints on the CLI
...
On another cloud where you've deployed a replica of your service, repeat the
same steps to start a second internal agent endpoint on the same URL.
...
1. Each agent's terminal UI, which will show a similar number of recent requests.
...
## 2. Add in another form factor

Pooling also isn't limited by how you start a given service or its replicas. If
you've already started up one agent endpoint on the CLI, you can start up
another with one of our [SDKs](/docs/agent-sdks) or the [Kubernetes
Operator](/docs/k8s) to load-balance between them.

Here's an example of a very simple Go app with the [ngrok Go
SDK](https://github.com/ngrok/ngrok-go). Just as with the agent CLI examples
above, this Go app starts up an agent endpoint at `$YOUR_DOMAIN.ngrok.app` with
pooling enabled.
...
Save this file as `main.go` and then initialize the dependencies.
...
Start your app to fire up the ngrok Go SDK and add a new agent endpoint to your
pool.
...
Make a few more requsets to responses from the Go app alongside any existing
agent CLIs you already have running, once again, despite these services
operating in entirely different clouds and deployment environments.

## 3. Step up to a Cloud Endpoint for custom traffic management
...
management fundatmentals, like authentication, URL rewrites/redirects, or header
...
internal URL, which is one that ends with `.internal`, like
`https://pooling.internal`.
