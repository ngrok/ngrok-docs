...
description: Configure site-to-site connectivity with end-to-end mTLS encrption to access APIs
tags:
  - guides
  - mtls
  - tls
...
page](https://ngrok.com/docs/cloud-edge/modules/mutual-tls/).
...
4. Click **2 targets** next to *DNS Targets* in the panel displayed on the right-hand side of the screen, 
as denoted by the arrow in the screenshot below.

![alt-text](2tunnels.png)

5. Click **Check Status**, as denoted by the arrow in the screenshot below.

![alt-text](check-status.png)

6. You should see a successful response similar to the screenshot below.

![alt-text](dns-success.png)

## Create a bot user
Bot users allow you to create authtokens not assigned to a member of your team. 
It's useful to separate the authtokens running in production from any individual team member
to reduce the risk of downtime if that team member leaves your organization and is deprovisioned.

To [create a bot user](https://ngrok.com/docs/iam/bot-users/), log in to the ngrok 
dashboard and click **Bot Users** under *Members*, then click [**Create a Bot User**](https://dashboard.ngrok.com/users/bots/new) 
in the dialog that displays. Give the bot user a name and click **Save**.

![alt-text](bot-user.png)
...
These are the addresses of ngrok’s global [Points of Presence (POPs)](http://localhost:3000/docs/network-edge/#points-of-presence). 
...
including the 
custom ingress address you set in a previous step, typically `connect.{YOUR_DOMAIN}`: 
...
tunnels:
  agent-api:
    proto: http
    domain: {YOUR_CUSTOM_INGRESS_ADDRESS}
    addr: 4040
    inspect: false
```
...
name for this tunnel. In this case, you’re starting a tunnel to serve the ngrok agent API on `https://agent-api.customer1.{YOUR_DOMAIN}`.
