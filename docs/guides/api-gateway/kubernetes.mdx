...
- Uses the client's IP address (via the [`conn.client_ip`
  variable](/traffic-policy/expressions/variables.mdx#connclient_ip))
...
- Explore other [Traffic Policy actions](/traffic-policy/actions//)
  ...
  authentication](/traffic-policy/actions/jwt-validation.mdx), [adding
  headers](/traffic-policy/actions/add-headers.mdx), [URL
  rewrites](/traffic-policy/actions/url-rewrite.mdx),
  [logging](/traffic-policy/actions/log.mdx), and more.
