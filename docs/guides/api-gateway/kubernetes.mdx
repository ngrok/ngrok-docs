...
1. Deploy the ngrok Kubernetes Operator to your cluster.
2. Set up a single hostname, attached to a [Cloud Endpoint](/docs/universal-gateway/cloud-endpoints/) that routes traffic to services based on pathname, and an internal [Agent
   Endpoint](/docs/universal-gateway/agent-endpoints) in your cluster to receive requests and route them to the right pod.
...
The two manifests below provision pods and resources for `abc` on
and `xyz`, respectively, and configure the container's environment variables to
respond uniquely to "prove" you're routing to multiple services.
