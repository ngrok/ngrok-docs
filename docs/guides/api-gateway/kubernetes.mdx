...
You'll need to replace `{YOUR_NGROK_DOMAIN}` with the domain you reserved, along
with `{YOUR_SERVICE}` and `{SERVICE_PORT}` based on the API service you've
already deployed to your Kubernetes cluster. If you have multiple API services
...
your API service is now publicly available at `{YOUR_NGROK_DOMAIN}`. That's a
start, but you likely want to also add in common features for API gateways, like
...
So far, our steps have been pretty generic, asking you to fill in variables like
`{YOUR_SERVICE}` to match an API service you've already deployed to your cluster.

If you're still wondering how to adopt this implementation process 
