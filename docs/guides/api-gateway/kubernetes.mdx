...
ngrok's [Traffic Policy](/http/traffic-policy/index.mdx) module allows you
...
- Uses the client's IP address (via the [`conn.client_ip`
  variable](/http/traffic-policy/expressions/variables.mdx#connclient_ip))
  to track usage against your capacity limit.

Check out the [Traffic Policy (for HTTP tunnels)
documentation](/http/traffic-policy/index.mdx) for a complete view
...
section](https://dashboard.ngrok.comedges/) of your ngrok dashboard.
...
- Learn about other ways to configure [Traffic Policy with the Gateway
  API](/k8s/user-guide.mdx#gateway-composition).
- Explore other [Traffic Policy actions](/http/traffic-policy/actions/index.mdx)
  you can enable on your API gateway, like [JWT
  authentication](/http/traffic-policy/actions/jwt-validation.mdx), [adding
  headers](/http/traffic-policy/actions/add-headers.mdx), [URL
  rewrites](/http/traffic-policy/actions/url-rewrite.mdx),
  [logging](/http/traffic-policy/actions/log.mdx), and more.
...
CD](/integrations/argocd/apiops.mdx).
