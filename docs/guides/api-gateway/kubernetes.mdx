...
ngrok Kubernetes Operator with the [Kubernetes Gateway API](https://ngrok.com/docs/k8s/getting-started-gwapi/).
...
- Kubernetes cluster and tools - You'll need access to a Kubernetes cluster, and you'll need `kubectl` and `helm` installed locally.

## Deploy the example API

If you're building a proof of concept with ngrok as the API gateway for your
APIs in Kubernetes, you can use our [example API
...
Now that you've deployed the example API service, you'll use the ngrok Kubernetes Operator to apply some initial ingress
rules to bring your API online.
...
```
### Create the API gateway manifest
...
If you're using the example API service, substitute
