...
<Tabs groupId="config_example" queryString="config">
  <TabItem value="step-00" label="Current state">
    <img src={require('./img/k8s-step0.png').default} className="border-1" />
  </TabItem>
...
<div className="ngrok--cards ngrok--cards-row">
  <div className="ngrok--card ngrok--card-sm">
    ...
    <p>Any tier will do&mdash;<a href="">register now</a>.</p>
  </div>
  <div className="ngrok--card ngrok--card-sm">
    <div className="ngrok--card-heading jc-space-between">
      <h3 className="fw-600">A reserved domain</h3>
    </div>
    <p>Grab yours in the <a href="">ngrok dashboard</a>.</p>
  </div>
</div>
<div className="ngrok--cards ngrok--cards-row">
  <div className="ngrok--card ngrok--card-sm">
  ...
  <div className="ngrok--card ngrok--card-sm">
    <div className="ngrok--card-heading jc-space-between">
      <h3 className="fw-600"><code>kubectl</code> and Helm</h3>
    </div>
...
Install the ngrok Kubernetes Operator into the `ngrok` namespace with Gateway API
...
- At least one `HTTPRoute`, with your `Gateway` as its parent, that matches
  paths to existing services and their `port`.
