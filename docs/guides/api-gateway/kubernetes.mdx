...
<div className="ngrok--cards ngrok--cards-row">
	...
	<div className="ngrok--card ngrok--card-sm">
		...
		<p>
			Grab yours in the{" "}
			<a href="https://dashboard.ngrok.com/">ngrok dashboard</a>.
		</p>
	</div>
</div>
<div className="ngrok--cards ngrok--cards-row">
	<div className="ngrok--card ngrok--card-sm">
		...
		<p>... and optionally, a functioning API service. If you don't yet have one, you can install our demo service. </p>
	</div>
	<div className="ngrok--card ngrok--card-sm">
		<div className="ngrok--card-heading jc-space-between">
			<h3 className="fw-600">
...
If you're building a proof of concept with ngrok as the API gateway for your APIs in Kubernetes, you can use our [example ngrok API service](https://github.com/ngrok-samples/ngrok-api-service). This service responds to all routes with information about the connection&mdash;think of it as the `pong` to your `curl`.

:::note
If you already have an API service, you can skip this step&mdash;just be aware that you'll need to edit the example configurations below to properly configure the ngrok Kubernetes Operator.
:::

To deploy the ngrok API service to your cluster, apply its `deployment.yaml`
file, which will create the `prod` namespace and create the deployment there.

```bash
https://raw.githubusercontent.com/ngrok-samples/ngrok-api-service/refs/heads/main/kubernetes/deployment.yaml
```
...
Install the ngrok Kubernetes Operator into the `prod` namespace with
...
If you're using the ngrok API service from **Step 0**, all you need to do is replace `{YOUR_NGROK_DOMAIN}` with the domain you reserved.

If you're deploying your own API service, you'll also need to edit `name: ngrok-api-service` and `port: 80` to match your deployment. If you have multiple API services
in different containers, you'll need to create multiple rules to match paths to
services.
...
your API service is now publicly available at `{YOUR_NGROK_DOMAIN}`. You can now test your API gateway and service.
...
You must make instantiate the `NgrokTrafficPolicy` CRD in your cluster before
