...
- ngrok account - [Sign up](https://dashboard.ngrok.com/signup) for for free if you don't already have one.
- API key - [Create an ngrok API key](https://dashboard.ngrok.com/api-keys/new) using the ngrok dashboard.
- Authtoken - [Create an authtoken](https://dashboard.ngrok.com/authtokens) using the ngrok dashboard.
- Reserved domain - [Reserve a domain](https://dashboard.ngrok.com/domains) in the ngrok dashboard or using the [ngrok API](/docs/api/resources/reserved-domains/#create-reserved-domain).
- ngrok agent - [Download](https://download.ngrok.com) the appropriate version, and install it on the same subnet as the API you wish to manage.

## Deploy the example API

To test the Traffic Policy rules in this guide, you can use [the sample API](https://github.com/ngrok-samples/ngrok-rest-api)
to simulate an environment with multiple APIs deployed and managed
independently.
...
You'll receive a `201 Unauthorized` error. Your API is now protected from unauthorized requests.
