...
```shell
docker run --rm --platform=linux/amd64 -p 4000:4000 -e MESSAGE='Hello from service abc!' -d joelatngrok/api-demo
```

Start up a second container on port `5000`.

```shell
docker run --rm --platform=linux/amd64 -p 5000:4000 -e MESSAGE='Hello from service xyz!' -d joelatngrok/api-demo
```
