...
- **An ngrok account**: [Sign up](https://dashboard.ngrok.com/signup) if you
...
- **The ngrok agent**: [Download](https://ngrok.com/downloads) the appropriate
...
```shell
docker run -p 4000:4000 -d joelatngrok/api-demo
```

## Create an internal agent endpoint

Your upstream API service needs a way of receiving traffic from the ngrok network, which you can establish with an agent endpoint on an internal URL like
`https://foo.internal`. Replace `4000` if you've brought your own API service.

```
ngrok http 4000 --url https://foo.internal
```
...
At this point, your multicloud API gateway is ready for traffic! Time to give it
a go.
...
Now is the time to start getting even more from [Traffic
Policy](/docs/traffic-policy/), our configuration language for managing traffic.
