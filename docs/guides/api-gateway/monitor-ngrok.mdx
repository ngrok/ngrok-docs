...
Now you can not only observe what happens to rate-limited requests, but use Traffic Inspector to dial in your threshold.
...
Let's set up webhooks with [ntfy.sh](https://ntfy.sh), a free notification service, as a quick demonstration.
...
Generally, if your API returns status codes in the `500` range instead of the `200` range, you have some debugging and remedation to attend to.
...
You've now not only set up ngrok as an API gateway with the agent or Kubernetes Operator, but also established fundamentals for observing it as a holistic system.

You're ready for production, but you still haven't reached the end of what's possible with Traffic Policy—explore the [Traffic Policy examples](/docs/traffic-policy/examples) for even more ways you can filter, manage, and orchestrate traffic.
