...
<Tabs>
	...
	<TabItem value="API">
			...
			API](/docs/api/index), which you can use to create a cloud endpoint and
...
- [Phases](/docs/traffic-policy/phases) are the distinct points in the lifecycle of
...
1. Filter for requests arriving on *only* `https://<YOUR_NGROK_DOMAIN>/abc` and
	 forward them to your internal agent endpoint in cloud A.
2. Filter for requsets arriving on *only* `https://<YOUR_NGROK_DOMAIN>/xyz` and
...
### Validate JWTs on all API and requests
