...
more redundant, or to access cloud-specific services, you need to make `>1` API
services available from `>1` clouds behind a single hostname.
...
management policies like authentication and rate limiting.
  ...
  template:
    ...
    spec:
      ...
      - name: abc-app
        ...
        env:
        - name: MESSAGE
          value: "Hello from `abc-app` on cloud A!"
---
apiVersion: v1
kind: Service
metadata:
  ...
  template:
    ...
    spec:
      ...
      - name: xyz-app
        ...
        env:
        - name: MESSAGE
          value: "Hello from `xyz-app` on cloud B!"
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  upstream:
    url: http://abc-app-service.default:4000
```
...
spec:
  ...
  upstream:
    url: http://xyz-app-service.default:4000
```
...
Hop over to the [**Endpoints** section](https://dashboard.ngrok.com/endpoints)
of the ngrok dashboard and click **+ New**. Leave the **Binding** value
**Public**, then enter the domain name you reserved earlier. Click **Create
Cloud Endpoint**.
...
The rules below uses [expressions](/docs/traffic-policy/concepts/expressions) to:

1. Route all traffic on `https://<YOUR_NGROK_DOMAIN>/abc` to your
   `https://abc-cloud-a.internal` agent endpoint in cloud A. 
2. Route all traffic
   on `https://<YOUR_NGROK_DOMAIN>/xyz` to your `https://xyz-cloud-b.internal`
   agent endpoint in cloud B.
...
```
{"message":"Hello from `abc-app` on cloud A!","host":"<YOUR_NGROK_DOMAIN>","req_headers":{"host":"<YOUR_NGROK_DOMAIN>","user-agent":"curl/8.7.1","accept":"*/*","accept-encoding":"gzip"},"method":"GET","url":"/abc","time":"2025-03-04T01:10:25.812Z","status":200}
```

And when you run `curl https://<YOUR_NGROK_DOMAIN>/xyz`?

```
{"message":"Hello from `xyz-app` on cloud B!","host":"<YOUR_NGROK_DOMAIN>","req_headers":{"host":"<YOUR_NGROK_DOMAIN>","user-agent":"curl/8.7.1","accept":"*/*","accept-encoding":"gzip"},"method":"GET","url":"/xyz","time":"2025-03-04T01:10:25.812Z","status":200}
```
...
rules is that they're *composable*—you can reuse them, chain them, and apply
