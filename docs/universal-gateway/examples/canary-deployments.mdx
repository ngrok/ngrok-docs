...
By controlling who accesses the new version, either by random distribution of traffic or a specific header attached to requests from certain customers, you can see exactly if and where something breaks before releasing it to all your users.

With ngrok, you can handle ingress to canary deployments by:

- Choosing between random distribution across all your customers or apply a specific header to some customers' requests to opt them in to a new version i.e. a feature flag.
...
<Tabs>
	...
	<TabItem value="header" label="By header">
		...
		on_http_request:
			- expressions:
			    - "req.headers.exists_one(x, x == 'x-canary') && req.headers['x-canary'].join(',') == 'true'"
				actions:
					- type: forward-internal
						config:
							url: https://service-canary.internal
...
If you opted for random distribution, you should increase the volume of traffic forward to your canary at regular intervals as you get confident in its stability.
For example, to increase the distribution to a 50-50 split between your two versions.
...
If you want to maintain `https://service.internal` as the canonical name for the stable production deployment, you can start another agent pointing to your new version with this canonical name, edit your policy, and shut down the agent for `https://service-canary.internal`.
...
- Explore other common gateway setups, like [multiplexing to many
	services](/docs/universal-gateway/examples/multiplex) or shipping a [custom "maintenance mode"](/docs/universal-gateway/examples/maintenance-mode/) during an outage or planned downtime a canary deployment can't help with.
