...
This policy rewrites the `Host` header of every HTTP request to `localhost` so that Wordpress accepts the requests.
...
This policy first checks whether an HTTP request's URL contains the `/wp-admin` path. If it does, then it applies the `restrict-ips` action to allow only the addresses you explicitly added.
