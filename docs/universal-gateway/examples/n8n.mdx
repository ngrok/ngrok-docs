...
ngrok works with the [self-hosted and enterprise editions](https://docs.n8n.io/hosting/) of n8n, including both Docker and npm-based installs.
...
**What's happening here?** This policy filters for *only* traffic that contains the specific `/webhook/$WORKFLOW_UUID` path, then verfies that it both originates from GitHub (or another [supported provider](/docs/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
