...
**What's happening here?** This policy filters for _only_ traffic that contains the specific `/webhook/$WORKFLOW_UUID` path, then verifies that it both originates from GitHub (or another [supported provider](/docs/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
