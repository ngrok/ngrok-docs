...
You can route specific traffic to the green environment for final tests, and once you make the switch to green, you can always quickly roll back to blue if you find issues.

ngrok makes blue-green deployments easier by helping you:

- Specify in your policy which environments are currently live and testing, and switch between them freely for each new blue-green deployment.
- Allow requests with certain headers to access your testing environment.
- Handle both **whole-environment** and **scoped (service-specific)** blue-green deployments:
  - Whole-environment deployments are ideal for monolithic-ish architectures where services are coupled and need to be updated together.
  - Scoped deployments work better when your services are entirely decoupled and can be deployed independently.

In this example, the `blue` deployment is your live deployment, while `green` is where you're stesting new features.
You can also add an `x-deployment-version` header to requests to manually route requsets to the green testing environment.
...
<Tabs>
	<TabItem value="single" label="Single service/whole-environment" default>
		...
		**What's happening here?**
		...
		These variables are then plumbed into the rest of the policy so that you don't have to manually change the values of cookies or internal URLs each time you do a blue-green deployment.

		The policy then checks whether your request contains the `x-deployment-version` header and its value equals the current `test` variable.
...
Edit your policy again to switch the environment and their state, where `green` becomes live and `blue` is your new testing environment.
...
- View your [Traffic
	Inspector](https://dashboard.ngrok.com/traffic-inspector) to see how your blue and green environment are behaving, particularly after making the switch, to know exactly when and why to roll back.
- Learn how to do [canary deployments](/docs/universal-gateway/examples/canary-deployments) with custom traffic splitting as lower-cost but more complex alternative to blue-green.
