...
- `$JWT_DOMAIN`: The domain name for your tenant at your JWT provider—for example, with Auth0, it looks something like `https://example.us.auth0`.
        ...
        config:
          ...
          jws:
            ...
            keys:
              sources:
                additional_jkus:
                  - $JWT_DOMAIN/.well-known/jwks.json

  # Forward the request if either the IP restriction OR the JWT validation actions were successful.
  - expressions:
      - "actions.ngrok.restrict_ips.action == 'allow' || actions.ngrok.jwt_validation.tokens[0].verified == true"
    actions:
...
If it's false—the request didn't match the IP `allow` list—then the policy then validates whether it contains a valid JWT token.

Next, the policy checks whether either IP restrictions `OR` JWT validation were successful, and if so, farwards to your upstream service.
