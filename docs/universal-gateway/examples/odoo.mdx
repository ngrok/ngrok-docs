...
description: "Use Odoo (an open-source ERP and business management suite) with ngrok when you want to securely expose Odoo's services to the internet without complex network setups."
sidebar_label: "Expose Odoo ERP Remotely"
---

# Expose and Secure Your Self-Hosted Odoo with ngrok

Odoo is great to self-host for development, demos, or integrations.
...
Reserve a domain (this example uses `your-odoo.ngrok.app`) and save it as an environment variable:
...
For example, Basic Auth for `/web*` and unrestricted access for public website paths.
:::
