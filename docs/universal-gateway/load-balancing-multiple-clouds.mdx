...
2. In [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector), select a request, then select the **Response** tab. Under **Ngrok-Agent-Ips** you should see that the IP for the responding agent changes from request to request with a similar frequency.
