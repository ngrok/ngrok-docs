...
Pooling works with [internal](/docs/universal-gateway/internal-endpoints/) Agent Endpoints, which can only receive traffic from other endpoints associated with your ngrok account.

A [Cloud Endpoints](/docs/universal-gateway/cloud-endpoints/) is a persistent public endpoint where you can manage a traffic policy for any upstream endpoints it forwards traffic to.
