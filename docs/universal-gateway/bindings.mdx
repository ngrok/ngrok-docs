...
Endpoints with a `public` binding have a publicly addressable [URL](/universal-gateway/public-endpoints/urls) that receives traffic from the internet via the ngrok cloud service's global points of presence. e.g. These endpoints might use an ngrok subdomain or a custom domain.
