...
Internal endpoints are non-public endpoints that make it easy to stage and prepare your services to receive traffic. They’re not directly accessible from the internet, but traffic from public or kubernetes endpoints can be routed to internal endpoints using the[`forward-internal` traffic policy action](/docs/traffic-policy/actions/forward-internal/).
...
:::tip
It's generally recommended to use [cloud endpoint](/docs/universal-gateway/cloud-endpoints/)s to manage a centralized Traffic Policy before routing its traffic to one or more internal [agent endpoints](/docs/universal-gateway/agent-endpoints/). See [the guide on using cloud endpoints with internal endpoints](/docs/guides/other-guides/forwarding-and-load-balancing-with-cloud-endpoints/) to learn more.
:::
