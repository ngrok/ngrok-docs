...
description: Learn how to use ngrok's Cloud Endpoints to centrally manage traffic to your services.
---


Cloud Endpoints are persistent, always-on endpoints whose creation, deletion and configuration is managed centrally via the Dashboard or API. They exist permanently until they are explicitly deleted.

:::tip
Cloud Endpoints are the counterpart to Agent Endpoints, which are created and managed by Secure Tunnel agents. See [the Agent Endpoints documentation](/universal-gateway/agent-endpoints/) for more information.
:::
...
:::note
Cloud Endpoints do not forward their traffic to an agent by default. This behavior must be configured with a traffic policy.
:::
