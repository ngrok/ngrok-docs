...
Endpoints pools are not yet generally available, [request access to the
...
and traffic to their URL is balanced among them. You have the flexibility to
define custom balancing strategies and you can even pool endpoints with
different Traffic Policies.

Pooling can be enabled or disabled on a per-endpoint basis.

## When to use Endpoint Pools

Endpoint pools are good choice when you want to:
...
ngrok api endpoints create \
    ...
    --traffic-policy $(cat policy-a.yaml) \
...
ngrok api endpoints create \
    --url https://your-domain.ngrok.app \
    --traffic-policy $(cat policy-b.yaml) \
    --pooling-enabled=true
```
...
Endpoint pools are automatically created and destroyed for you. Whenever you
create two endpoints with the same URL and binding and pooling enabled, an
Endpoint Pool is automatically created for you (unless one of those Endpoints
[disables pooling](#disabling-pooling)). When only a single endpoint remains in
a pool, the pool is deleted.

## Enabling Pooling

Endpoints do not pool by default. You must enable pooling on each endpoint you
want to pool by setting `pooling_enabled` to `true`.
