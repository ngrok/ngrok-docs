...
Endpoints pooling is not yet generally available, [request access to the
...
Endpoint pooling is enabled on a per-endpoint basis. Endpoint pooling is off by
default. You must enable pooling on each endpoint you want to pool by setting
...
Endpoint pooling is available in all plans. Each endpoint in a pool is billed
separately.
