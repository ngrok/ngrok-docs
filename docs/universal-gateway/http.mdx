...
import WildcardEndpoints1 from "/examples/agent-cli/wildcard-endpoints-1.mdx";
...
### Start Multiple Endpoints that Receive Traffic for the Same Wildcard Endpoint

If you have reserved the wildcard domain `*.example.com` and you want to start
multiple endpoints that correspond to different subdomains of `example.com`, you can use the ngrok
agent to establish endpoints for those subdomains regardless of where your upstream services for those endpoints are without reserving additional subdomains.

For example, if you had reserved `*.example.com` and had 2 different APIs at `api1.example.com` and `api2.example.com` that are running in different locations that you need to be able to route requests to, you can specify subdomains via the CLI and ngrok will handle
