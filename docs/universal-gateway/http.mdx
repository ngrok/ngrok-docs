...
With the [Agent CLI](/agent/cli), you can create separate endpoints for subdomains of a wildcard domain no matter where the upstream services are running.

For example, if you reserve `*.example.com` and want to route requests to `api1.example.com` and `api2.example.com`, you can specify subdomains via the CLI and ngrok will handle routing requests to the correct endpoint, even if they're on different ports.
