...
### Start Multiple Endpoints that Receive Traffic for the Same Wildcard Domain

If you have reserved the wildcard domain `*.example.com` and you want to start
multiple endpoints that correspond to different subdomains of `example.com`, 
you can use the `--url` flag to specify the subdomain for each endpoint.
Traffic will then be forwarded to the online wildcard endpoint that is closest to the request URL. 
