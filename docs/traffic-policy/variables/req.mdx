...
| [`req.cookies[k][i].attributes`](#reqcookieskiattributes)   | map[string]string      | The key value map of the attributes of the cookie.                                                                                                                                              |
| [`req.cookies[k][i].domain`](#reqcookieskidomain)           | string                 | The domain for which the cookie is valid.                                                                                                                                                       |
| [`req.cookies[k][i].expires`](#reqcookieskiexpires)         | timestamp              | The expiration time of the cookie.                                                                                                                                                              |
| [`req.cookies[k][i].http_only`](#reqcookieskihttp_only)     | boolean                | True if the cookie is accessible only through the HTTP protocol.                                                                                                                                |
| [`req.cookies[k][i].max_age`](#reqcookieskimax_age)         | duration               | The max age of the cookie.                                                                                                                                                                      |
| [`req.cookies[k][i].name`](#reqcookieskiname)               | string                 | The name of the cookie.                                                                                                                                                                         |
| [`req.cookies[k][i].path`](#reqcookieskipath)               | string                 | The path for which the cookie applies.                                                                                                                                                          |
| [`req.cookies[k][i].same_site`](#reqcookieskisame_site)     | string                 | The SameSite attribute of the cookie.                                                                                                                                                           |
| [`req.cookies[k][i].secure`](#reqcookieskisecure)           | boolean                | True if the cookie should only be sent over HTTPS.                                                                                                                                              |
...
<ConfigExample
	config={{
		expressions: [
			"req.cookies['sessionId'][0].attributes['Domain'] == nba.com",
		],
	}}
/>

### `req.cookies[k][i].domain`

The domain for which the cookie is valid.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].domain == 'nba.com'"],
	}}
/>

### `req.cookies[k][i].expires`

The expiration time of the cookie.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].expires > timestamp(time.now)"],
	}}
/>

### `req.cookies[k][i].http_only`

True if the cookie is accessible only through the HTTP protocol.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].http_only"],
	}}
/>

### `req.cookies[k][i].max_age`

The max age of the cookie.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].max_age > duration(3600)"],
	}}
/>
...
### `req.cookies[k][i].path`

The path for which the cookies applies.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].path == '/"],
	}}
/>

### `req.cookies[k][i].same_site`

The SameSite attribute of the cookie.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].same_site > 0"],
	}}
/>

### `req.cookies[k][i].secure`

True if the cookie shuold only be sent over HTTPS.

<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].secure"],
	}}
/>
...
<ConfigExample
	config={{
		expressions: ["req.cookies['sessionId'][0].value > 0"],
	}}
/>
