...
1. If so, it routes the traffic to a different [internal agent endpoint](/network-edge/internal-endpoints/).
...
:::tip
Change the value the random number is checked against to split the traffic in different ratios. For example,`>=0.8` would send 80% of the traffic to one endpoint and 20% to the other.
:::
