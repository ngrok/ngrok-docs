...
We want to configure ngrok to automatically compress responses from our API, 
but only for the api endpoint. The following 
[Traffic Policy](/docs/http/traffic-policy/index.mdx) configuration demonstrates
how to set this up using the `compress-response` action with an expression to 
