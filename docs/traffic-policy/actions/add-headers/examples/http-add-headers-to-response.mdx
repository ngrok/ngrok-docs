...
The following configuration will add headers to the response from the upstream 
service when the method is `GET` and the URL starts with `/status/200`.
...
to demonstrate the use of string interpolation to include the request connection 
start time.
