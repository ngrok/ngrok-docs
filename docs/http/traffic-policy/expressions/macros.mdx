...
| [`rand.double()`](#randdouble)                                             | `double`    | Returns a random `double` between `0` and `1`.                                                                                                                                                                     |
| [`rand.int(min int, max int)`](#randintlow-int-high-int)                   | `int`       | Returns a random `int` between the provided `min` and `max` values. Only supports positive integers and `min` must be larger than the provided `max`. By default, `min` is `0` and `max` is `1`.                   |
