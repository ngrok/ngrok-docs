...
Next, you need to reserve your subdomain with an [ngrok-managed domain](/universal-gateway/domains/) like `.ngrok.app`, `.ngrok.dev`, `.ngrok.pro`, or even `.ngrok.pizza`. For an example deployment like this, `YOUR_COMPANY-digitalocean-gslb.ngrok.app` would work great.

You can reserve your domain in one of two ways: with the [ngrok API](/api/), or in the [ngrok dashboard](https://dashboard.ngrok.com/cloud-edge/domains).
