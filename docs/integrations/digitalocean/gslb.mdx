...
To use ngrok's [Global Server Load Balancing](/network-edge/gslb/) with DigitalOcean:
...
In this guide, you'll learn how to layer ngrok's [Global Server Load Balancing](/network-edge/gslb/) (GSLB) on top of a VM-based deployment on DigitalOcean.
...
Next, you need to reserve your subdomain with an [ngrok-managed domain](/network-edge/domains-and-tcp-addresses/) like `.ngrok.app`, `.ngrok.dev`, `.ngrok.pro`, or even `.ngrok.pizza`. For an example deployment like this, `YOUR_COMPANY-digitalocean-gslb.ngrok.app` would work great.
...
With a [Weighted Backend](/docs/network-edge/edges#weighted), you can specify custom weights for Tunnel Group backends, shaping precisely how to load-balance traffic across the infrastructure you've distributed around the globe. Without a Weighted Backend, the ngrok Edge and [GSLB](/network-edge/gslb/) will always favor the Point of Presence (PoP) and VM nearest the user making requests.
