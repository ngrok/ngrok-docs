...
To use ngrok's [Global Server Load Balancing](/universal-gateway/network/gslb/) with DigitalOcean:
...
In this guide, you'll learn how to layer ngrok's [Global Server Load Balancing](/universal-gateway/network/gslb/) (GSLB) on top of a VM-based deployment on DigitalOcean.
...
You can reserve your domain in one of two ways: with the [ngrok API](/api/), or in the [ngrok dashboard](https://dashboard.ngrok.comdomains).
...
With ngrok, you can manage multiple endpoints, such as domains or routes, using a single [Edge](/universal-gateway/endpoints/edges/) configuration in the cloud. Edges let you update endpoints without taking them offline, connect multiple ngrok agents to a single endpoint, apply modules like [OAuth](/http/oauth/) or [IP restrictions](/http/ip-restrictions/), and balance the load between your upstream services.
...
With a [Weighted Backend](/universal-gateway/endpoints/edges/#weighted), you can specify custom weights for Tunnel Group backends, shaping precisely how to load-balance traffic across the infrastructure you've distributed around the globe. Without a Weighted Backend, the ngrok Edge and [GSLB](/universal-gateway/network/gslb/) will always favor the Point of Presence (PoP) and VM nearest the user making requests.
...
You can also navigate to the [Edges dashboard](https://dashboard.ngrok.comedges) to see your Edge configuration on the domain you reserved with no online tunnels.
...
To confirm your global load balancing, head over to the [Edges dashboard](https://dashboard.ngrok.comedges) once again to confirm your tunnels are online and weighted equally at 33.33%.
...
Your demo API is already globally load-balanced, but if you want to extend your ngrok usage even further, you can enable one or more [Traffic Policy modules](/http/traffic-policy/index.mdx) on your Edge to control traffic or establish standards for how your upstream services are accessed.

You have two options:

- Using `curl` with the ngrok API's [`/edges/https/{edge_id}/routes/{id}/policy` endpoint](/api/resources/edge-route-traffic-policy-module.mdx). For this step, you will need to have exported the `ROUTE_ID` environment variable in step #4.
...
- In the Traffic Policy module section of your [Edge](https://dashboard.ngrok.comedges) in the ngrok dashboard, click the **Edit Traffic Policy** button and then the **YAML** button and add the YAML below.
...
- Provision a Kubernetes cluster with the same workload and the ngrok [Kubernetes Operator](/k8s/index.mdx) to load-balance between VM- and Kubernetes-based deployments of the same API or application.
