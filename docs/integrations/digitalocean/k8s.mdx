...
To demonstrate how both [fanout ingress](/k8s/with-edges.mdx#simple-fanout) and [OAuth support](/http/oauth.mdx) work, you can quickly launch an open-source observability stack including [Prometheus](https://github.com/prometheus), [Grafana](https://github.com/grafana/grafana), and [Alertmanager](https://github.com/prometheus/alertmanager/) on your cluster.
...
If you followed along, you already used [name-based virtual hosting](/k8s/with-edges.mdx#name-based-virtual-hosting) to direct traffic to different apps with or without authentication enabled. You also might find the [simple fanout](/k8s/with-edges.mdx#simple-fanout) useful for quickly deploying secure ingress for multiple apps running on the same cluster.

Learn more about the ngrok Kubernetes Operator, or contribute to its ongoing development, by checking out the [GitHub repository](https://github.com/ngrok/ngrok-operator). We also have an extensive explainer for [how the Operator works](/k8s/how-it-works.mdx), plus additional [documentation](/k8s/index.mdx), which also includes details on leveraging the new Kubernetes Gateway API.
