...
1. Provision your cluster with the command below, replacing `NODE_SIZE` and `CLUSTER_NAME` with your chosen size and a relevant name.
   ...
   This command not only provisions your cluster, but also installs the [ngrok Kubernetes Operator](https://github.com/ngrok/kubernetes-ingress-controller) from their [1-Click catalog](https://marketplace.digitalocean.com/apps/ngrok-ingress-controller).
...
1. Get your the `AUTHTOKEN` and `API_KEY`, which the ngrok Kubernetes Operator will use to authenticate against your existing ngrok account.
