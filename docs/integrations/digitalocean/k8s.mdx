---
description: Add ingress to any app running in a Kubernetes cluster managed by DigitalOcean using the ngrok Kubernetes Ingress Controller.
---
...
To use the ngrok Kubernetes Ingress Controller with DigitalOcean:
...
3. [Configure the ngrok Kubernetes Ingress Controller](#configure-the-ngrok-kubernetes-ingress-controller)
4. [Enable extra features of ngrok's Cloud Edge](#enable-extra-features-of-ngrok)

:::

Using this guide, you'll launch a new cluster on [DigitalOcean](https://digitalocean.com) and use the DigitalOcean Marketplace to provision the [ngrok Kubernetes Ingress Controller](https://marketplace.digitalocean.com/apps/ngrok-ingress-controller) to securely ingress public traffic to a demo app.
...
In this guide, we'll stick with the CLI as much as possible to showcase how tightly the ngrok Kubernetes Ingress Controller can integrate with DigitalOcean and how you might automate these steps in the future.
...
1. Provision your cluster with the command below, replacing `NODE_SIZE` and `CLUSTER_NAME` with your chosen size and a relevant name.
   ...
   This command not only provisions your cluster, but also installs the [ngrok Kubernetes Ingress Controller](https://github.com/ngrok/kubernetes-ingress-controller) from their [1-Click catalog](https://marketplace.digitalocean.com/apps/ngrok-ingress-controller).
...
1. You can now verify that the cluster is running successfully by looking at the pods in the new `ngrok-ingress-controller` namespace... wait, `CreateContainerConfigError`?
   ...
   That's actually to be expected at this point. If you run `kubectl describe pods -n ngrok-ingress-controller YOUR_NGROK_INGRESS_CONTAINER`, you'd see the following error: `kubelet            Error: secret "ngrok-auth" not found`. Let's remedy that.

1. Get your the `AUTHTOKEN` and `API_KEY`, which the ngrok Kubernetes Ingress Controller will use to authenticate against your existing ngrok account.
...
1. Create a file named `ngrok_creds.yaml` on your local workstation with the below YAML, replacing `API_KEY` and `AUTHTOKEN` but retaining the quotation (`"`) marks.
   ...
   The ngrok Kubernetes Ingress Controller pod should pick up on the new secret and authenticate properly. 
   
   Your new Kubernetes cluster, managed by DigitalOcean, is all set up—and with the ngrok Kubernetes Ingress Controller installed with a single option on the CLI.

## **Step 2**: Deploy an example app on your DigitalOcean-managed cluster {#deploy-an-example-app}

Let's explore how you can apps or services to your DigitalOcean-managed Kubernetes cluster. The DigitalOcean has a repository of [sample Kubernetes apps](https://github.com/digitalocean/kubernetes-sample-apps/tree/master/bookinfo-example), including the [Bookinfo](https://github.com/digitalocean/kubernetes-sample-apps/tree/master/bookinfo-example) app, which we'll use.
...
## **Step 3**: Configure the ngrok Kubernetes Ingress Controller {#configure-the-ngrok-kubernetes-ingress-controller}
...
Instead, finish configuring the ngrok Kubernetes Ingress Controller to direct incoming requests directly to the container running the user-facing Bookinfo container. The [Bookinfo docs](https://github.com/digitalocean/kubernetes-sample-apps/tree/master/bookinfo-example) recommend port-forwarding to the `productpage` service, which runs on port `9080`—you can use that information to configure the ngrok Kubernetes Ingress Controller.

1. Create a new file called `bookinfo-ingress.yaml` on your local workstation. This configuration defines how the ngrok Kubernetes Ingress Controller will route traffic arriving on `NGROK_DOMAIN` to the `store-front` service on port `80` as defined in the manifest.
...
Learn more about the ngrok Ingress Controller, or contribute to its ongoing development, by checking out the [GitHub repository](https://github.com/ngrok/kubernetes-ingress-controller). We also have an extensive explainer for [how the Operator works](/docs/k8s/how-it-works.mdx), plus additional [documentation](/docs/k8s/index.mdx), which also includes details on leveraging the new Kubernetes Gateway API.
