...
3. [Configure the ngrok Kubernetes Operator](#configure-the-ngrok-kubernetes-kubernetes-operator)
4. [Enable extra features of ngrok's Cloud Edge](#enable-extra-features-of-ngrok)

:::

Using this guide, you'll launch a new cluster on [DigitalOcean](https://digitalocean.com) and use the DigitalOcean Marketplace to provision the [ngrok Kubernetes Operator](https://marketplace.digitalocean.com/apps/ngrok-kubernetes-operator) to securely ingress public traffic to a demo app.
...
1. Provision your cluster with the command below, replacing `NODE_SIZE` and `CLUSTER_NAME` with your chosen size and a relevant name.
   ...
   ```bash
   doctl kubernetes clusters create --size NODE_SIZE CLUSTER_NAME --1-clicks ngrok-operator
   ```
...
1. You can now verify that the cluster is running successfully by looking at the pods in the new `ngrok-operator` namespace.
   ...
   The `CreateContainerConfigError` is to be expected at this point. If you run `kubectl describe pods -n ngrok-operator YOUR_NGROK_INGRESS_CONTAINER`, you'd see the following error: `kubelet            Error: secret "ngrok-auth" not found`.
...
## **Step 3**: Configure the ngrok Kubernetes Operator {#configure-the-ngrok-kubernetes-operator}
