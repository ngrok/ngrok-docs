...
To use ngrok's [Global Load Balancer](/universal-gateway/network/gslb/) with Linode:
...
In this guide, you'll learn how to layer ngrok's [Global Load Balancer](/universal-gateway/network/gslb/) (GSLB) on top of a VM-based deployment on Linode.
...
- Using `curl` with the ngrok API's [`/edges/https/{edge_id}/routes/{id}/policy` endpoint](/api/resources/edge-route-traffic-policy-module.mdx). For this, you will need find the `id` of the catch-all Route you created in [step 4](#create-weighted-backend).
...
- In the Traffic Policy module section of your [Edge](https://dashboard.ngrok.comedges) in the ngrok dashboard, where you can click the **Edit Traffic Policy** button and add the YAML below.
