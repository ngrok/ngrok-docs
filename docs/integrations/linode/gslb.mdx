...
To use ngrok's [Global Server Load Balancing](/network-edge/gslb/) with Linode:
...
In this guide, you'll learn how to layer ngrok's [Global Server Load Balancing](/network-edge/gslb/) (GSLB) on top of a VM-based deployment on Linode.
...
Next, you need to reserve your subdomain with an [ngrok-managed domain](/network-edge/domains-and-tcp-addresses/) like `.ngrok.app`, `.ngrok.dev`, `.ngrok.pro`, or even `.ngrok.pizza`. For an example deployment like this, `YOUR_COMPANY-linode-gslb.ngrok.app` would work great.
