...
3. An account with the necessary privileges to create a container on one of the following managed Kubernetes services:
...
1. Create an ngrok static subdomain for ingress, if you don't have one already. Navigate to the [**Domains** section](https://dashboard.ngrok.com/cloud-edge/domains) of the ngrok dashboard and click **Create Domain** or **New Domain**. This static subdomain will be your `NGROK_DOMAIN` for the remainder of this guide.
...
1. Add your additional profiles by clicking on the **+** next to **Addon layers**. Choose the ngrok Kubernetes Ingress Controller profile, then repeat for the 2048 game profile.
