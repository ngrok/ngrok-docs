---
description: Add ingress to any app running in a Kubernetes cluster managed by Spectro Cloud's Palette platform using the ngrok Kubernetes Ingress Controller.
---
...
1. [Create an add-on cluster profile for the ngrok Kubernetes Ingress Controller](#create-a-cluster-profile-in-palette})
2. [Create your cluster with Palette](#create-your-cluster-with-palette)
3. [Create an add-on cluster profile for an example app and ngrok Kubernetes Ingress Controller](#create-add-on-cluster-profile-ngrok)
...
Palette, from [Spectro Cloud](https://www.spectrocloud.com/), is a profile-based Kubernetes management platform. It gives IT teams the control and visibility to give internal developers curated Kubernetes stacks without sacrificing governance, declarative lifecycle management, and supported environments.
...
You'll take the configuration and deployment steps modular and declarative.

If you already have a cluster profile, skip to [Create your cluster with Palette](#create-your-cluster-with-palette). If you already have a functioning cluster in Palette, skip to [Step 3: Create an add-on cluster profile for an example app and ngrok Kubernetes Ingress Controller](#create-add-on-cluster-profile-ngrok).
...
1. Palette now shows you the layers of this new cluster profile. You can also add new Packs, manifests, or Helm charts, but to save you progress for now, click **Next**, then **Finish Configuration**.

  ![Finalize your cluster profile](img/finalize-profile.png)

## **Step 2**: Create an add-on cluster profile for the ngrok Kubernetes Ingress Controller {#create-add-on-cluster-profile-ngrok}
...
For the former, Palette has a [ready-to-apply pack](https://docs.spectrocloud.com/integrations/ngrok/) for the ngrok Kubernetes Ingress Controller. For the latter, you can either add an example app as detailed in the following steps or connect to an existing app.
...
1. Add another manifest to create a Kubernetes ingress service, which will inform the ngrok Kubernetes Ingress Controller to create a new edge for your app. Name it `2048-ingress` and create an `ingress` manifest with the following YAML content, replacing the `NGROK_DOMAIN` variable with the subdomain you created, which should look like `one-two-three.ngrok-free.app`.
...
1. Add your additional profiles by clicking on the **+** next to **Addon layers**. Choose the profile for the ngrok Kubernetes Ingress Controller, then repeat for the 2048 game profile.
...
You can use the ngrok Kubernetes Ingress Controller and Palette to manage multiple routes on a single edge using [name-based virtual hosting](https://github.com/ngrok/kubernetes-ingress-controller/blob/main/docs/user-guide/ingress-to-edge-relationship.md#name-based-virtual-hosting) or [route
modules](https://github.com/ngrok/kubernetes-ingress-controller/blob/main/docs/user-guide/route-modules.md). [Custom domains](https://github.com/ngrok/kubernetes-ingress-controller/blob/main/docs/user-guide/custom-domain.md) and a [circuit breaker](/docs/http/circuit-breaker/) are also good next steps to make your integration production-ready.
