...
1. Install the ngrok Kubernetes Operator with Helm under a new `ngrok-operator` namespace, with the `useExperimentalGatewayApi=true` option.
...
1. Verify you have installed the ngrok Kubernetes Operator successfully via a single running pod.
   ...
   ```bash
   kubectl get pods --namespace ngrok-operator
