...
Certain modifications, like [changing headers](/k8s/?k8s-basic-path=gatewayAPI#modify-headers) and [redirecting traffic](/k8s/?k8s-basic-path=gatewayAPI#redirect-traffic) based on paths, are currently supported.
