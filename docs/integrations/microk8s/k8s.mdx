...
- **Canonical MicroK8s**: An open-source system for low-ops, minimal, production-ready Kubernetes. MicroK8s' single-node configuration, small size, and customizable footprint lets you use it for local development, in minimal environments like GitHub's CI/CD, or using Kubernetes on IoT hardware.
...
1. If you don't have a domain, you'll need to create one or use your [dev domain](/universal-gateway/domains/#dev-domains). Navigate to the [**Domains** section](https://dashboard.ngrok.com/domains) of the ngrok dashboard and click **Create Domain** or **New Domain**. This domain will be your `NGROK_DOMAIN` for the remainder of this guide.
...
1. Create a new file named `2048-gateway.yaml` on your workstation with the below YAML, replacing `$YOUR_DOMAIN`:
...
1. Access your 2048 demo app by navigating to your ngrok domain, e.g. `https://$YOUR_DOMAIN`. ngrok's network
