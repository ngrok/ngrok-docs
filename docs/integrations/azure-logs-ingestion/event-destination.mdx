...
# Azure Logs Ingestion Log Destination
...
You have now granted access for the ngrok application to ingest logs into the DCR, which is the final step before we can create an Log Destination.

## **Step 6**: Gather necessary data for Log Destination {#event-destination-data}
...
## **Step 7**: Create the new Log Destination in ngrok {#create-event-destination}
