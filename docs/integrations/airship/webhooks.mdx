...
ngrok's [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector) captures all requests made through your ngrok endpoint to your localhost app. Click on any request to view detailed information about both the request and response.

**Note**: By default, accounts only collect traffic metadata to avoid exposing secrets. Enable full capture in your account Settings to capture complete request and response data.
...
Test your webhook handling code without triggering new events from Airship using the Traffic Inspector's replay feature:

1. Enable full capture in your ngrok account settings to capture complete request data.

1. Send a test webhook from Airship to generate traffic in your Traffic Inspector.

1. Select the request you want to replay in the [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector).

1. Choose your replay option:
   - Click **Replay** to send the exact same request again
   - Select **Replay with modifications** to edit the request before sending

1. Modify the request (optional): Edit any part of the original request, such as changing the `device_type` field in the request body.
