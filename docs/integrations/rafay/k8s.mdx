...
- **The [ngrok Kubernetes Ingress Controller](https://ngrok.com/blog-post/ngrok-k8s)**: ngrok's official controller for adding secure public ingress and middleware execution to your Kubernetes apps with ngrok's Cloud Edge. With ngrok, you can manage and secure app traffic at every stage of the development lifecycle while benefitting from simpler configurations, security, and edge acceleration.
...
2. Create a file on your local workstation named `ngrok-values.yaml` with the values below, replacing the defaults with your Authtoken and new API key.
...
3. While you're in the ngrok dashboard, create an ngrok static subdomain for ingress. Navigate to the [**Domains**
...
1. Navigate to your ngrok subdomain, e.g. `https://NGROK_DOMAIN.ngrok.app`, in your browser to see your example app as managed, deployed, and publicly networked via Rafay, Kubernetes, and ngrok! 
