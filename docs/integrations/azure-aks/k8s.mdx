...
In this guide, you'll launch a new cluster with [Azure Kubernetes Service](https://azure.microsoft.com/en-us/products/kubernetes-service) and launch an demo app. You'll then add the ngrok Kubernetes Ingress Controller to route public traffic through an encrypted, feature-rich tunnel directly to your demo app for a complete proof of concept.

In the end, you'll have learned enough to deploy a real production-ready Kubernetes app with AKS.
...
AKS is a managed hosted Kubernetes environment from Microsoft. AKS simplifies the deployment, health monitoring, and maintenance of cloud native applications, whether you deploy them in Azure, in on-premises data centers, or at the edge. With [40 regions](https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/?products=kubernetes-service), you should be able to deploy a cluster close to your customers.

Together, the ngrok Kubernetes Ingress Controller and AKS abstract complexity away from your entire networking stack (both internal and external traffic), while giving you access to additional monitoring features and more resiliency.
...
2. An [Azure account](https://azure.microsoft.com/en-us/pricing/purchase-options/pay-as-you-go).
...
Start by creating a new managed Kubernetes cluster in AKS. If you already have one, you can skip to [Step 2: Install the ngrok Ingress Controller](#install-the-ngrok-ingress-controller).
...
1. Configure your new cluster with the wizard. The default options are generally safe bets, but there are a few you might want to look at depending on your needs, how your Azure subscription is configured, and budget:
...
To showcase how this integration works, you'll deploy the [AKS Store](https://github.com/Azure-Samples/azure-voting-app-redis) app, which uses a microservices architecture for connecting frontend UI to API-like services, which pass data to RabbitMQ and MongoDB in the backend. To showcase the features of AKS, you'll deploy this demo app directly in the Azure Portal.

:::tip

If you prefer the CLI, you can also save the YAML below to a `.yaml` file on your local workstation and deploy with `kubectl apply -f ...`.
...
Now that your demo app is publicly accessible through the ngrok Cloud Edge, you can quickly layer on [additional capabilities](https://ngrok.com/docs/http/), like authentication, without having to configure and deploy complex infrastructure. Let's see what that looks like for restricting access to your demo app to the owners of specific email addresses or domains.
