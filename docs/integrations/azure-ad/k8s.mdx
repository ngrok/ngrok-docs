...
Next, you'll configure and deploy the [ngrok Kubernetes Operator](https://github.com/ngrok/ngrok-operator) to expose your app to the public internet through the ngrok Cloud Endpoint.
...
1. Apply the `boutique-ingress.yaml` manifest you just created.
   ...
   Give your cluster a few moments to launch the necessary resources and for ngrok's Cloud Endpoint to pick up the new tunnel.
...
Your Kubernetes-based app is now publicly accessible through the ngrok Cloud Endpoint—to restrict access to only authorized users with proper Microsoft Entra ID credentials, you first need to enable SAML on your Edge.
