---
title: Okta SSO (SAML) for your ngrok endpoints
sidebar_label: Okta Endpoint SSO (SAML)
...
This article details how to configure Okta as the primary Identity Provider for ngrok endpoints.
By integrating Okta SSO with ngrok, you can:

- **Restrict access to ngrok endpoints** only to users authenticated via Okta
- **Use Okta security policies, MFA authenticators** — including Okta Verify, FastPass, and FIDO2 — **and ThreatInsights to control access to ngrok endpoints**.
- **Use Okta's Dashboard to facilitate access to ngrok apps**.

## Requirements

To configure ngrok endpoints with Okta, you must have:

- an Okta account with administrative rights to create apps
- an [ngrok Pay-as-you-go Account](https://ngrok.com/pricing) with an authtoken or admin access to configure edges with SAML.
