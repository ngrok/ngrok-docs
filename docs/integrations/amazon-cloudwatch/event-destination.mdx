...
1. Using a browser, go to the [ngrok dashboard](https://dashboard.ngrok.com) and navigate to **Log Exporting** under the **Traffic Observability** section on the left hand navigation and select **Create Subscription**.
...
To send the events to CloudWatch we'll need to assign an Log Destination to the Log Export.
...
4. Once all required inputs have values, select **Send Test Event** and you should be presented with a Success message. Select **Done** and the CloudWatch Log Destination setup is complete.
