...
1. [Create Log Destination](#create-destination)
...
## **Step 1**: Get prerequisite information: {#prerequisites}
...
- Acquire a Datadog API key. To either create or retrieve the API key, reference the following link [Datadog API documentation](https://docs.datadoghq.com/account_management/api-app-keys/#add-an-api-key-or-client-token).

## **Step 2**: Create a Log Export {#create-log-export}
...
Once complete, select **Add Log Sources** to confirm your selections.

![ngrok event sources](img/event_sources.png)

## **Step 3**: Create Log Destination {#create-destination}

To send the events to Datadog we'll need to assign an Log Destination to the Log Export.
...
3. Once all required inputs have values, select **Send Test Event** and you should be presented with a Success message. Select **Done** and the Datadog Log Destination setup is complete.
