---
description: Use Descope OIDC to secure access to ngrok tunnels
---
...
To secure access to ngrok with Descope Single Sign-On using OpenID Connect:
...
This article details how to configure Descope as the primary Identity Provider for ngrok tunnels.
By integrating Descope SSO with ngrok, you can:

- **Restrict access to ngrok tunnels** only to users authenticated via Descope
...
- an Descope account with administrative rights to a Descope Project
...
1. Access your ngrok tunnel (i.e., `https://descope-sso-test.ngrok.io` or using the copied endpoint URL).
