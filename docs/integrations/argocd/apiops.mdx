...
The project comes with a basic example of such a Traffic Policy—[see `traffic-policy.yaml` for details](https://github.com/joelhans/ngrok-apiops-demo/blob/main/traffic-policy.yaml)—but because rate limiting is typically the first order of business in protecing any API from abuse, let's walk through that process.
...
1. Test your new rate limiting policy by `curl`-ing your API in a quick loop.

   ```bash
   for i in `seq 1 20`; do \
     curl -s -o /dev/null \                                                   
       -w "\n%{http_code}" \
       -X GET https://ngrok-apiops-demo.ngrok.app/legend ; \
   done
   ```

   You should see [`429` response codes](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/429), as your ngrok API gateway begins to rate limit your IP address.
