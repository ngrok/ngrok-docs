...
- ngrok's [API gateway](https://ngrok.com/use-cases/api-gateway), that works out-of-the-box with a globally available edge network.
...
- The new [**Kubernetes Gateway API**](https://kubernetes.io/docs/concepts/services-networking/gateway/), a role-oriented mechanism for provisioning load-balancing infrastructure and routing traffic, with fantastic support for [developer-defined](https://ngrok.com/blog-post/developer-defined-golden-path-kubernetes-gateway-api) paths to deploying APIs in production.
...
Start by deploying the [ngrok Kubernetes Operator](https://github.com/ngrok/kubernetes-ingress-controller), which will operate as your production-grade API gateway, to your Kubernetes cluster.
...
Next, you need to set up the Git repository for the API you'll deploy behind your ngrok API gateway. Because GitOps (and thus APIOps) relies upon all configurations being declarative and version-controlled, you must configure the hostname of your deployment directly in a Git repository, which means you can't simply clone the demo API and apply to your cluster.
...
1. Fork the repository for the demo API at [joelhans/ngrok-apiops-demo](https://github.com/joelhans/ngrok-apiops-demo).

1. Open `gateway.yaml` and replace the content of lines 18 and 37 with the ngrok domain you just created.
...
The project comes with a very basic example of such a Traffic Policy—[see `traffic-policy.yaml` for details](https://github.com/joelhans/ngrok-apiops-demo/blob/main/traffic-policy.yaml)—but because rate limiting is typically the first order of business in protecing any API from abuse, let's walk through that process.
...
1. In your fork of the demo API project, create a new file called `rate-limiting.yaml`. Open the file and add the following YAML:
   ...
   This example policy includes a very low limit for demonstration purposes, but you can change the `capacity` value to your needs in production.

1. Add, commit, and push this change to your Git repository. Argo CD will pick up the changes from your repository with its next auto sync reconcile state with the `NgrokTrafficPolicy` CRD.
...
If you're looking for more Traffic Policy opportunities, including JWT authentication, check out our [library of "drop-in" examples](https://ngrok.com/blog-post/api-gateway-policy-management-examples) and the full [reference documentation](/docs/http/traffic-policy/index.mdx).
