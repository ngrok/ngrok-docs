...
Alternatively, clone your repository locally, add some content, commit, and then push the content to your repository on GitLab:
...
1. Create a traffic policy file named `gitlab_policy.yml`, replacing `  your_secret_token` with the value you have provided to the **Secret token** field (See [Integrate GitLab](#setup-webhook)):
