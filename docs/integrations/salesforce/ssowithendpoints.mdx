...
In this guide, you will use Salesforce as an OAuth provider along with the ngrok OIDC traffic policy action to secure your ngrok resources. You will use Salesforce as the primary IdP for your endpoints and the OIDC traffic policy action will supplement an ID token that will be used to identify users that log in to your endpoint.
...
#### Through Salesforce you will obtain:

- An external client application issuer URL
- A client ID that will be used in your traffic policy
- A client secret that will be used in your traffic policy

#### Through ngrok’s OIDC flow you will obtain:

- An ID token that can be used to identify users
- The ability to route and restrict access at a more granular level based on user metadata
...
### Step 4: Verify your Salesforce External Client Application Configuration
...
### Step 6: Create a Cloud Endpoint and Configure OIDC Traffic Policy Action
...
4. Follow the steps found here:
   [Create a Cloud Endpoint](https://ngrok.com/docs/universal-gateway/cloud-endpoints/quickstart/#dashboard) to create your cloud endpoint.
