...
while other developers or teams can create, update, and manage configuration resources such as `HTTPRoute` that define which services the traffic should be sent to.
This allows your operations/infrastrucure team to focus on the higher-level policy about what kinds of traffic to accept while allowing other teams to self-service deploying their
