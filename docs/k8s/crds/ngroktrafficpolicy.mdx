...
The `NgrokTrafficPolicy` CRD does not contain a status. This is not a resource created in the ngrok API directly. Instead other CRDs like `AgentEndpoint` and `CloudEndpoint` reference a `NgrokTrafficPolicy` by name.

If there are errors in the Traffic Policy, these messages will be shown on the other CRD that is trying to be created with this Traffic Policy Configuration.

🚧 **Note:** Currently ngroktrafficpolicy.status duplicates the policy json on its status but this will be removed in a future version.
