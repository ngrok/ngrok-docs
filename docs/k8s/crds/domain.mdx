...
| [`status.acmeChallengeCnameTarget`](#statusacmechallengecnametarget) | `string` | ACMEChallengeCNAMETarget is the CNAME target for ACME challenge (wildcards only)                                                         |
| [`status.certificate`](#statuscertificate)   | `Object`          | Certificate contains information about the TLS certificate                                                                               |
| [`status.certificateManagementPolicy`](#statuscertificatemanagementpolicy) | `Object` | CertificateManagementPolicy contains the certificate management configuration                                                             |
| [`status.certificateManagementStatus`](#statuscertificatemanagementstatus) | `Object` | CertificateManagementStatus contains the certificate management status                                                                   |
| [`status.conditions`](#statusconditions)     | `[]Object`        | Conditions represent the latest available observations of the domain's state                                                             |
...
Conditions represent the latest available observations of the domain's state. The ngrok operator uses several condition types to track the domain lifecycle and provide detailed status information.
