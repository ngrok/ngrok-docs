...
The `status` field contains the current state of the `AgentEndpoint` as reported by the ngrok operator.
...
| ---------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| [`status.assignedURL`](#statusassignedurl)     | `string`   | The assigned URL. This will either be the user-supplied url, or the generated assigned url depending on the configuration of spec.url |
| 🚧 [`status.domain`](#statusdomain)            | `Object`   | TODO: Reference to the Domain resource associated with this endpoint. For internal endpoints, this will be nil                        |
| 🚧 [`status.conditions`](#statusconditions)    | `[]Object` | Conditions describe the current conditions of the AgentEndpoint                                                                       |
| [`status.trafficPolicy`](#statustrafficpolicy) | `string`   | Identifies any traffic policies attached to the AgentEndpoint ("inline", "none", or reference name)                                   |

### `status.assignedURL`

The assigned URL. This will either be the user-supplied url, or the generated assigned url depending on the configuration of spec.url.

**Type:** `string`

### 🚧 `status.domain`

🚧 **TODO:** Reference to the Domain resource associated with this endpoint. For internal endpoints, this will be nil.
...
| Field Name                                          | Type     | Description                      |
| --------------------------------------------------- | -------- | -------------------------------- |
| [`status.domain.name`](#statusdomainname)           | `string` | Name of the Domain resource      |
| [`status.domain.namespace`](#statusdomainnamespace) | `string` | Namespace of the Domain resource |

### 🚧 `status.domain.name`

🚧 **TODO:** Name of the Domain resource.

**Type:** `string`

### 🚧 `status.domain.namespace`

🚧 **TODO:** Namespace of the Domain resource.
...
🚧 **TODO:** Conditions describe the current conditions of the AgentEndpoint. The ngrok operator uses several condition types to track the endpoint lifecycle and provide detailed status information.

**Type:** `[]Object`

#### Condition Types

The ngrok operator sets the following condition types on AgentEndpoint resources:

| Condition Type         | Description                                         |
| ---------------------- | --------------------------------------------------- |
| `Ready`                | Overall endpoint readiness status                   |
| `DomainReady`          | Whether the domain is ready for use                 |
| `EndpointCreated`      | Whether the endpoint was successfully created       |
...
| Reason                 | Description                             |
| ---------------------- | --------------------------------------- |
| `EndpointActive`       | Endpoint is active and ready            |
| `DomainReady`          | Domain is ready for use                 |
| `EndpointCreated`      | Endpoint was successfully created       |
...
| `EndpointError`        | Error creating endpoint                 |

## Status Examples

### Successful AgentEndpoint

An AgentEndpoint that is fully provisioned and ready for use:

```yaml
status:
  assignedURL: https://bezek-is-back.ngrok.app
  domain:
    name: example-domain
    namespace: default
  conditions:
  - lastTransitionTime: "2025-09-22T15:22:33Z"
    message: AgentEndpoint is active and ready
    observedGeneration: 1
    reason: EndpointActive
    status: "True"
    type: Ready
  - lastTransitionTime: "2025-09-22T15:17:46Z"
...
### AgentEndpoint with Traffic Policy Error

An AgentEndpoint that has an invalid traffic policy configuration:
...
status:
  ...
  - lastTransitionTime: "2025-09-22T15:17:34Z"
    ...
    status: "False"
    type: Ready
  - lastTransitionTime: "2025-09-22T15:17:44Z"
    message: Domain is ready
    observedGeneration: 1
    reason: DomainReady
    status: "True"
...
### AgentEndpoint Waiting for Domain

An AgentEndpoint that is waiting for its domain to be ready:
