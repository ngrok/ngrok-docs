...
Start by reading the [ingress-to-edge-relationship](/k8s/with-edges.mdx) documentation to understand how ingress, gateway, and route objects are converted into ngrok edges.
...
ngrok offers the ability to restrict access to your edges by IP address via [IP Restrictions](/http/ip-restrictions/).
...
ngrok's Cloud Edge [Modules](/http/#modules) allow you to configure features like compression, IP Restrictions, OAuth, adding/removing headers, and more.
...
[Circuit breakers](/http/circuit-breaker/) are used to protect upstream servers by rejecting traffic to them when they become overwhelmed.
...
The [Request Headers](/http/request-headers/) module allows you to add and remove headers from HTTP requests before they are sent to your upstream server.
...
The [Response Headers module](/http/response-headers/) allows you to add and remove headers from HTTP responses before they are returned to the client.
...
[IP Restrictions](/http/ip-restrictions/) allow you to attach one or more IP policies to the route.
...
The [OAuth module](/http/oauth/) enforces an OAuth authentication flow in front of any route it is enabled on.
...
The [OIDC module](/http/openid-connect/) restricts endpoint access to only users authorized by a OpenID Identity Provider.
...
The [SAML module](/http/saml/) restricts endpoint access to only users authorized by a SAML IdP.
...
This [mutual TLS (mTLS) module](/http/mutual-tls/) performs authentication when the ngrok edge terminates TLS on incoming connections to your HTTP endpoint. The client must present a valid TLS certificate that is signed by one of the specified CAs or the connection will be rejected.
...
The [webhook verification module](/http/webhook-verification/) allows ngrok to assert requests to your endpoint originate from a supported webhook provider like Slack or Github.
...
- [Policy Expressions](/http/traffic-policy/expressions/)
- [Policy Actions](/http/traffic-policy/actions/)
...
ngrok offers [TCP](/tcp/) and
[TLS](/tls/) Edges which can be used to
...
By default, services of type `LoadBalancer` are exposed using a ngrok [TCP Edge](/tcp/#edges). A reserved address is automatically created for the service and the service's status will be updated with the reserved address. Other projects like [external-dns](https://github.com/ngrok/ngrok-operator/docs/examples/external-dns/README.md) can be used to create a CNAME record for the reserved address automatically.
...
**Note**: the `k8s.ngrok.com/domain` annotation is required to use TLS and will expose the service as a [TLS Edge](/tls/).
...
Policies](/tls/ip-restrictions/) for limiting access
...
Agent Endpoints along with [Cloud Endpoints](/network-edge/cloud-endpoints/) are the successors to [Edges](/network-edge/edges/). Agent Endpoints are ephemeral and tied to the lifetime of the ngrok agent.
...
[TLS endpoints](/tls/) enable you to deliver any network service that runs over a TLS-based protocol. TLS endpoints make no assumptions about the wrapped protocol being transported.
...
Agent Endpoints with a `url` ending in `.internal` can be used to create [Internal Endpoints](/network-edge/internal-endpoints/).
...
The following showcases how you could also use a [Cloud Endpoint](/network-edge/cloud-endpoints/) to forward traffic to your internal Agent Endpoint
