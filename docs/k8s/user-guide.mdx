...
By default, services of type `LoadBalancer` are exposed using a ngrok [TCP Edge](/universal-gateway/tcp/#edges). A reserved address is automatically created for the service and the service's status will be updated with the reserved address. Other projects like [external-dns](https://github.com/ngrok/ngrok-operator/docs/examples/external-dns/README.md) can be used to create a CNAME record for the reserved address automatically.
...
Policies](/traffic-policy/actions/ip-restrictions/) for limiting access
