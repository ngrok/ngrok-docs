...
If you have [ExternalDNS](https://github.com/kubernetes-sigs/external-dns) installed and configured, you can create an Ingress object to fully automate the process of reserving domains in ngrok, updating DNS records with your DNS provider to the ngrok-provided CNAME record, and receiving TLS certificates.
...
For example, if you create an ingress object like one shown below, the operator will attempt to reserve and register the domain `foo.bar.com` with ngrok:
