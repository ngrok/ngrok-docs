...
Navigate to the [bot users](https://dashboard.ngrok.com/service-users) section of your dashboard and click on “new bot user”.

![Bot User interface screenshot](./_img//bot-user-ss-1.png)

Create an authtoken for that bot user and assign it an ACL rule. By assigning an ACL rule, you can restrict what endpoints the bot user with this authtoken can create. Navigate to the authtokens section on your dashboard and click “new authtoken”. Assign it an ACL rule as shown below to restrict any agent using it with permission to only create internal endpoints with a URL matching `*.cust1.internal`.

![Bot User configuration interface screenshot](./_img//bot-user-ss-2.png)
