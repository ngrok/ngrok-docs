...
The Ingress Controller within the ngrok Operator uses Kubernetes Ingress to connect your Kubernetes cluster and the ngrok service.
...
This only can be applied to ingress controllers that create static IPs to route to. With ngrok, an Edge always has a hostname, otherwise it's not route-able. If a rule with no host is created, it will be dropped.
...
The [NgrokModuleSet CRD](./user-guide/crds.md#ngrok-module-sets) is a collection of route module configurations. When set on an ingress object, it applies to all routes on that ingress object, but not routes on other ingress objects that may be combined to form the same edge.
...
- Kubernetes Ingress is simple, making annotations the thing only to add things like policies or modules: The Route objects have the capability to add additional rules to direct and modify traffic.
...
The ngrok Operator currently supports matches and multiple filters for HTTPRoute. Some of these filters align with modules/policies that are supported by ngrok. These are implemented using ngrok policy and will generate a policy document that is uploaded to the ngrok service.
