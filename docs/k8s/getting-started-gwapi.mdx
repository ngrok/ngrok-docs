...
This would modify the headers by removing the `x-request-id` header, adding an additional one and setting another header to a specific value.
...
additional charge. You only incur costs if the resources provisioned by your usage of the Gateway API, such as endpoints or traffic policies, incur a cost. More information canb be found on the [pricing page](https://ngrok.com/pricing)
