...
<Tabs groupId="kind" queryString="kind">
	...
	❌ `Ingress` resources do not natively support IP-based restrictions, but they can be extended to take advantage of ngrok's [IP restriction policy action](/traffic-policy/actions/restrict-ips/).
  </TabItem>
	<TabItem value="gateway-api" label="Gateway API">
	❌ Gateway API resources do not natively support IP-based restrictions, but they can be extended to take advantage of ngrok's [IP restriction policy action](/traffic-policy/actions/restrict-ips/).
