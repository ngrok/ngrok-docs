...
<Tabs groupId="kind" queryString="kind">
	...
	❌ `Ingress` resources do not natively support response compression, but they can be extended to take advantage of ngrok's [compress response traffic policy action](/traffic-policy/actions/compress-response/).
  </TabItem>
	<TabItem value="gateway-api" label="Gateway API">
	❌ Gateway API resources do not natively support response compression, but they can be extended to take advantage of ngrok's [compress response traffic policy action](/traffic-policy/actions/compress-response/).
  </TabItem>
</Tabs>
<!-- prettier-ignore-end -->
