...
The following examples show how to set up an endpoint that accepts requests that contian basic auth credentials for either `user:password1` or `admin:password2`.
...
<Tabs groupId="kind" queryString="kind">
	...
	If you prefer, `NgrokTrafficPolicy` can also be used on the route level by using an `externalRef` filter on an `HTTPRoute`. See the [using Gateway API guide](/kubernetes/guides/using-gwapi) for examples.
