...
❌ `Ingress` and Gateway API resources do not natively support circuit breaking, but they can be extended to take advantage of ngrok's [circuit breaking traffic policy action](/traffic-policy/actions/circuit-breaker/).
