...
❌ `Ingress` and Gateway API resources do not natively support request denial configuration, but they can be extended to take advantage of ngrok's [deny request traffic policy action](/traffic-policy/actions/deny/).
