...
❌ `Ingress` and Gateway API resources do not natively support JWT validation, but they can be extended to take advantage of ngrok's [JWT validation traffic policy action](/traffic-policy/actions/jwt-validation/).
