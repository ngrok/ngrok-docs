...
<Tabs groupId="kind" queryString="kind">
	...
	❌ `Ingress` resources do not natively support rate limiting, but they can be extended to take advantage of ngrok's [rate limit traffic policy action](/traffic-policy/actions/rate-limit/).
  </TabItem>
	<TabItem value="gateway-api" label="Gateway API">
	❌ Gateway API resources do not natively support rate limiting, but they can be extended to take advantage of ngrok's [rate limit traffic policy action](/traffic-policy/actions/rate-limit/).
  </TabItem>
</Tabs>
<!-- prettier-ignore-end -->
