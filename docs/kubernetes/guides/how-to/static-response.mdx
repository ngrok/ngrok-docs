...
<Tabs groupId="kind" queryString="kind">
	...
	❌ `Ingress` resources do not natively support serving custom responses, but they can be extended to take advantage of ngrok's [custom response traffic policy action](/traffic-policy/actions/custom-response/).

  </TabItem>
	<TabItem value="gateway-api" label="Gateway API">
	❌ Gateway API resources do not natively support serving custom responses, but they can be extended to take advantage of ngrok's [custom response traffic policy action](/traffic-policy/actions/custom-response/).
