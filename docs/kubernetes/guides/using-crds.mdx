...
pagination_next: kubernetes/guides/using-ingresses
---
...
You can also head to the [Custom Resource Definition docs](/kubernetes/crds) for more detailed information about the structure of each resource and all of their fields.
...
The [endpoint types page](/kubernetes/guides/endpoint-types) showcases using different protocols with `AgentEndpoint` and/or `CloudEndpoint` resources.
See the [AgentEndpoint Reference](/kubernetes/crds/agentendpoint) for more detailed information about `AgentEndpoint` structure and fields.
...
The [endpoint types page](/kubernetes/guides/endpoint-types) showcases using different protocols with `AgentEndpoint` and/or `CloudEndpoint` resources.
See the [CloudEndpoint Reference](/kubernetes/crds/cloudendpoint) for more detailed information about `CloudEndpoint` structure and fields.
...
For a more in-depth guide on using `CloudEndpoints` to forward requests to other endpoints using the `forward-internal` action, see the [request routing guide](/kubernetes/guides/request-routing)
...
`Ingress` and Gateway API resources have several methods of accepting `NgrokTrafficPolicy` resources outlined in the [using ingresses](/kubernetes/guides/using-ingresses) and [using Gateway API](/kubernetes/guides/using-gwapi) pages.
...
See the [NgrokTrafficPolicy Reference](/kubernetes/crds/ngroktrafficpolicy) for more detailed information about `NgrokTrafficPolicy` structure and fields.
...
- BoundEndpoints project endpoints with a binding type of `kubernetes` into one or more Kubernetes clusters running the ngrok operator.
  For more information on endpoint bindings, refer to [the bindings page](/kubernetes/guides/bindings).
- [BoundEndpoint Reference](/kubernetes/crds/boundendpoint)
  **KubernetesOperators**:
...
- [KubernetesOperator Reference](/kubernetes/crds/kubernetesoperator)
