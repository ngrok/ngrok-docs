...
ngrok supports the concept of endpoint bindings which allow you to controll how your endpoints are accessed.
...
This can be used within a single cluster to use ngrok traffic policies on your endpoints for `Service` to `Service` networking in order to perform operations such as rate-limiting, header manupulation, etc.
...
See the [operator Helm page](/kubernetes/installation/helm) for more information about the `bindings` value and all the other configuration options such as configuring custom annotations/labels on the projected services created by Kubernetes bound endpoints.
...
For more information about bindings, see [the bindings page](/kubernetes/guides/bindings)
