...
Before updating, it is always best to check which verion of the ngrok Kubernets operator you are running
and comparing your current image version (not the helm chart version) to the most recent version or the version you
want to update to. You can refer top the [operator changelog](/kubernetes/changelog) to view what has changed since the
...
## 5. Update the Operator with Helm
