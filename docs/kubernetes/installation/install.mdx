---
title: Install
pagination_next: kubernetes/guides/quickstart
---
...
**Need a quick cluster for testing/development? Check out the [local cluster guide](/kubernetes/guides/local-cluster) first!**
...
You can select either the standard or experimental set of Gateway API CRDs depending on your preference. Either verison will work with the ngrok Kubernetes operator.
...
Many of the development versions of the ngrok Kubernetes opator follow that format with `rc` standing for Release Candidate. These are early development builds before an official release
...
Head to the [quickstart guide](/kubernetes/guides/quickstart) to get started with an example deployment.
