...
pagination_next: kubernetes/crds/cloudendpoint
---
...
re-use of common traffic policies such as enforcing ratlimiting/authentication across all of your endpoints without needing to redefine the traffic policy on each individual resource that needs to use it.
...
| ---------------------------- | ----------------- | -------- | ------- | ------------------------------------------------------ |
| [`spec.policy`](#specpolicy) | `json.RawMessage` | yes      | none    | Configuraiton for the traffic policy rules and actions |
