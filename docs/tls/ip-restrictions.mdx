...
import AgentCliExample from "/examples/agent-cli/tls-ip-restrictions.mdx";
import AgentConfigExample from "/examples/agent-config/tls-ip-restrictions.mdx";
import GoSdkExample from "/examples/go-sdk/tls-ip-restrictions.mdx";
import JavascriptSdkExample from "/examples/javascript-sdk/tls-ip-restrictions.mdx";
import KubernetesExample from "/examples/k8s/tls-ip-restrictions.mdx";
import PythonSdkExample from "/examples/python-sdk/tls-ip-restrictions.mdx";
import RustSdkExample from "/examples/rust-sdk/tls-ip-restrictions.mdx";
import SshExample from "/examples/ssh/tls-ip-restrictions.mdx";
...
| Parameter         | Description                                                                                                                                                                                                      |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **IP Policy IDs** | A set of IP policies that will be used to check if a source IP is allowed access. See the [TLS Edge IP Restrictions Module API Resource](/api/resources/tls-edge-ip-restriction-module/) for additional details. |
...
IP Restrictions is a supported module for TLS Edges. When using IP Restrictions
via Edges, you specify a set of references to one or more IP Policy objects,
each of which contains a list of IP Policy Rule objects.
