...
There are [over 40 event types published](/obs/reference) via ngrok at the time of writing.
They break down into two important classes of event:

- **[Traffic Events](/obs/reference/#traffic-events)**: When traffic transits through your endpoints like processing an HTTP request or TCP connection
- **[Audit Events](/obs/reference/#audit-events)**: Changes to your account like create/update/delete of Domains, API Keys, IP Policies, etc.
...
Event subscriptions define which [Event Sources](/obs/reference/) to capture and which
...
ngrok emits [Audit Events](/obs/reference/#audit-events) when changes are made to
...
Events](/obs/reference/#traffic-events) like processing an HTTP request or TCP
connection.
...
Events](/obs/reference/#traffic-events) since the velocity of these events can be quite high. Filters are a boolean
...
#### [ip_policy_created.v0](/obs/reference/#ip-policy) event
