...
If you don't have an app to work with, you can create a minimal a Rust app to set up a basic HTTP server at port `8080`.

To do so, create a new Rust project using the following terminal commands:
...
Then, replace the contents of `src/main.rs` with the following code.

```rust filename="service/src/main.rs"
...
In the root `service`, execute the following command to start the server:
...
First, create a new Rust project in the terminal and navigate into it.
...
```bash
cargo new ngrok-rust-demo
cd ngrok-rust-demo
```

Next, add the ngrok Rust SDK to your project by running the following commands:
