...
1. [Node.js installed](https://www.nodejs.org/) on your machine. You can check this by running `node -v` in your terminal.
...
If you don't already have a local app you'd like to work with, you can create a minimal Node.js app with the following code to set up a basic HTTP server at port `8080`.
...
```bash
mkdir ngrok-nodejs-demo && cd ngrok-nodejs-demo
touch .env
```
...
You'll need to install [the `dotenv` package](https://www.npmjs.com/package/dotenv) to access your environment variable.
...
:::tip
If you don't want to use a `.env` file, you can instead [export your auth token as an environment variable](https://support.apple.com/en-il/guide/terminal/apd382cc5fa-4f58-4449-b20a-41c53c006f8f/mac) in your terminal session, or [call `TK`](TK) in your code.
:::
