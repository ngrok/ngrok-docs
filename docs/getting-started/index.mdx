...
ngrok makes it simple to add authentication to your app using a [Traffic Policy](/traffic-policy/).

Create a new file in your project directory to define the policy—you can use YAML or JSON format.
The policy below instructs the agent endpoint to redirect the user to Google OAuth when they visit the public URL.
...
Terminate the agent endpoint that you started up in step 5, then restart it with the command below—replace `$PORT` as before, and replace `$YOUR_TRAFFIC_POLICY_FILE` with the corresponding file name (such as `policy.yml`).

```bash
ngrok http $PORT --traffic-policy-file $YOUR_TRAFFIC_POLICY_FILE
```
