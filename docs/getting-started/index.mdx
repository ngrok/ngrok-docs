...
Common use cases include granting remote access to IoT devices, enabling your Kubernetes clusters to communicate with each other, allowing external connections to a local gaming server, and more.

The simplest way to accomplish this is to create an endpoint that forwards public traffic to your localhost using the [ngrok Agent CLI](/agent/).
...
:::tip
This example uses ngrok's default Google OAuth application. To use your own, see [the OAuth Traffic Policy Action documentation](https://ngrok-docs-git-sg-doc-326-agent-endpoints-cli-f7e245-ngrok-dev.vercel.app/docs/traffic-policy/actions/oauth/#google-example).
:::
