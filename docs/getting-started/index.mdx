...
Below the auth token, paste the following snippet into the editor and then follow the instructions to exit and save the config file.
...
- If you need to interact with ngrok agent endpoints programmatically, we recommend using one of our Agent SDK packages which are available for [JavaScript (Node.js)](/getting-started/javascript/), [Go](/getting-started/go/), [Python](/getting-started/python/), and [Rust](/getting-started/rust/).
