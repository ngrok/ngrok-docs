...
ngrok makes it simple to add authentication to your app using the [Traffic Policy](/traffic-policy/) feature.

Create a new file in your project directory to define the policy—you can use YAML or JSON format.
The policy below says that whenever an HTTP/S request is made to the public URL, the agent endpoint should redirect the user to Google OAuth for authentication.
...
Terminate the agent endpoint that you started up in step 5, then restart it with the command below—replace `$PORT` as before, and replace `$YOUR_TRAFFIC_POLICY_FILE` with the corresponding file name (such as `policy.yml`) to let the agent know where to find it.
