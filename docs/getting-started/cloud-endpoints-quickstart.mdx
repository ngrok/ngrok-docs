...
Cloud Endpoints are a persistent way to service traffic. 
Unlike Agent Endpoints, which are ephemeral and run locally, cloud endpoints are persistent and run on ngrok's cloud service—allowing you to accept public traffic even if your internal apps or services are down.
...
A common use case for this pattern is to create a cloud endpoint that accepts public traffic, authenticates the visitor, then routes them to one or more internal agent endpoints running on machines that should only be accessed by privileged members of your organization.
...
:::note 
This doc assumes you've already completed the [Agent CLI Quickstart](/getting-started/)—if not, you should start there to learn about ngrok's fundamental features before continuing here.
:::
...
Next you'll be taken to a page where you can manage your new endpoint's [Traffic Policy](/traffic-policy/).
...
There are many different things you could do with your cloud endpoint from here.
Let's put it to work servicing an app running on your local device.
...
- See [the cloud endpoints overview](/universal-gateway/cloud-endpoints/) to learn more about use cases, how they compare to agent endpoints, and more.
- If you need to interact with cloud endpoints programmatically, you should consider using one of ngrok's API client libraries which are available for [Go](/), [Python](/), and [Rust](/).
- See [Load Balancing with Cloud Endpoints](/guides/other-guides/forwarding-and-load-balancing-with-cloud-endpoints) to learn how to distribute traffic across multiple internal endpoints.
- This quickstart only scratches the surface of what's possible with a traffic policy—check out the [Actions overview](/traffic-policy/actions/) to see what else you can do.
