...
Now that you're familiar with the key concepts necessary to work with cloud endpoints, try creating another one using the ngrok API, which you can access via the Agent CLI.
...
To see the fallback response, terminate your agent endpoint and then visit your cloud endpoint URL again.
...
- If you need to interact with cloud endpoints programmatically, we recommend using one of our API client libraries which are available for [Go](/), [Python](/), and [Rust](/).
