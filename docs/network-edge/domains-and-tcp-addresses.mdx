...
It is possible to reserve your domain apex (i.e. `example.com`) using the ngrok dashboard or API.
However, you will need to create a `CNAME` record in place of your `A` record, which not all DNS providers support.
If you are trying to reserve your domain apex because you need to create endpoints for multiple subdomains, you should
