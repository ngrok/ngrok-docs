...
Similar to [Domains](#domains), TCP Addresses are region-independent. They
support [global server load balancing](/network-edge/gslb). TCP addresses
provisioned in one region are available to agents connected to any other
region.
