...
When you create a custom agent ingress you'll get the option to choose between
...
traffic inspection, since the proxy won't automatically trust the ngrok-issued
certificate.

### Let's Encrypt Cert Issuer

The Let's Encrypt cert issuer is not automatically trusted by the agent and
...
public CAs). This also applies to any ngrok agent SDK or SSH Reverse Tunnel agent.

It is not immediately provisioned - blocked on DNS delegation and the propagation
thereof. The provisioning process could take up to 48 hours if propagation is
