...
The Connect URL is the network address that the ngrok agent and agent
SDKs use to connect to the ngrok service. The default Connect URL is
...
You can customize the Connect URL by creating your own Connect URL. You
do this by delegating control of a DNS subdomain. ngrok will then populate the
A/AAAA records and issue the certificates for your new Connect URL.
...
Agents authenticating to your Connect URL must authenticate to your
...
### Create the Connect URL

When you create a new Connect URL, you specify the domain you wish to use.
You must own this domain yourself and have the ability to create `NS` records
on it via your DNS provider.

- **[Create a new Connect URL on the ngrok
  Dashboard](https://dashboard.ngrok.com/tunnels/ingress)**
- **[Create a new Connect URL via API](/api/resources/agent-ingresses/)**

:::note

When creating a new custom Connect URL, there is an option to configure the
...
After you create a new Connect URL, you will be prompted to create `NS`
records on the domain you chose.

### Update agent configuration

Finally, you must reconfigure your ngrok agent or agent SDKs to use your new
Connect URL by adding the [`connect_url`](/agent/config/v3/#connect_url)
...
Custom Connect URLs require that you choose a particular region that your
...
Endpoints created by agents using custom Connect URLs are still delivered with
...
When you create a custom Connect URL, the DNS, certificates and hostnames are
...
When you create a custom Connect URL, you have the option to choose between
...
If Let's Encrypt is selected, the Connect URL drawer will show the following
...
or APIs inside of your customers' networks, a custom Connect URL allows you
to brand ngrok's connectivity as your own.

### Policy Enforcement

If you are using ngrok for development and testing, you can use your own
Connect URL to ensure everyone at your org does not use their personal
...
Connect URLs from your machines and networks, but allowing traffic to
your custom Connect URL.

## Pricing

Creating your own Connect URL is not available on any self-serve plan.
[Contact us](mailto:support@ngrok.com?subject=Custom+Connect+URL) to get in touch about using custom Connect URLs.
