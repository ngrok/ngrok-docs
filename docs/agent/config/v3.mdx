...
| [`agent_tls_termination.mutual_tls_certificate_authorities`](#mutual_tls_certificate_authorities) |          | Specifies one or more Root CA certificates (as paths or raw PEM data) to validate client certificates for mutual TLS authentication. Applicable only for `tls://` URLs.            |
| [`agent_tls_termination.server_certificate`](#server_certificate)                                 |          | The server certificate (provided as a file path or raw PEM data) used by the ngrok agent for TLS termination. Applicable only for `tls://` URLs.                                   |
| [`agent_tls_termination.server_key`](#server_key)                                                 |          | The private key (provided as a file path or raw PEM data) corresponding to the server certificate, used for TLS termination by the ngrok agent. Applicable only for `tls://` URLs. |
...
String[]. Specifies one or more Root CA certificates (as paths or raw PEM data) to validate client certificates for mutual TLS authentication. Applicable only for `tls://` URLs.

##### `agent_tls_termination.server_certificate`

String. The server certificate (provided as a file path or raw PEM data) used by the ngrok agent for TLS termination. Applicable only for `tls://` URLs.

##### `agent_tls_termination.server_key`

String. The private key (provided as a file path or raw PEM data) corresponding to the server certificate, used for TLS termination by the ngrok agent. Applicable only for `tls://` URLs.

#### Example: Agent TLS Termination
...
openssl req -new -key server.key -out server.csr -subj "/CN=your-domain.ngrok.io"
  ...
  - name: foo
    ...
    agent_tls_termination:
      ...
      mutual_tls_certificate_authorities:
        - /home/<your-user>/.config/ngrok/ca.crt
EOF
...
curl -v https://your-domain.ngrok.io --cacert ca.crt
# Expected Output:
# * CONNECTED(00000003)
# * depth=1 CN = ExampleCA
# * verify return:1
# * depth=0 CN = your-domain.ngrok.io
# * verify return:1
# * SSL handshake successful
# * Verify return code: 0 (ok)

# 9. Verify mutual TLS connection with client certificate
curl -v https://your-domain.ngrok.io \
  --cacert ca.crt \
  --cert client.crt \
  --key client.key
# Expected Output:
...
##### Example: Agent MTLS Termination
...
# 8. Verify connection to the endpoint with Root CA only
curl -vk https://your-domain.ngrok.io --cacert ca.crt
