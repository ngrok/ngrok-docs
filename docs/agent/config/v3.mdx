...
options under the `agent` field and change the `version` field to `3`.
...
| [`upstream.protocol`](#upstreamprotocol)             |          | HTTP, HTTPS |                                                                                                                                                                        |
| [`upstream.proxy_protocol`](#upstreamproxy_protocol) |          | HTTP, HTTPS | The version of [PROXY protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) to use with this endpoint, empty if not using. Example values are 1 or 2. |
...
String. The upstream application layer protocol to be used for HTTP endpoints.
...
You can find the configuration options for `tunnels` under the [Agent Configuration v2 docs](https://ngrok.com/docs/agent/config/v2/#tunnel-configurations).
