...
We proactively monitor and ban any accounts we identify that are involved with these attacks, and also work with 3rd parties that report malware and abuse via abuse@ngrok.com and our [abuse APIs](/api-reference/abusereports/create).
...
#### How does ngrok secure the distribution of our binaries? 

When ngrok releases new versions of the ngrok agent via equinox, we also sign those releases in multiple ways:

- We sign each MacOS and Windows release with code-signing certificates that the operating system verifies. You can verify that those certificates state that we are the publisher.
- The released executables are also signed with a private key that only our release pipeline has access to that are used for ngrok’s optional self-update mechanism. If you ask ngrok to update to a new version (via `ngrok update`) it cryptographically verifies the new version was signed with a trusted CA that is packaged into its executable.
...
Here is an example verification flow for macOS using our 3.8.0 binary:
