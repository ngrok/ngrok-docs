...
- Use either a [Domain](/gateway/domains/) or [TCP Address](/gateway/domains/tcp-addresses/)
- Use the `public` [binding](/gateway/endpoints/bindings/)
...
Follow the [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/) to set up an ngrok account and create your first public endpoint.
...
- [JWT Validation](/gateway/traffic-policy/actions/jwt-validation/)
- [OAuth](/gateway/traffic-policy/actions/oauth/)
- [OIDC](/gateway/traffic-policy/actions/oidc/)
- [IP restrictions](/gateway/traffic-policy/actions/restrict-ips/)
- [Mutual TLS](/gateway/traffic-policy/actions/terminate-tls/#example-traffic-policy-document)
...
- The hostname and port must match the address of a [TCP
  Address](/gateway/domains/tcp-addresses) in your account.
