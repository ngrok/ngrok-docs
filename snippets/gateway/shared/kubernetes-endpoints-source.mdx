...
operator](/gateway/k8s/). They enable you to connect to ngrok endpoints without making
them publicly addressable.

Kubernetes endpoints have a
[binding](/gateway/endpoints/bindings) of `kubernetes`.
...
<Steps>
  ...
  <Step title="Create a new namespace">
    ```bash
    kubectl create ns prod
    ```
  </Step>

  <Step title="Create an ngrok endpoint">
  ...
  <Step title="Connect to the endpoint">
...
- **Namespacing** - Kubernetes endpoints are namespaced on a per-account basis.
  ...
  [pool](/gateway/endpoints/endpoint-pooling).
...
- Kubernetes endpoints support [Endpoint
  Pooling](/gateway/endpoints/endpoint-pooling).
...
See [the docs on enabling bindings](/gateway/k8s/guides/bindings/#enable-bindings-for-the-operator) to learn more.
