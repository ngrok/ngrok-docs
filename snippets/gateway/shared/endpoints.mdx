import TypesComparison from "/snippets/shared/types-comparison.mdx";
...
- **Go live instantly**: Enable access to local prototypes, internal tools or [Kubernetes](/gateway/endpoints/kubernetes-endpoints/) services in seconds.
...
- **[Traffic Policy](/gateway/traffic-policy/):** Configuration to manage, manipulate and secure traffic.

### Control access with bindings

An endpoint's [binding](/gateway/endpoints/bindings) defines _where traffic originates_:

- **Public:** Accepts traffic from the open internet (for example, `https://your-app.ngrok.io`).
- **Internal:** Restricted to accept traffic only through the [`forward-internal` Traffic Policy action](/gateway/traffic-policy/actions/forward-internal/) (for example, `http://your-api.internal`).
- **[Kubernetes](/gateway/k8s/):** Restricted to accept traffic only from Kubernetes clusters using the ngrok Kubernetes Operator.
...
Add business logic and route traffic during the request lifecycle with [Traffic Policy](/gateway/traffic-policy/):

- **Route traffic by anything:** Paths, headers or any other available variable.
- **Manipulate traffic through [actions](/gateway/traffic-policy/actions/):** Redirect urls, inject headers or send custom responses.
- **Secure:** Add [OAuth](/gateway/traffic-policy/actions/oauth/), [IP restrictions](/gateway/traffic-policy/actions/restrict-ips) or [mutual TLS](/gateway/traffic-policy/actions/terminate-tls/#enabling-mutual-tls).

### Deliver traffic anywhere

You can host endpoints on your local machine with the [ngrok Agent](/gateway/agent/) or in the cloud with [ngrok Cloud Endpoints](/gateway/endpoints/cloud-endpoints/).
...
Enable access to a local web app on port `8080` with the [ngrok CLI](/gateway/agent/cli):
...
To learn more, follow ngrok's [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/).

## Load balancing with Endpoint Pooling

[Endpoint Pooling](/gateway/endpoints/endpoint-pooling) allows you to create multiple endpoints with the same URL. Traffic sent to a URL with an Endpoint Pool is [load-balanced](/gateway/domains/global-load-balancer/) among the Endpoints in the pool.

See the [Endpoint Pooling quickstart](/gateway/endpoints/endpoint-pooling/#quickstart) to learn more.

## Managing endpoints with ngrok's APIs

[Cloud Endpoints](/gateway/endpoints/cloud-endpoints/) are
programmatically managed via:

- [`/endpoints` API Resource](/api-reference/endpoints/list of the [API](/api/)
- [`CloudEndpoint` CRD](/gateway/k8s/crds/) of the [Kubernetes Operator](/gateway/k8s/)

[Agent Endpoints](/gateway/endpoints/agent-endpoints), by contrast,
are programtically managed via:

- [Agent SDKs](/agent-sdks/)
- [Agent API](/gateway/agent/api/)
- [Kubernetes Operator](/gateway/k8s/installation/helm)
...
- **[Cloud Endpoints](/gateway/endpoints/cloud-endpoints):** Create always-on endpoints to deliver and manage traffic to custom responses, internal endpoints or third-party services.
...
- **[Agent Endpoints](/gateway/endpoints/agent-endpoints):** Create an endpoint that only exists as long the agent is running to deliver and manage traffic to your local applications, files, services or third-party services.
