...
- **Go live instantly**: Enable access to local prototypes, internal tools or [Kubernetes](/endpoints/kubernetes-endpoints/) services in seconds.
...
An endpoint's [binding](/endpoints/bindings) defines _where traffic originates_:
...
You can host endpoints on your local machine with the [ngrok Agent](/agent/) or in the cloud with [ngrok Cloud Endpoints](/endpoints/cloud-endpoints/).
...
To learn more, follow ngrok's [Agent CLI Quickstart](/endpoints/agent-cli-quickstart/).

## Load balancing with Endpoint Pooling

[Endpoint Pooling](/endpoints/endpoint-pooling) allows you to create multiple endpoints with the same URL. Traffic sent to a URL with an Endpoint Pool is [load-balanced](/domains/global-load-balancer/) among the Endpoints in the pool.

See the [Endpoint Pooling quickstart](/endpoints/endpoint-pooling/#quickstart) to learn more.

## Managing endpoints with ngrok's APIs

[Cloud Endpoints](/endpoints/cloud-endpoints/) are
...
[Agent Endpoints](/endpoints/agent-endpoints), by contrast,
...
- **[Cloud Endpoints](/endpoints/cloud-endpoints):** Create always-on endpoints to deliver and manage traffic to custom responses, internal endpoints or third-party services.
...
- **[Agent Endpoints](/endpoints/agent-endpoints):** Create an endpoint that only exists as long the agent is running to deliver and manage traffic to your local applications, files, services or third-party services.
