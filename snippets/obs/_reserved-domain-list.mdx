...
| reserved_domains.acme_challenge_cname_target | string | DNS CNAME target for the host _acme-challenge.example.com, where example.com is your reserved domain name. This is required to issue certificates for non-ngrok reserved domains with wildcard patterns. Must be null for domains without wildcard patterns and ngrok subdomains. |
