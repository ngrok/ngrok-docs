...
| Name | Type | Description | Filter || backend.connection_reused | bool | True if ngrok reused a TCP connection to transmit the HTTP request to the upstream service. |
...
| Name | Type | Description | Filter || conn.bytes_in | int64 | The number of bytes arriving at an endpoint from the frontend |
