...
3. Unlike a traditional reverse proxy, ngrok forwards traffic to your upstream
   services over secure connections established by the [ngrok agent](/gateway/agent/),
   [Agent SDKs](/agent-sdks) or [Kubernetes Controller](/gateway/k8s/). So which PoPs do
...
the execution of [Modules](/gateway/traffic-policy/actions/) that you configure on your
...
When you use [endpoint pooling](/gateway/endpoints/endpoint-pooling), ngrok will automatically distribute traffic to the closest upstream if you run geographically distributed copies of your upstream application or service.
...
agent](/gateway/agent/changelog#ngrok-agent-300---2022-03-28), the agent uses GSLB to
