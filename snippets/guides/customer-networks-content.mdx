...
- An ngrok agent configured on a server within customer 1's network. See the [Agent CLI Quickstart](/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
...
Create a **Cloud Endpoint** with a [**Kubernetes binding](/endpoints/bindings/).** This will create a privately addressable way to access the remote services. These endpoints will only be accessible within Kubernetes clusters where you install the ngrok Operator.
