...
<Tip>
For detailed instructions on using ngrok with Kubernetes, check out the [k8s quickstart](/gateway/k8s/).
</Tip>
...
For MacOS and Windows, you must use the special URL `host.docker.internal` as described in the [Docker networking documentation](https://docs.docker.com/desktop/features/networking/networking-how-tos/).
...
The ngrok docker image wraps the ngrok agent executable. Read the documentation for the [ngrok agent CLI docs](/gateway/agent/cli/) for all commands.
...
[Traffic Policy](/gateway/traffic-policy/) is a configuration language that offers you the flexibility to filter, match, manage, and orchestrate traffic to your endpoints.
