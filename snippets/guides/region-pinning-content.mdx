...
Configure your [domain](/domains) to resolve to the IP addresses of one (or several) of ngrok's globally distributed physical points of presence (PoPs). 
...
<Info>
The same configuration will soon support [dedicated IP addresses](/domains/dedicated-ips/) [preview feature], in addition to Points of Presence.
</Info>

## How it works

1. Choose which ngrok points of presence your domain can use. 
2. All requests to [endpoints](/endpoints) on that domain will *only* be routed through your chosen PoPs. 

<img 
  src="/img/docs/region-pinning-how-it-works-diagram.png" 
  alt="Flowchart showing users in US, EU, and Asia all routing through a domain to the nearest allowed ngrok Point of Presence from a pinned set of regions"
  width={1371}
  height={1901}
...
You can specify a list of ngrok physical PoPs (like "Frankfurt" or "California"), [dedicated IPs](/domains/dedicated-ips/), or both.
...
You can "un-pin" a domain. Removing Regional Resolution settings restores the domain to its *default* IP resolution behavior: [global load balancing](https://ngrok.com/docs/domains/global-load-balancer) across *all* ngrok points of presence.
...
When a domain uses dedicated IPs, you can create matching [endpoints](/endpoints/) on any inbound port.
...
See the [Dedicated IPs](/domains/dedicated-ips/) documentation for configuration examples, use cases, and pricing:
...
- Dedicated IPs: yes, dedicated IPs are available for any [ngrok-supported protocol](/endpoints/protocols), including HTTP/S, TCP, and TLS.
...
| | Europe _[coming soon]_ | `european-union` |
| | United States _[coming soon]_ | `united-states` |
