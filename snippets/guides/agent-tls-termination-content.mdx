...
<Tip>
If your service doesn't support TLS termination, you can still use Agent TLS termination with [Zero-Knowledge TLS](/gateway/domains/tls-termination/#end-to-end-encryption).
</Tip>
...
<Steps>
  <Step title="Generate a cert and key pair">
...
=======
You can configure your endpoint with an [agent configuration file](/gateway/agent/config/). To create a new configuration file with your generated cert and key pair, run the following command:
>>>>>>> origin/main
...
<<<<<<< HEAD
    To make the agent authenticate clients as part of the TLS handshake, add one or more Certificate Authorities to the `agent_tls_termination` block with `mutual_tls_certificate_authorities`.
    The agent will reject any client whose certificate isn't signed by one of the listed CAs.
=======
<Tip>
You can run `ngrok config edit` to open the configuration file in your default text editor. [Learn more about the `ngrok config` command](/gateway/agent/cli/#ngrok-config).
</Tip>
>>>>>>> origin/main
    ...
    See [`agent_tls_termination.mutual_tls_certificate_authorities`](/agent/config/v3#agent_tls_termination-mutual_tls_certificate_authorities) for the full reference.

    <Note>
    This is different from mTLS terminated at the ngrok edge via [the `terminate-tls` Traffic Policy action](/gateway/mtls-termination/).
    With agent-side mTLS, the client certificate is validated by your agent; traffic between the ngrok edge and your agent is still carried over the agent's secure TLS connection to ngrok, but client-certificate authentication is not performed on that hop.
    </Note>
  </Step>

<<<<<<< HEAD
  <Step title="Start your endpoint">
...
=======
See [`agent_tls_termination.mutual_tls_certificate_authorities`](/gateway/agent/config/v3#agent_tls_termination-mutual_tls_certificate_authorities) for the full reference.

<Note>
This is different from mTLS terminated at the ngrok edge via [the `terminate-tls` Traffic Policy action](/gateway/domains/mtls-termination/).
With agent-side mTLS, the client certificate is validated by your agent; traffic between the ngrok edge and your agent is still carried over the agent's secure TLS connection to ngrok, but client-certificate authentication is not performed on that hop.
</Note>
>>>>>>> origin/main

  <Step title="Start your upstream server">
  ...
  <Step title="Try connecting to your endpoint">
