...
The ngrok agent uses a simple [YAML configuration file](/gateway/agent/config),
...
<<<<<<< HEAD
Follow the [Agent CLI Quickstart](/gateway/agent-cli-quickstart/) or visit the [download page](https://download.ngrok.com) to install and set up the ngrok agent.
=======
...
The ngrok agent uses a [YAML configuration file](/gateway/agent/config) to customize
...
Consult the [ngrok agent configuration file reference](/gateway/agent/config/)
...
| ----------------- | ------------------------------------------ |
| `NGROK_AUTHTOKEN` | [`authtoken`](/gateway/agent/config/v3/#authtoken) |
| `NGROK_API_KEY`   | [`api_key`](/gateway/agent/config/v3/#api-key)     |
...
Consult the [ngrok agent API reference documentation](/gateway/agent/api/) for more details.
...
- [Kubernetes Operator](/gateway/k8s/)
- [SSH Reverse Tunnel](/gateway/agent/ssh-reverse-tunnel-agent/)
...
The [ngrok agent changelog](/gateway/agent/changelog) is published with details on
