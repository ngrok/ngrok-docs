...
A map of pod annotations prefixed with `ngrok-`. Only annotations with the `ngrok-` prefix are included. The combined size of all included annotations must not exceed 1024 bytes. If the limit is exceeded, `conn.k8s.pod.metadata.error_code` will be set to `ERR_NGROK_28000` instead.
