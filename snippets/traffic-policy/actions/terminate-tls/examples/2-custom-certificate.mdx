...
These commands will result in five new files: `ca.key`, `ca.crt`, `server.key`, `server.csr`, and `server.key`. You will use the contents of `server.key` and `server.crt` in your traffic policy to enable your custom certificate on your endpoint and `ca.crt` to trust the certificate when using `curl`.
