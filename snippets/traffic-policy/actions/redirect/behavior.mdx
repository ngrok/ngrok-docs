...
If the specified `from` regular expression matches the request URL, the URL will be modified according to the `to` field.
If no `from` field is specified, the entire URL will be treated as a match.
