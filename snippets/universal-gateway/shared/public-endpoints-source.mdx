Public endpoints enable your services to receive geo-balanced internet traffic using [ngrok's global points of presence](/universal-gateway/edges/).

Public endpoints enable you to receive traffic from the internet via ngrok’s [global points of presence](/universal-gateway/edges/).
This is the most common type of endpoint and the easiest way to start.

Public endpoint URLs must:

- Use either a [Domain](/universal-gateway/domains/) or [TCP Address](/universal-gateway/tcp-addresses/)
- Use the `public` [binding](/universal-gateway/bindings/)
...
To keep such services both public and secure, you can add a Traffic Policy to your endpoint for authentication and authorization.
Traffic Policy actions enable you to do the following with your endpoints:
...
Public endpoints can be created programmatically.
Consult the documentation on [Endpoint APIs](/api-reference/endpoints/list.

## Pricing

Public endpoints are available on all plans.
