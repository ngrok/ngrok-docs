...
You can think of endpoints as a gateway to anything you want to enable access for—whether that's a local development server on your laptop, a production Kubernetes cluster, a database behind a corporate firewall, or a cloud API.
Endpoints bridge the gap between your resources and the traffic that needs to reach them.
...
You can specify your endpoint's URL protocol to indicate the type of traffic it should handle.
This allows you to configure your endpoint to match your services and application type:
...
[Endpoint Pooling](/universal-gateway/endpoint-pooling) allows you to create multiple endpoints with the same URL. Traffic sent to a URL with an Endpoint Pool is [load-balanced](/universal-gateway/global-load-balancer/) among the Endpoints in the pool.
...
[Cloud Endpoints](/universal-gateway/cloud-endpoints/) are programmatically managed via:
...
[Agent Endpoints](/universal-gateway/agent-endpoints), by contrast, are programtically managed via:
...
On the Pay-as-you-go plan, there is no limit on the number of endpoints you can create.
An endpoint that transmits data in a clock hour is counted as an active endpoint for that hour and charged an endpoint hour.
