...
Before upgrading, it's beneficial to have the latest version of the ngrok agent installed. If you don't, please [update to the latest version](/gateway/agent/cli#ngrok-update) before proceeding.
...
- Installing the Authtoken is now done with the command `ngrok config add-authtoken TOKEN`. Check out the [ngrok config reference documentation](/gateway/agent/cli#ngrok-config) for more information.
...
Now that you've upgraded to the latest configuration file, you're ready to start the latest ngrok agent. There's a lot of new stuff in there, but your old commands should work as well (with the changes from above). Run `ngrok -h` for help or check out the [ngrok agent reference documentation](/gateway/agent/cli/).
