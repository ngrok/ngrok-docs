---
title: ngrok Agent CLI Configuration File
sidebarTitle: Overview
description: Learn about the ngrok Agent CLI configuration file for running multiple tunnels simultaneously and configuring advanced settings.
---
...
ngrok configuration files to merge when starting the ngrok agent.
The rules for
merging are as follows:

- Values from later configs override those from earlier configs.
  For example, if config1.yml and config2.yml both set `api_key`, then calling `ngrok http --config config1.yml,config2.yml 80` will use the `api_key` from config2.yml.
