...
However, if you want more control and security across many devices, you can generate a unique authtoken for each device in the [ngrok Dashboard](https://dashboard.ngrok.com/authtokens) or via the `ngrok api credentials` command.
...
See [the Connect URL docs](/gateway/agent/connect-url/) for more information.
...
The allowed values are `internal`, `public`, and `kubernetes` which allow you to create [public](/gateway/endpoints/public-endpoints/), [internal](/gateway/endpoints/internal-endpoints/), and [kubernetes endpoints](/gateway/endpoints/kubernetes-endpoints/).
...
- [HTTP Traffic Policy Docs](/gateway/traffic-policy/)
...
You can find the configuration options for `tunnels` under the [Agent Configuration v2 docs](/gateway/agent/config/v2/#tunnel-configurations).
