...
- **Something that stays running.** [Install it as a native OS service](/gateway/agent/service#running-ngrok-in-the-background) for automatic startup and crash recovery, then [stop, restart, or upgrade it remotely](/gateway/agent/service#remote-management) from the API or dashboard.
