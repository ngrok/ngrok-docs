...
the [ngrok agent CLI API page](/gateway/agent/cli-api) for convenience. For more information about the ngrok API and interfacing with it directly, see [the ngrok api page](/gateway/agent/api).
...
Open https://dashboard.ngrok.com/traffic-inspector to inspect traffic through your endpoints.
...
*See Also*:
[`server_addr`](/gateway/agent/config/#server-addr)
[Custom Agent Ingress](/gateway/agent/connect-url/)
...
For more information about running ngrok as a service, check out the [ngrok service section in the secure tunnels documentation](/gateway/agent/service#running-ngrok-in-the-background)
