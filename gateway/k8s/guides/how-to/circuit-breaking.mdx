...
Check out the [circuit breaking Traffic Policy action](/gateway/traffic-policy/actions/circuit-breaker/) page for more details about how it functions and the parameters it accepts.
...
<<<<<<< HEAD:k8s/guides/how-to/circuit-breaking.mdx
=======
The following example showcases supplying the `NgrokTrafficPolicy` on a `Gateway` resource. All requests to the `Gateway` will run the Traffic Policy.
...
spec:
  ...
  listeners:
    - name: example-hostname
      ...
      protocol: HTTPS
```
>>>>>>> origin/main:gateway/k8s/guides/how-to/circuit-breaking.mdx
</Tab>
</Tabs>
