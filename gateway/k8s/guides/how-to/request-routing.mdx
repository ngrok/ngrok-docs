...
description: Configure request routing in Kubernetes to direct incoming traffic to the appropriate backend services based on rules.
---
...
`CloudEndpoint` resources do not have configuration for an upstream, but you can use Traffic Policy configuration with the [forward-internal](/gateway/traffic-policy/actions/forward-internal/) action to forward to other endpoints.
...
Check out the [Traffic Policy expressions](/gateway/traffic-policy/how-it-works#expressions) page for more information about the expressions you can write.
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Prefix Path Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Tab title="CloudEndpoint with AgentEndpoint">
<Steps>
  <Step title="Create an AgentEndpoint for your upstream Service">
    ```yaml
    apiVersion: ngrok.k8s.ngrok.com/v1alpha1
    kind: AgentEndpoint
    metadata:
  ...
  <Step title="Create a CloudEndpoint that routes to the AgentEndpoint">
    ```yaml
    apiVersion: ngrok.k8s.ngrok.com/v1alpha1
    kind: CloudEndpoint
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Request Method Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Exact Header Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Regex Header Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Exact Query Parameter Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with Regex Query Parameter Matching">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
