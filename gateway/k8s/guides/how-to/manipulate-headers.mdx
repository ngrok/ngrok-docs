...
Check out the [add headers](/gateway/traffic-policy/actions/add-headers/) and [remove headers](/gateway/traffic-policy/actions/remove-headers/) Traffic Policy action pages for more details about how they function and the parameters they accept.
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with the RequestHeaderModifier filter">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with the ResponseHeaderModifier filter">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
