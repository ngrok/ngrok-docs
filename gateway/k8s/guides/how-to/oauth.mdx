...
sidebarTitle: OAuth
description: Configure OAuth authentication in Kubernetes to secure your endpoints by requiring valid OAuth tokens before forwarding requests.
---
...
Check out the [OAuth Traffic Policy action](/gateway/traffic-policy/actions/oauth/) page for more details about how it functions and the parameters it accepts.
Consult the list of [supported providers](/gateway/traffic-policy/actions/oauth/#supported-providers) for step-by-step integration guides for each one.
...
<<<<<<< HEAD:k8s/guides/how-to/oauth.mdx
=======
The following example showcases supplying the `NgrokTrafficPolicy` on a `Gateway` resource. All requests to the `Gateway` will run the Traffic Policy.
...
spec:
  ...
  listeners:
    - name: example-hostname
      ...
      protocol: HTTPS
```
>>>>>>> origin/main:gateway/k8s/guides/how-to/oauth.mdx
</Tab>
</Tabs>
