...
If you need more secure authentication methods for production traffic, consider the [OAuth](/gateway/k8s/guides/how-to/oauth), [OIDC](/gateway/k8s/guides/how-to/oidc), and [JWT Validation](/gateway/k8s/guides/how-to/jwt-validation) guides.
...
Check out the [basic auth Traffic Policy action](/gateway/traffic-policy/actions/basic-auth/) page for more details about how it functions and the parameters it accepts.
...
<Steps>
  <Step title="Create an NgrokTrafficPolicy">
    ```yaml
    apiVersion: ngrok.k8s.ngrok.com/v1alpha1
  ...
  <Step title="Use the NgrokTrafficPolicy on an Ingress">
    ```yaml
...
<<<<<<< HEAD:k8s/guides/how-to/basic-auth.mdx
    ```yaml
...
=======
  The following example showcases supplying the `NgrokTrafficPolicy` on a `Gateway` resource. All requests to the `Gateway` will run the Traffic Policy.
  If you prefer, `NgrokTrafficPolicy` can also be used on the route level by using an `externalRef` filter on an `HTTPRoute`. See the [using Gateway API guide](/gateway/k8s/guides/using-gwapi) for examples.
>>>>>>> origin/main:gateway/k8s/guides/how-to/basic-auth.mdx
