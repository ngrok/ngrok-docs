...
Check out the [URL rewrite Traffic Policy action](/gateway/traffic-policy/actions/url-rewrite/) page for more details about how it functions and the parameters it accepts.
...
<Steps>
  ...
  <Step title="Create an HTTPRoute with a URLRewrite Filter">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
