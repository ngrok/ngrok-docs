---
title: Securing Your Endpoints with JWT Validation
sidebarTitle: JWT Validation
description: Configure JWT validation in Kubernetes to secure your endpoints by verifying authentication tokens at the network edge.
---
...
Check out the [JWT validation Traffic Policy action](/gateway/traffic-policy/actions/jwt-validation/) page for more details about how it functions and the parameters it accepts.
...
<<<<<<< HEAD:k8s/guides/how-to/jwt-validation.mdx
=======
The following example showcases supplying the `NgrokTrafficPolicy` on a `Gateway` resource. All requests to the `Gateway` will run the Traffic Policy.
...
spec:
  ...
  listeners:
    - name: example-hostname
      ...
      protocol: HTTPS
```
>>>>>>> origin/main:gateway/k8s/guides/how-to/jwt-validation.mdx
</Tab>
</Tabs>
