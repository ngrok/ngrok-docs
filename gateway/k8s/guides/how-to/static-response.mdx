...
description: Configure static responses in Kubernetes to return predefined responses directly from the gateway without backend calls.
---
...
Check out the [custom response Traffic Policy action](/gateway/traffic-policy/actions/custom-response/) page for more details about how it functions and the parameters it accepts.
...
<<<<<<< HEAD:k8s/guides/how-to/static-response.mdx
=======
The following example showcases supplying the `NgrokTrafficPolicy` on a `Gateway` resource. All requests to the `Gateway` will run the Traffic Policy.
...
spec:
  ...
  listeners:
    - name: example-hostname
      ...
      protocol: HTTPS
```
>>>>>>> origin/main:gateway/k8s/guides/how-to/static-response.mdx
</Tab>
</Tabs>
