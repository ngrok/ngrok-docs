...
<Steps titleSize="h3">
  ...
  <Step title="Configure your upstream">
  ...
  <Step title="Create a Kubernetes Secret for the client certificate">
    ```bash
...
<Steps titleSize="h3">
  <Step titleSize="h3" title="Create an AgentEndpoint that uses the client certificate">
    ```yaml
...
<Steps titleSize="h3">
  <Step title="Create a Gateway that uses the client certificate Secret">
    ```yaml
  ...
  <Step title="Create an HTTPRoute that routes to the upstream">
    ```yaml
    apiVersion: gateway.networking.k8s.io/v1
    kind: HTTPRoute
    metadata:
  ...
  <Step title="Annotate the service">
