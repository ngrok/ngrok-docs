...
marked for deletion. When uninstalling the Operator, the [pre-delete cleanup hook](/gateway/k8s/installation/uninstall#how-the-uninstall-process-works) handles this automatically, but if it was disabled or failed, you may need to intervene manually.
...
- To learn more about how the automated cleanup process handles finalizers during uninstall, see [Uninstalling the Operator](/gateway/k8s/installation/uninstall).
- See [Troubleshooting](/gateway/k8s/guides/troubleshooting#resources-not-getting-deleted) for help with resources that won't delete.
