...
- The [ngrok Kubernetes Operator installed](/gateway/k8s/installation/helm) in your cluster
...
**HTTPS endpoints** terminate TLS on ngrok's network. The `Authorization: Bearer <token>` header passes through transparently to the Kubernetes API server, so any token-based strategy works.
...
See the [restrict IPs guide](/gateway/k8s/guides/how-to/restrict-ips) for more details.
