...
See [Restrict Kubernetes bound endpoints](/gateway/k8s/guides/bindings/#restrict-kubernetes-bound-endpoints) for more details.

## 2. Project your local service

With your local service running (for example, on port `8080`), create a Kubernetes-bound endpoint.
The ngrok CLI works best if you're _not_ running the service in a [local K8s cluster](/gateway/k8s/guides/local-cluster), like [k3d](https://k3d.io/stable/) or [kind](https://kind.sigs.k8s.io/)—for that, use the ngrok Kubernetes Operator on the local cluster as well.

<Tabs>
  <Tab title="ngrok CLI">
>>>>>>> origin/main:gateway/k8s/guides/local-projection.mdx
    ```bash
    helm install ngrok-operator ngrok/ngrok-operator \
      --namespace ngrok-operator \
      --create-namespace \
  ...
  <Step title="Project your local service">
    With your local service running (for example, on port `8080`), create a Kubernetes-bound endpoint.
    The ngrok CLI works best if you're _not_ running the service in a [local K8s cluster](/k8s/guides/local-cluster), like [k3d](https://k3d.io/stable/) or [kind](https://kind.sigs.k8s.io/)—for that, use the ngrok Kubernetes Operator on the local cluster as well.
  ...
  <Step title="Redirect existing traffic to your local service">
...
- [Bindings](/gateway/k8s/guides/bindings/): Learn about all binding types and how they work with the Operator
- [Kubernetes endpoints](/gateway/endpoints/kubernetes-endpoints/): Deep dive into URL formats and service creation
- [CloudEndpoint CRD](/gateway/k8s/crds/cloudendpoint/): Create persistent Kubernetes-bound endpoints declaratively
