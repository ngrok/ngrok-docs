...
You can also head to the [Custom Resource Definition docs](/gateway/k8s/crds) for more detailed information about the structure of each resource and all of their fields.

## Using `AgentEndpoint`

The `AgentEndpoint` resource is used to create [Agent Endpoints](/gateway/endpoints/agent-endpoints/) using the Operator.

At a high level, Agent Endpoints are endpoints that accept traffic and forward it to an upstream of your choice.
They may optionally include [Traffic Policy](/gateway/traffic-policy/) configuration specified in-line or as a reference to an `NgrokTrafficPolicy` resource.
...
The [endpoint types page](/gateway/k8s/guides/endpoint-types) showcases using different protocols with `AgentEndpoint` and/or `CloudEndpoint` resources.
See the [AgentEndpoint Reference](/gateway/k8s/crds/agentendpoint) for more detailed information about `AgentEndpoint` structure and fields.
...
The `CloudEndpoint` resource is used to create [Cloud Endpoints](/gateway/endpoints/cloud-endpoints/) using the Operator.
...
The [endpoint types page](/gateway/k8s/guides/endpoint-types) showcases using different protocols with `AgentEndpoint` and/or `CloudEndpoint` resources.
See the [CloudEndpoint Reference](/gateway/k8s/crds/cloudendpoint) for more detailed information about `CloudEndpoint` structure and fields.
...
For a more in-depth guide on using `CloudEndpoints` to forward requests to other endpoints using the `forward-internal` action, see the [request routing guide](/gateway/k8s/guides/how-to/request-routing)

## Using `NgrokTrafficPolicy`

`NgrokTrafficPolicy` allows you to specify a [Traffic Policy](/gateway/traffic-policy/) configuration to be used on one or more `Ingress`, `Gateway`, `CloudEndpoint`, or `AgentEndpoint` resources.
...
`Ingress` and Gateway API resources have several methods of accepting `NgrokTrafficPolicy` resources outlined in the [using ingresses](/gateway/k8s/guides/using-ingresses) and [using Gateway API](/gateway/k8s/guides/using-gwapi) pages.
...
See the [NgrokTrafficPolicy Reference](/gateway/k8s/crds/ngroktrafficpolicy) for more detailed information about `NgrokTrafficPolicy` structure and fields.
...
- Domains reserve domains in your ngrok that are used in hostnames across `Ingress`, etc. resources.
  - [Domain Reference](/gateway/k8s/crds/domain)
- **AgentEndpoints**:
  - AgentEndpoints establish connections to your upstream services to handle forwarding traffic.
  - [AgentEndpoint Reference](/gateway/k8s/crds/agentendpoint)
- **BoundEndpoints**:
  ...
  - For more information on endpoint bindings, refer to [the bindings page](/gateway/k8s/guides/bindings).
  - [BoundEndpoint Reference](/gateway/k8s/crds/boundendpoint)
- **KubernetesOperators**:
  ...
  - [KubernetesOperator Reference](/gateway/k8s/crds/kubernetesoperator)
