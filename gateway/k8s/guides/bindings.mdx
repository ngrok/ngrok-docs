...
For a full reference of the available variables, error handling, and usage examples, see [Restricting Access by Kubernetes Pod Identity](/gateway/k8s/guides/how-to/pod-identity).
...
See the [Operator Helm page](/gateway/k8s/installation/helm) for more information about the `bindings` value and all the other configuration options such as configuring custom annotations/labels on the projected services created by Kubernetes bound endpoints.
