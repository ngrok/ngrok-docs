...
Pooling](/gateway/endpoints/endpoint-pooling/) lets you share traffic between
replicas of your services on any number of clusters by deploying them on the same
[internal](/gateway/endpoints/internal-endpoints/) URL. With the addition
of a [Cloud Endpoint](/gateway/endpoints/cloud-endpoints) and [Traffic
...
[individual K8s
services](/gateway/k8s/load-balancing/load-balancing-kubernetes) with
ngrok custom resources, Ingress objects, or Gateway API resources. This guide uses [custom
resources](/gateway/k8s/guides/using-crds) to create endpoints for your services.
</Tip>
...
<<<<<<< HEAD:k8s/load-balancing/load-balancing-kubernetes-clusters.mdx
  <Step title="Create internal Agent Endpoints on your first cluster">
...
=======
Check out the [installation instructions](/gateway/k8s/installation/helm/) for
details on how to use Helm to deploy the open-source Operator to each cluster
you'd like to load-balance between.
>>>>>>> origin/main:gateway/k8s/load-balancing/load-balancing-kubernetes-clusters.mdx
...
will proxy traffic to your `foo`, `bar`, and `baz` pods, respectively. When you
use custom resources to create Agent Endpoints, pooling is [enabled
automatically](/gateway/k8s/guides/pooling/#endpoint-pooling-with-agentendpoints).
>>>>>>> origin/main:gateway/k8s/load-balancing/load-balancing-kubernetes-clusters.mdx
  ...
  <Step title="Create a Cloud Endpoint and add routing with Traffic Policy">
...
=======
1. Uses the [`forward-internal`
   action](/gateway/traffic-policy/actions/forward-internal/) to route all requests to
   the `/foo` path to the pool you created at `https://foo.internal`. - The pooled endpoints then proxy requests proxies the request to the `foo`
   Service/Deployment in your cluster.
2. Does the same for the paths `/bar` and `/baz`.
3. Denies all traffic to paths that don't (yet) have a service.
...
- [Route requests by subdomain or
  header](/gateway/traffic-policy/examples/route-requests/) instead of paths.
- Add authentication with the
  [`basic-auth`](/gateway/traffic-policy/actions/basic-auth/),
  [`oauth`](/gateway/traffic-policy/actions/oauth/), or
  [`jwt-validation`](/gateway/traffic-policy/actions/jwt-validation/) Traffic
  Policy actions.
- [Rate limit requests](/gateway/traffic-policy/actions/rate-limit/) to prevent
...
- [Deploy a multicloud API
  gateway](/gateway/k8s/load-balancing/load-balancing-kubernetes-clusters/)
