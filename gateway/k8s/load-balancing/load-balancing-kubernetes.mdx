...
[Endpoint Pooling](/gateway/endpoints/endpoint-pooling/) makes load
...
[Ingress](/gateway/k8s/guides/using-ingresses) objects, or [Gateway
API](/gateway/k8s/guides/using-gwapi) resources.
...
<<<<<<< HEAD:k8s/load-balancing/load-balancing-kubernetes.mdx
  <Step title="Create your first Agent Endpoint">
...
details on how to use Helm to deploy the open-source Operator to each cluster
you'd like to load-balance between.
>>>>>>> origin/main:gateway/k8s/load-balancing/load-balancing-kubernetes.mdx
  ...
  <Step title="Create a second Agent Endpoint to enable pooling">
...
Endpoint](/gateway/endpoints/cloud-endpoints/) that serves as the "front
door" to all your Kubernetes services, then create a pool of [internal Agent
Endpoints](/gateway/endpoints/internal-endpoints/) on a pooled URL like
`https://example.internal`. Curious? See the [step-by-step
guide](/gateway/k8s/load-balancing/load-balancing-kubernetes-clusters) that
