...
To check connectivity from where your pods actually run, you can run [`ngrok diagnose`](/gateway/agent/connectivity#troubleshooting) as a `Job`, using the [pre-built agent image](https://hub.docker.com/r/ngrok/ngrok). This checks name resolution, TCP, and TLS to `connect.ngrok-agent.com`.
