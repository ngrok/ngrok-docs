...
description: ngrok improves your existing Wordpress installation by managing your TLS certificates, accelerating your blog on the global load balancer, and protecting it from DDoS attacks.
---
...
<Steps>
  ...
  <Step title="Start your Wordpress endpoint">
  ...
  <Step title="Try out your Wordpress endpoint">
    <TryOut />
  </Step>
</Steps>
...
You can use the [`restrict-ips` Traffic Policy action](/gateway/traffic-policy/actions/restrict-ips/) to allow only yours or other trusted IPs.
...
- Read more about [Traffic Policy](/traffic-policy) and its [core concepts](/gateway/traffic-policy/how-it-works).
- View your Wordpress traffic in [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector).
- Use the [`rewrite` action](/gateway/traffic-policy/actions/redirect) to create ["pretty" blog URLs](/gateway/traffic-policy/examples/url-rewrites/).
