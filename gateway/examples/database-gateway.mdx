...
<Steps titleSize="h2">
  <Step title="Create an endpoint for your database service">

    Start an internal [Agent Endpoint](/gateway/endpoints/agent-endpoints/) for your database proxy or HTTP database interface, replacing `$PORT` based on where it listens.
  ...
  <Step title="(Optional) Create a vault and secrets">

    For production environments, store your CA certificate securely using [Traffic Policy Secrets](/gateway/traffic-policy/secrets). This step is optional—you can also include the certificate directly in your policy.
...
```
  ...
  <Step title="Apply Traffic Policy to your Cloud Endpoint">
    ...
    **What's happening here?** This policy enforces mTLS certificate authentication where clients must present valid certificates signed by your trusted CA. Clients are rate limited based on their certificate identity, dangerous SQL operations are blocked, and authenticated requests are forwarded to your database service.
    ...
    If you don't have certificates, you can generate them using the steps in the [mTLS example documentation](/gateway/traffic-policy/actions/terminate-tls/#enabling-mutual-tls):
...
- Learn more about [mTLS certificate authentication](/gateway/traffic-policy/actions/terminate-tls) for high-security environments
- For token-based authentication, explore [JWT validation](/gateway/traffic-policy/actions/jwt-validation) as a scalable alternative to basic authentication
- Set up [comprehensive logging](/gateway/traffic-policy/actions/log) to send database access events to your SIEM or monitoring platform
- Use [URL rewriting](/gateway/traffic-policy/actions/url-rewrite) to transform database queries or add security constraints
