...
sidebarTitle: Custom Error Pages
...
<Steps>
  ...
  <Step title="Add routing to your service and error handling with Traffic Policy">
...
- See other examples of using the [`custom-response`](/gateway/traffic-policy/actions/custom-response/#examples) to create error pages or messages, such as a JSON-based response for an API service.
...
- Add the [`log` action](/gateway/traffic-policy/actions/log) before the `custom-response` to send error events over to your observability platform for enriched debugging.
