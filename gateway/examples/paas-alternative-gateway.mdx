...
<Steps>
  <Step title="Create endpoints for your application instances">
    Start multiple [Agent Endpoints](/gateway/agent-endpoints/) with the same URL to create an endpoint pool for automatic load balancing. Replace `$PORT` with your application ports.
...
=======
Start multiple [Agent Endpoints](/gateway/endpoints/agent-endpoints/) with the same URL to create an endpoint pool for automatic load balancing. Replace `$PORT` with your application ports.
You can also use one of the [SDKs](/agent-sdks) or the [Kubernetes Operator](/k8s).
>>>>>>> origin/main
    ...
    <Tip>
    By using the same URL with `--pooling-enabled`, ngrok automatically creates an [Endpoint Pool](/gateway/endpoint-pooling) that distributes traffic round-robin across all healthy instances with automatic failover.
    </Tip>
  </Step>
...
=======
<Tip>
By using the same URL with `--pooling-enabled`, ngrok automatically creates an [Endpoint Pool](/gateway/endpoints/endpoint-pooling) that distributes traffic round-robin across all healthy instances with automatic failover.
</Tip>
>>>>>>> origin/main

    <Tip>
    For a production PaaS alternative, consider using a [custom domain](/gateway/custom-domains/) you own instead of an ngrok subdomain.
    </Tip>
  </Step>
...
<<<<<<< HEAD
  <Step title="Create a vault and secrets">
    Store your authentication and API secrets securely using [Traffic Policy Secrets](/traffic-policy/secrets).
=======
<Tip>
For a production PaaS alternative, consider using a [custom domain](/gateway/domains/custom-domains/) you own instead of an ngrok subdomain.
</Tip>
>>>>>>> origin/main
...
<<<<<<< HEAD
    ...
    ngrok api secrets create \
      ...
      --vault-id "$VAULT_ID"
=======
Store your authentication and API secrets securely using [Traffic Policy Secrets](/gateway/traffic-policy/secrets).
>>>>>>> origin/main
  ...
  <Step title="Apply Traffic Policy to your Cloud Endpoint">
  ...
  <Step title="Try out your endpoint">
    <TryOut />
...
- Learn about [Endpoint Pooling](/gateway/endpoints/endpoint-pooling) for simpler automatic load balancing
- Explore [custom domains](/gateway/domains/custom-domains) to use your own domain instead of ngrok subdomains
- Use [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector) to monitor performance and debug issues
- Add [circuit breaker protection](/gateway/traffic-policy/actions/circuit-breaker) to handle upstream failures gracefully
