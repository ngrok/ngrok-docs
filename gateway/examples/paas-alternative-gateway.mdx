...
<Steps>
  <Step title="Create endpoints for your application instances">
    Start multiple [Agent Endpoints](/endpoints/agent-endpoints/) with the same URL to create an endpoint pool for automatic load balancing. Replace `$PORT` with your application ports.
    ...
    <Tip>
    By using the same URL with `--pooling-enabled`, ngrok automatically creates an [Endpoint Pool](/endpoints/endpoint-pooling) that distributes traffic round-robin across all healthy instances with automatic failover.
    </Tip>
  </Step>
  ...
  <Step title="Reserve a domain">
    ...
    <Tip>
    For a production PaaS alternative, consider using a [custom domain](/domains/custom-domains/) you own instead of an ngrok subdomain.
    </Tip>
  </Step>
...
- Learn about [Endpoint Pooling](/endpoints/endpoint-pooling) for simpler automatic load balancing
- Explore [custom domains](/domains/custom-domains) to use your own domain instead of ngrok subdomains
