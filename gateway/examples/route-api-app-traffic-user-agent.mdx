---
title: Route API and App Traffic by User Agent
sidebarTitle: Route by User Agent
...
<Steps>
  <Step title="Set up your JWT provider">
  ...
  <Step title="Start internal endpoints for your services">
    Start an internal [Agent Endpoint](/gateway/agent-endpoints/), replacing `$PORT` based on where your app service listens.
...
=======
Start an internal [Agent Endpoint](/gateway/endpoints/agent-endpoints/), replacing `$PORT` based on where your app service listens.
You can also use one of the [SDKs](/agent-sdks) or the [Kubernetes Operator](/k8s).
>>>>>>> origin/main
  ...
  <Step title="Add JWT validation, OAuth, and routing with Traffic Policy">
...
- Explore [other authentication methods](/gateway/traffic-policy/examples/add-authentication/) like Basic Auth, OIDC, or SAML.
- Learn about [more routing options](/gateway/traffic-policy/examples/route-requests/) beyond user agent, like routing by path, header, cookie, or geographic location.
- Explore all the configuration options in the [`jwt-validation`](/gateway/traffic-policy/actions/jwt-validation) and [`oauth`](/gateway/traffic-policy/actions/oauth) actions.
