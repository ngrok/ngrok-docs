...
To route traffic to specific regions, you can use ngrok's Cloud Endpoints, internal Agent Endpoints, and Traffic Policy engine to match incoming traffic based on the [client's IP geolocation data](/gateway/traffic-policy/variables/connection/#conngeocountry-code) using [ISO 3166 country codes](https://www.iso.org/iso-3166-country-codes.html), then forward requests to the appropriate regional endpoints.
...
This also implements the ['front door' pattern](/gateway/examples/front-door-pattern), which creates a loose coupling between the traffic management logic at your gateway and your services.

<Steps>
  <Step title="Start endpoints for your services">
    On a server designated for U.S. traffic, start an internal [Agent Endpoint](/gateway/agent-endpoints/), replacing `$PORT` based on where your service listens.
...
<<<<<<< HEAD
    ...
    ```
=======
On a server designated for U.S. traffic, start an internal [Agent Endpoint](/gateway/endpoints/agent-endpoints/), replacing `$PORT` based on where your service listens.
You can also use one of the [SDKs](/agent-sdks) or the [Kubernetes Operator](/k8s).
>>>>>>> origin/main
  ...
  <Step title="Add routing to your services with Traffic Policy">
...
- Explore [other routing methods](/gateway/traffic-policy/examples/route-requests/), like routing by path, header, cookie, or IP intelligence.
- [Add authentication](/gateway/traffic-policy/examples/add-authentication/) to your regional endpoints with OAuth, OIDC, or JWT validation.
- View your traffic in [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector).
- Check out [Endpoint Pools](/gateway/endpoints/endpoint-pooling/) for dead-simple load balancing.
