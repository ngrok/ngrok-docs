...
>>>>>>> origin/main
  ...
  <Step title="Restrict access to your admin path with Traffic Policy">
    ...
    on_http_request:
      - expressions:
          ...
          - "conn.client_ip.geo.location.country_code != 'US' || req.user_agent.is_bot || 'proxy.anonymous.tor' in conn.client_ip.categories || 'blocklist.greensnow_co' in conn.client_ip.categories"
        actions:
          - type: deny
      - expressions:
          - "req.url.path.includes('/admin')"
  ...
  <Step title="Try out your restricted admin tools">
    <TryOut />
  </Step>
</Steps>
...
- Explore other examples of using the [`forward-internal`](/gateway/traffic-policy/actions/forward-internal/#examples) and [`deny`](/gateway/traffic-policy/actions/deny/#examples-on_http_request).
