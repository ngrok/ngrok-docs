...
<Steps titleSize="h2">
  <Step title="Create endpoints for your services">
    ...
    Start internal [Agent Endpoints](/gateway/endpoints/agent-endpoints/) for the services that will handle webhooks from different providers.
  ...
  <Step title="(Optional) Create a vault and secrets">
    ...
    For production environments, store your webhook secrets securely using [Secrets for Traffic Policy](/gateway/traffic-policy/secrets). This step is optional—you can also use plaintext secrets directly in your policy.
...
- View the complete list of [70+ supported webhook providers](/gateway/traffic-policy/actions/verify-webhook/#supported-providers) including Twilio, Shopify, and DocuSign
- Add the [`log` action](/gateway/traffic-policy/actions/log) to send webhook events to your observability platform for monitoring and debugging
...
- Learn more about [webhook signature verification](/gateway/traffic-policy/actions/verify-webhook) and security best practices
