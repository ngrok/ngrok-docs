...
1. Put a URL online to accept internet traffic to a [Cloud Endpoint](/endpoints/cloud-endpoints/), which allows you to control how that traffic accesses the internal resources where your [internal Agent Endpoints](/endpoints/agent-endpoints/) are running. This is called the ["front door"](/gateway/examples/front-door-pattern) pattern.
...
<Steps>
  <Step title="Create endpoints for your services">
    Start internal [Agent Endpoints](/endpoints/agent-endpoints/), replacing `$PORT` based on where your service listens, for each of your services on the systems where they run.
  ...
  <Step title="Reserve a domain">
    Navigate to the [**Domains** section](https://dashboard.ngrok.com/domains) of the ngrok dashboard and click **New +** to reserve a [wildcard domain](/domains/custom-domains/) like `*.example.com`.
