...
<Steps titleSize="h2">
  <Step titleSize="h2" title="Try out your n8n endpoint">
...
This policy applies the [`restrict-ips`](/gateway/traffic-policy/actions/restrict-ips) Traffic Policy action and allows only
...
If you want to receive webhooks from external services, but also want to control precisely which services can send these requests and during what conditions, you can use the [`verify-webhooks` action](/gateway/traffic-policy/actions/verify-webhook).
...
This policy filters for _only_ traffic that contains the specific `/webhook/$WORKFLOW_UUID` path. It then verifies that the traffic both originates from GitHub (or another [supported provider](/gateway/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
</Info>
