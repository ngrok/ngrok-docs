...
<Steps>
  ...
  <Step title="Create AgentEndpoints for your microservices">
  ...
  <Step title="(Optional) Create secrets for authentication">
      ...
      - actions:
          - type: owasp-crs-request
            config:
              on_error: halt
=======
Store your JWT validation keys using [Traffic Policy Secrets](/gateway/traffic-policy/secrets).
>>>>>>> origin/main
  ...
  <Step title="Try out your microservices gateway">
    <TryOut />
...
- Explore [other authentication methods](/gateway/traffic-policy/examples/add-authentication/) like OAuth, OIDC, or Basic Auth.
- Learn about [more routing options](/gateway/traffic-policy/examples/route-requests/) like routing by header, cookie, or geographic location.
- Read about [other gateway shapes](https://ngrok.com/blog-post/api-gateway-shapes-patterns-2025) you can build with ngrok.
- Learn about [AgentEndpoint CRDs](/gateway/k8s/crds/agentendpoint) for more advanced Kubernetes configurations.
- Use [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector) to debug microservice request flows.
- Configure [endpoint pooling](/gateway/k8s/guides/pooling) for high availability across multiple service replicas.
