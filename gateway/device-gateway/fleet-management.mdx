...
- **Per-device auth tokens** scoped with [ACL bindings](/gateway/site-to-site-connectivity/authtoken-acls) so each token can only create endpoints on its assigned domain
- **Wildcard domains** for unique URLs per site (for example, `*.factory.example.com`)
...
Use the [ngrok API](/api/) or the [Agent API](/gateway/agent/api/) to manage tunnel lifecycle programmatically from your cloud.
...
- [Full walkthrough](/gateway/device-gateway/agent): see multi-tenancy and credential management in action
- [Security](/gateway/device-gateway/security): credential rotation, IP restrictions, and compliance
- [Python SDK guide](/gateway/device-gateway/sdk): manage device connectivity programmatically with the SDK
