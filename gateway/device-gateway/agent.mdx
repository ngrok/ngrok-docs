...
- An ngrok agent configured on your local machine. See the [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
...
An [internal endpoint](/gateway/endpoints/internal-endpoints/) enables a service inside the factory network to be reachable
...
A [Cloud Endpoint](/gateway/endpoints/cloud-endpoints/) is a permanent, externally accessible entry point into the factory
network that's also:
...
tab on the dashboard, and apply a [Traffic Policy](/gateway/traffic-policy/). Make sure you have a certificate on hand or generate them using the
instructions in the [terminate-tls](/gateway/traffic-policy/actions/terminate-tls/#generate-certificates) docs.
