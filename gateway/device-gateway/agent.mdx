...
- An ngrok agent configured on your local machine. See the [Agent CLI Quickstart](/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
...
An [internal endpoint](/endpoints/internal-endpoints/) enables a service inside the factory network to be reachable
...
A [Cloud Endpoint](/endpoints/cloud-endpoints/) is a permanent, externally accessible entry point into the factory
network that's also:
