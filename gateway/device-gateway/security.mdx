...
Each device or site gets its own authtoken scoped with [ACL bindings](/gateway/site-to-site-connectivity/authtoken-acls).
If one token is compromised, revoke it immediately without affecting the rest of your fleet:

- **Create** per-device or per-site [auth tokens](/gateway/agent/authtokens)
- **Scope** tokens to [limit which endpoints they can create](/gateway/agent/authtokens#authtoken-acls)
...
Limit access to your device endpoints to specific IP addresses or CIDR ranges [using Traffic Policy](/gateway/traffic-policy/actions/restrict-ips/). The following example demonstrates an [agent config](/gateway/agent/config/) which allows traffic only from a specific IP:
...
See the [mTLS Termination](/gateway/domains/mtls-termination) documentation for configuration details.
...
<Tip>
 This example will work out of the box, but it uses ngrok's managed OAuth application. This means you won't be able to customize the provider's authentication behavior. For full control, [use your own OAuth application](/gateway/traffic-policy/actions/oauth#using-a-custom-provider).
</Tip>
...
Additional security features include [RBAC](/iam/rbac), [SSO & SCIM](/iam/sso), [audit logs](/obs/events), [DDoS protection](/gateway/domains/ddos-protection), and [data residency](/gateway/domains/region-pinning/) options.
...
- **Data residency:** Control where traffic is processed and is available using [Region pinning](/gateway/domains/region-pinning/), available on [Pay-as-you-go](/pricing-limits/#pay-as-you-go-faqs) plans and above.
...
- [Traffic Policy](/gateway/device-gateway/traffic-policy): add authentication, rate limiting, and access control to your endpoints.
- [Fleet management](/gateway/device-gateway/fleet-management): manage multi-tenancy and credentials at scale
- [Full walkthrough](/gateway/device-gateway/agent): see security controls in action in a detailed example scenario
