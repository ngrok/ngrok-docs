...
Limit access to your device endpoints to specific IP addresses or CIDR ranges [using Traffic Policy](/gateway/traffic-policy/actions/restrict-ips/). The following example demonstrates an [agent config](/agent/config/) which allows traffic only from a specific IP:
