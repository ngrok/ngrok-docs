...
<Steps>
  ...
  <Step title="Enable SSH access">
    If you want to perform remote administration on your ARM64 device using a reserved TCP address and an encrypted tunnel, instead of relying on IP addresses, you can create a [TCP tunnel](/endpoints/tcp/) at port `22` on your ARM64 device.
  ...
  <Step title="Enable ingress to a service on your ARM64-based Linux device">
    Separately from SSH access, you can also use ngrok to create an [HTTP tunnel](/endpoints/http/) to route traffic to specific applications or services running on your ARM64 device's `localhost` network.
    ...
    1. Optionally, you can reserve a [static subdomain](/domains/) like so:
...
- Bring a [custom domain](/domains/custom-domains/) to ngrok to create static endpoints.
