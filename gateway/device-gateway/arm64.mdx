...
This guide is specific to ARM64 devices on Linux—if you're using a different device or CPU architecture, first check whether your platform meets the ngrok agent's [system and resource requirements](/gateway/agent/#system-requirements). Other guides are also available that may fit your use case more precisely:

- [Linux AMD64 devices](/gateway/device-gateway/linux/)
- [Raspberry Pi](/gateway/device-gateway/raspberry-pi/)
- [Raspberry Pi OS](/gateway/device-gateway/raspbian/)
...
- Any ARM64 device running Linux (see the [system requirements](/gateway/agent/#system-requirements) for all supported architectures)
...
<<<<<<< HEAD:guides/device-gateway/arm64.mdx
       ::::warning
       ...
       ::::
=======
If you want to perform remote administration on your ARM64 device using a reserved TCP address and an encrypted tunnel, instead of relying on IP addresses, you can create a [TCP tunnel](/gateway/endpoints/tcp/) at port `22` on your ARM64 device.
>>>>>>> origin/main:gateway/device-gateway/arm64.mdx
  ...
  <Step title="Enable ingress to a service on your ARM64-based Linux device">
...
=======
Separately from SSH access, you can also use ngrok to create an [HTTP tunnel](/gateway/endpoints/http/) to route traffic to specific applications or services running on your ARM64 device's `localhost` network.

<Note>
If you already established a TCP tunnel for SSH access, you'll either need to create a second tunnel for the HTTP tunnel, or use the [agent configuration file](/gateway/agent/config/v3/) to define multiple tunnels.
</Note>
>>>>>>> origin/main:gateway/device-gateway/arm64.mdx
    ...
    1. Optionally, you can reserve a [static subdomain](/gateway/domains/) like so:
  ...
  <Step title="Add a Traffic Policy to restrict IPs">
...
Now that you have SSH tunneling and service ingress handled via ngrok, you may also want to protect those services from unknown and untrusted users. You'll use the [Restrict IPs](/gateway/traffic-policy/actions/restrict-ips) Traffic Policy action.
