...
This guide is specific to ARM64 devices on Linux—if you're using a different device or CPU architecture, first check whether your platform meets the ngrok agent's [system and resource requirements](/gateway/agent/install#system-requirements). Other guides are also available that may fit your use case more precisely:
...
- Any ARM64 device running Linux (see the [system requirements](/gateway/agent/install#system-requirements) for all supported architectures)
...
- Install [ngrok as a service](/gateway/agent/service#running-ngrok-in-the-background) to start after your ARM64 device boots and automatically restart after crashes.
