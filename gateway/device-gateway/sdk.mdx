...
Reserve a [wildcard domain](/api-reference/reserveddomains/list), which will allow you to
create wildcard endpoints and receive traffic on any subdomain of your domain. For billing information, see [wildcard endpoints pricing](/pricing-limits/#wildcard-endpoints).
...
<img src="./img/aws-route53-cname-record-device-gateway.png" alt="AWS Route 53 Quick create record form showing a wildcard CNAME record for the device gateway subdomain pointing to the ngrok CNAME target" width="2198" height="1310" />
...
<img src="./img/dashboard-domain-dns-targets-2-targets.png" alt="ngrok dashboard Domains page showing a reserved domain's detail panel with the DNS Targets 2 targets link highlighted" width="3326" height="1918" />

5. Click **Check Status**, as denoted by the arrow in the screenshot below.

<img src="./img/dashboard-domain-dns-check-status.png" alt="ngrok dashboard DNS Targets panel showing CNAME and TLS record values with the Check Status button highlighted" width="3850" height="1918" />

6. You should see a successful response similar to the screenshot below.

<img src="./img/dashboard-domain-dns-validation-success.png" alt="ngrok dashboard DNS Targets panel showing successful CNAME and TLS record validation with green checkmarks" width="3326" height="1918" />
...
```json
{
	"id": "service_2fmwUXhnImKswMgZKCsx7cnbt2l",
	"uri": "https://api.ngrok.com/service_users/service_2fmwUXhnImKswMgZKCsx7cnbt2l",
...
<img src="./img/new-authtoken-bot-user-acl-rules.png" alt="ngrok dashboard New Authtoken dialog with a bot user as owner and a wildcard domain ACL binding rule" width="3326" height="1918" />
...
Update line `109` with your domain. Since you reserved a wildcard domain, you can create endpoints on any subdomain
of that domain without reserving them individually. For example, for the domain `*.sitea.configurable-domain.com`, you could create an endpoint at `api.sitea.configurable-domain.com`.
...
file, `test.json`, which contains the default [policies](/gateway/traffic-policy/) it applies to any new request that doesn't specify policy information. You could update this file to control policies applied to new tunnels rather than passing the policy information in each request.
...
any [policies](/gateway/traffic-policy/). However, this sample application applies the default policies defined in `test.json`
...
Run the following command, substituting the appropriate values for the variables to create a new tunnel, specifying [policies](/gateway/traffic-policy/) directly in the request.
