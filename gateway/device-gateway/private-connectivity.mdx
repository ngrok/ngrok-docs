...
- An ngrok agent installed in a remote IoT-based network. See the [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.

<Steps>
  <Step title="Install ngrokd in your local environment, start it, and set your API key">
  ...
  <Step title="Install the ngrok agent within your remote network and configure private Agent Endpoints in ngrok.yml">
  ...
  <Step title="Test end-to-end connectivity from your local environment to upstream devices">
    ...
    ```
<<<<<<< HEAD:guides/device-gateway/private-connectivity.mdx
  </Step>
</Steps>
=======
  </Tab>
...
First, install the agent in your remote network, either on a gateway or directly on a device.
Follow the [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/) for installation instructions.
...
ssh user@camera.example.com -p 22
```
>>>>>>> origin/main:gateway/device-gateway/private-connectivity.mdx
