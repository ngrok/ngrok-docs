...
<Note>
Running the ngrok Kubernetes Operator instead of the agent? Its egress requirements differ. See [network egress requirements](/gateway/k8s/installation/network-egress) for the hosts and ports to allow.
</Note>
...
If you're having trouble using the ngrok agent to start a tunnel, the first step is to run [`ngrok diagnose`](/gateway/agent/connectivity#troubleshooting) which will produce a report that will help you identify connection issues.
...
To generate the [more verbose diagnostic report](/gateway/agent/cli/#flags-11), update your job's `args` with the `-w` flag and file location:
