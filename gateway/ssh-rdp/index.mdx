...
<img src="./img/ssh-rdp-remote-access-architecture.png" alt="Architecture diagram for SSH and RDP remote access through ngrok" width="4236" height="1519" />
...
- An ngrok agent, configured on your edge gateway or remote device/service. See the [Agent CLI Quickstart](/gateway/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
- [An ngrok API Key](https://dashboard.ngrok.com/api-keys/new). You'll need an account first.

<Steps>
  <Step title="Create a Service User and authtoken to enable isolated Agent management">
    ...
    curl -X POST \
     -H "Authorization: Bearer <NGROK_API_KEY>" \
     -H "Content-Type: application/json" \
     -d '{"description":"Authtoken for Gateway 1", "owner_id":"<SERVICE_USER_ID>", "acl": ["bind:*.gateway1.internal"]}' \
     "https://api.ngrok.com/tunnel_credentials"
    ```
  </Step>

  <Step title="Define internal endpoints in ngrok.yml for privatized device access">
    An [internal endpoint](/gateway/internal-endpoints/) enables a service inside the network to be reachable within ngrok, without being publicly exposed. They can:
...
=======
An [internal endpoint](/gateway/endpoints/internal-endpoints/) enables a service inside the network to be reachable within ngrok, without being publicly exposed. They can:
>>>>>>> origin/main:gateway/ssh-rdp/index.mdx
  ...
  <Step title="Reserve a tcp address for each device/server">
  ...
  <Step title="Create your TCP Cloud Endpoint and attach a Traffic Policy">
    A [Cloud Endpoint](/gateway/cloud-endpoints/) is a permanent, externally accessible entry point into the network that's also:
...
=======
A [Cloud Endpoint](/gateway/endpoints/cloud-endpoints/) is a permanent, externally accessible entry point into the network that's also:
>>>>>>> origin/main:gateway/ssh-rdp/index.mdx

  <Step title="Secure your Cloud Endpoint with IP restrictions">
...
<<<<<<< HEAD:guides/ssh-rdp/index.mdx
    ...
    ```
=======
The curl command below leverages ngrok's platform API to create a Cloud Endpoint and attach a Traffic Policy to it. ngrok's [Traffic Policy](/gateway/traffic-policy/) is the perfect way to forward and manipulate traffic in a flexible and robust manner. In this case, this command creates the endpoint and attaches a [forward-internal](/gateway/traffic-policy/actions/forward-internal/) action which will forward traffic from the Cloud Endpoint to the internal endpoint. This will also need to be done for each device connected to the edge gateway where the agent sits.
>>>>>>> origin/main:gateway/ssh-rdp/index.mdx
  ...
  <Step title="Use your ngrok TCP endpoints with your SSH/RDP clients">
...
4. Not sure how to explain ngrok to your end users? Check out [this guide which details ngrok's standards on security, trust, compliance, and privacy](/gateway/site-to-site-connectivity/).
