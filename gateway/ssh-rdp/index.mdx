...
- An ngrok agent, configured on your edge gateway or remote device/service. See the [Agent CLI Quickstart](/endpoints/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
...
<Steps>
  ...
  <Step title="Define internal endpoints in ngrok.yml for privatized device access">
    An [internal endpoint](/endpoints/internal-endpoints/) enables a service inside the network to be reachable within ngrok, without being publicly exposed. They can:
  ...
  <Step title="Create your TCP Cloud Endpoint and attach a Traffic Policy">
    A [Cloud Endpoint](/endpoints/cloud-endpoints/) is a permanent, externally accessible entry point into the network that's also:
