...
<Tip>
Running in K8s? Check out [this guide](/gateway/site-to-site-connectivity/customer-networks) on securely connecting your local cluster to customer networks.
</Tip>
...
- The [ngrok agent](/gateway/endpoints/agent-cli-quickstart/) installed on a machine inside the remote network you want to connect to.
- A service running in the remote network that you want to reach (this quickstart uses port `8080` as an example).

<Steps>
  <Step title="Configure an Internal Endpoint">
...
<<<<<<< HEAD:guides/site-to-site-connectivity/quickstart.mdx
    Next, start the agent:
=======
Internal Endpoints are private endpoints that only receive traffic when a Cloud Endpoint forwards to them using the [forward-internal](/gateway/traffic-policy/actions/forward-internal/) Traffic Policy action.
>>>>>>> origin/main:gateway/site-to-site-connectivity/quickstart.mdx
...
- [Configure site-to-site connectivity](/gateway/site-to-site-connectivity/) for a full walkthrough with multiple endpoints, mTLS, and IP restrictions.
...
- [Install ngrok as a background service](/gateway/site-to-site-connectivity/background-service) so the agent starts on boot and recovers from failures.
- [Eliminate single points of failure](/gateway/site-to-site-connectivity/redundant-agents) by running redundant agents for high availability.
