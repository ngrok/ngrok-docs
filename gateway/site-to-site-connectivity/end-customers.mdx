...
Your vendor wants to create a secure persistent connection between your network and theirs, which allows them to access and take action on your services and data. This is called [**site-to-site connectivity**](https://ngrok.com/use-cases/saas-customer-connectivity).
...
15.3 million developers use ngrok. It is recommended by category leaders including Twilio, GitHub, Okta, Microsoft, Zoom, and Shopify. ngrok operates a global network and has handled over 100 trillion total requests.
...
ngrok has three primitives: the **cloud service**, the [**agent**](/gateway/agent/), and the [**dashboard**](https://dashboard.ngrok.com/).
...
2. The ngrok agent connects out to the ngrok network at the default [Connect URL](/gateway/agent/connect-url/) at `connect.ngrok-agent.com` (or a custom one).
...
agent](/gateway/agent/changelog/) to
...
- Enable the [IP restrictions](/gateway/traffic-policy/actions/restrict-ips/)
  action on your endpoints to allow only trusted connections from your vendor.
- Enforce [basic auth](/gateway/traffic-policy/actions/basic-auth/), [OAuth](/gateway/traffic-policy/actions/oauth/),
  [OpenID](/gateway/traffic-policy/actions/oidc/),
  [JWT](/gateway/traffic-policy/actions/jwt-validation/),
  [SAML](/gateway/traffic-policy/actions/saml), or
  [mTLS](/gateway/domains/mtls-termination/)
...
2. **TLS termination at the ngrok agent**. Often referred to as [zero-knowledge TLS](/gateway/domains/tls-termination/#end-to-end-encryption), this model encrypts your TLS tunnels end-to-end, using filesystem paths to your TLS certificate and key, without requiring you to reconfigure your upstream service for TLS termination.
3. **TLS termination at your upstream service**. This model implements [end-to-end zero-knowledge encryption](/gateway/domains/tls-termination/#end-to-end-encryption), where the ngrok network forwards unterminated connections through to your upstream application.
...
1. Configure your ngrok agent to trust a [specific
   root certificate](/gateway/agent/config/v3/#connect-cas) you own on the host's OS
...
The best way to disallow other uses of ngrok on your network is working with your vendor to specify a [custom Connect URL](/gateway/agent/connect-url/#custom-connect-url).

Your vendor will configure their DNS to issue certificates for the custom address, such as `your-company.us.connect.your-vendor.com:443`. Then they'll work with you to [reconfigure your ngrok agents](/gateway/agent/connect-url/) to utilize the custom Connect URL.
...
Intelligence](/gateway/traffic-policy/variables/ip-intel/) system is also used to prevent
...
documentation](/gateway/agent/install#system-requirements) for details about supported CPU
...
They might configure your ngrok agent to [run in the background](/gateway/agent/service#running-ngrok-in-the-background) as a native operating system service. This functionality works on all Linux, Windows, and macOS systems, and once installed, the ngrok service starts at boot-time, automatically restarts after crashes, and sends logs to your system's native logging service.
...
with the [Kubernetes Operator](/gateway/k8s/), they will recommend the [container
restart
...
- Binary from the [downloads page](https://download.ngrok.com): Follow the same process again or [update directly from your CLI](/gateway/agent/install#updates) with `ngrok update`.
...
1. [DNS resolution](/gateway/agent/connectivity#dns-resolution) to discover the appropriate connection address.
2. [Verification](/gateway/agent/connectivity#tls-verification) of the TLS certificate returned by the ngrok network, signed by ngrok's root certificate authority, against the certificate authorities bundled into the agent.
...
After the agent connects to the network, it also sends [heartbeat](/gateway/agent/connectivity#heartbeats) messages to validate the liveness of the connection, and [checks for updates](/gateway/agent/connectivity#ngrok-agent-update-check) on startup.

### How does the Agent continue running if my server loses connection or times out?

The ngrok agent utilizes a [heartbeat](/gateway/agent/connectivity#heartbeats) that attempts to reach ngrok's network every 10 seconds. If the agent doesn't receive a response within the tolerance window (15 seconds by default), it terminates the existing connection and attempts to reconnect.
...
Your vendor can [configure](/gateway/agent/config/v3/#heartbeat-interval) both the
