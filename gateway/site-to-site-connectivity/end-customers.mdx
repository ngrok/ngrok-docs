...
documentation](/gateway/agent/#system-requirements) for details about supported CPU
...
- Binary from the [downloads page](https://download.ngrok.com): Follow the same process again or [update directly from your CLI](/gateway/agent/#updates) with `ngrok update`.
...
1. [DNS resolution](/gateway/agent/#dns-resolution) to discover the appropriate connection address.
2. [Verification](/gateway/agent/#tls-verification) of the TLS certificate returned by the ngrok network, signed by ngrok's root certificate authority, against the certificate authorities bundled into the agent.
...
After the agent connects to the network, it also sends [heartbeat](/gateway/agent/#heartbeats) messages to validate the liveness of the connection, and [checks for updates](/gateway/agent/#ngrok-agent-update-check) on startup.

### How does the Agent continue running if my server loses connection or times out?

The ngrok agent utilizes a [heartbeat](/gateway/agent/#heartbeats) that attempts to reach ngrok's network every 10 seconds. If the agent doesn't receive a response within the tolerance window (15 seconds by default), it terminates the existing connection and attempts to reconnect.
