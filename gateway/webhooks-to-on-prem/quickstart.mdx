...
<Tip>
  ...
  tutorial](/gateway/webhooks-to-on-prem/tutorial) covers three variants: behind
...
This example verifies GitHub webhooks, but the shape is identical for any [supported provider](/gateway/traffic-policy/actions/verify-webhook/#supported-providers).
...
- [Follow the full tutorial](/gateway/webhooks-to-on-prem/tutorial) for a complete setup with multiple providers, vaulted secrets, and authtoken ACLs.
- [Store your signing secrets in a vault](/gateway/traffic-policy/secrets) instead of pasting them into your policy.
- [Verify webhooks from a provider that isn't on the supported list](/gateway/webhooks-to-on-prem/tutorial#verifying-a-provider-that-isnt-on-the-supported-list) using a shared-secret check or upstream verification.
- Read how [signature verification](/gateway/traffic-policy/actions/verify-webhook) works and review security best practices.
