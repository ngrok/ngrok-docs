...
<Note>
  ...
  providers](/gateway/traffic-policy/actions/verify-webhook/#supported-providers).
</Note>

## How it works

Webhooks hit one public address that you configure for your Cloud Endpoint. A single Traffic Policy on that address checks the signature on every request, rejects anything that fails, and only forwards verified requests. The ngrok agent opens an outbound connection to ngrok and forwards webhook traffic to your internal service, so you never have to open an inbound port or tap your firewall.
...
<Steps>
  <Step title="Give the receiving service a private address">
    Each service that handles webhooks gets an internal [Agent Endpoint](/gateway/agent-endpoints/): a private address that only receives traffic when it's forwarded through the [`forward-internal`](/traffic-policy/actions/forward-internal/) action. Internal endpoint hostnames must end with `.internal`.

<<<<<<< HEAD:guides/webhooks-to-on-prem/tutorial.mdx
    How you create it depends on where the service runs.
=======
Each service that handles webhooks gets an internal [Agent Endpoint](/gateway/endpoints/agent-endpoints/): a private address that only receives traffic when it's forwarded through the [`forward-internal`](/gateway/traffic-policy/actions/forward-internal/) action. Internal endpoint hostnames must end with `.internal`.
>>>>>>> origin/main:gateway/webhooks-to-on-prem/tutorial.mdx
  ...
  <Step title="Create the front door">
    Navigate to the [**Endpoints**](https://dashboard.ngrok.com/endpoints) section of the dashboard, click **New +**, then **Cloud Endpoint**, and enter the domain you want to use for your public URL. (In Kubernetes, you can create this as a [`CloudEndpoint`](/k8s/crds/cloudendpoint/) resource instead.)
  </Step>

  <Step title="(Optional) Store your signing secrets in a vault">
  ...
  <Step title="Verify the sender and route inward">
          ...
          - expressions:
            ...
            actions:
              ...
              - type: forward-internal
                config:
                  url: https://deploy-service.internal
=======
  <Tab title="In Kubernetes">
    Let the [ngrok Operator](/gateway/k8s/) manage the endpoint as a version-controlled resource, so the platform team owns it in Git rather than in someone's shell history.
>>>>>>> origin/main:gateway/webhooks-to-on-prem/tutorial.mdx
          ...
          - expressions:
            ...
            actions:
              ...
              - type: forward-internal
                config:
                  url: https://notif-service.internal
            ...
            actions:
              ...
              - type: forward-internal
                config:
                  url: https://notif-service.internal
=======
Navigate to the [**Endpoints**](https://dashboard.ngrok.com/endpoints) section of the dashboard, click **New +**, then **Cloud Endpoint**, and enter the domain you want to use for your public URL. (In Kubernetes, you can create this as a [`CloudEndpoint`](/gateway/k8s/crds/cloudendpoint/) resource instead.)
>>>>>>> origin/main:gateway/webhooks-to-on-prem/tutorial.mdx
...
=======
Each route verifies a provider's signature using that provider's signing secret. For production, keep those secrets in [Secrets for Traffic Policy](/gateway/traffic-policy/secrets) so they're encrypted and referenced by name rather than pasted in plaintext. This step is optional, but a vault keeps secrets out of your policy source and lets you rotate them without editing routes.
>>>>>>> origin/main:gateway/webhooks-to-on-prem/tutorial.mdx

  <Step title="Add the guardrail your use case needs">
...
=======
On your Cloud Endpoint's Traffic Policy, each rule matches a provider by request path, verifies the signature with [`verify-webhook`](/gateway/traffic-policy/actions/verify-webhook), and forwards verified requests to the matching internal service with [`forward-internal`](/gateway/traffic-policy/actions/forward-internal/). A request with a bad or missing signature is rejected with a `403`.
>>>>>>> origin/main:gateway/webhooks-to-on-prem/tutorial.mdx
  ...
  <Step title="Configure your webhook URL at each provider and test">
...
<Tabs>
  ...
  <Tab title="Regulated workloads">
    ...
    - **End-to-end encryption.** Terminate TLS within your network at [the ngrok agent](/gateway/agent/agent-tls-termination) or at your upstream service. With this configuration, ngrok forwards ciphertext rather than decrypting traffic at the edge.
    - **No stored payloads.** Leave full traffic capture off. Use TCP or TLS endpoints to bypass all traffic capture, including most request metadata.
  ...
  <Tab title="Platform-governed">
    ...
    - **Scope the agent's authtoken with an ACL** so you define what endpoints that agent can start. See [Authtoken ACLs](/gateway/agent/cli-api) for the full syntax, but as an example:
...
`verify-webhook` supports a fixed set of [providers](/gateway/traffic-policy/actions/verify-webhook/#supported-providers). If your webhook provider isn't on the list, here are your options:
...
Combine with the [`restrict-ips`](/gateway/traffic-policy/actions/restrict-ips) action locked to the provider's published IP ranges or with `jwt-validation` if the webhook arrives as a signed JWT.

### Ask us to add it

Email us at support@ngrok.com to [request your provider](/gateway/traffic-policy/actions/verify-webhook/#supported-providers).
