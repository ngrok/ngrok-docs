...
<Steps>
  <Step title="Give the receiving service a private address">
    Each service that handles webhooks gets an internal [Agent Endpoint](/endpoints/agent-endpoints/): a private address that only receives traffic when it's forwarded through the [`forward-internal`](/traffic-policy/actions/forward-internal/) action. Internal endpoint hostnames must end with `.internal`.
