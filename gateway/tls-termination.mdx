...
- Uses the [TLS
  Certificate](/gateway/tls-certificates/) attached to the
...
| --------------------------------- | --------------------------------------------------------- |
| [HTTP](/gateway/http/)  | None                                                      |
| [HTTPS](/gateway/http/) | Always at ngrok's cloud service.                          |
| [TLS](/gateway/tls/)    | Default no termination, configurable with `terminate-tls` |
| [TCP](/gateway/tcp/)    | Default no termination, configurable with `terminate-tls` |
...
balancer](/gateway/global-load-balancer).

## Certificates

When the ngrok cloud service terminates TLS connections, it does so with the
[TLS Certificate](/gateway/tls-certificates/) attached to the
[Domain](/gateway/domains/) which matches the Endpoint URL's
hostname. See the documentation on [TLS
Certificates](/gateway/tls-certificates/) for more details on
how they are
[selected](/gateway/tls-certificates/#certificate-selection),
