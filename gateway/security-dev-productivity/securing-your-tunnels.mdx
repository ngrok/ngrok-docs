...
- If your local service is not running on the same machine as the ngrok agent, it is recommended to set up TLS encryption for the ngrok agent to the upstream service leg of the tunnel using the [local HTTPS feature](/endpoints/http/#https-forwarding).
- For custom domains, use ngrok's [Automated TLS certificates](/domains/tls-certificates/#automated) to have ngrok automatically provision a TLS certificate for your endpoint from [Let's Encrypt](https://letsencrypt.org/about/).
...
See [Terminating TLS Connections](/domains/tls-termination/) for more details.

### Minimum TLS version

It is possible to specify the minimum TLS version that clients are required to use to talk to the ngrok cloud service for your endpoint. See [the `terminate-tls` Traffic Policy action](/traffic-policy/actions/terminate-tls/#changing-supported-tls-versions) for more details.
