...
- Assign a [unique Authtoken to each ngrok agent](/gateway/agent/authtokens) deployment to isolate issues if a specific Authtoken is compromised.
- Set up a [minimum ACL per Authtoken](/gateway/agent/authtokens#authtoken-acls) to limit the endpoints each agent is able to start.
...
- If your local service is not running on the same machine as the ngrok agent, it is recommended to set up TLS encryption for the ngrok agent to the upstream service leg of the tunnel using the [local HTTPS feature](/gateway/endpoints/http/#https-forwarding).
- For custom domains, use ngrok's [Automated TLS certificates](/gateway/domains/tls-certificates/#automated) to have ngrok automatically provision a TLS certificate for your endpoint from [Let's Encrypt](https://letsencrypt.org/about/).
...
If your organization uses a custom Connect URL, your default ngrok configuration will not work. Edit your ngrok agent configuration to add a [`connect_url`](/gateway/agent/config/v3/#connect-url) parameter to use the custom Connect URL of your organization.
...
See [Terminating TLS Connections](/gateway/domains/tls-termination/) for more details.

### Minimum TLS version

It is possible to specify the minimum TLS version that clients are required to use to talk to the ngrok cloud service for your endpoint. See [the `terminate-tls` Traffic Policy action](/gateway/traffic-policy/actions/terminate-tls/#changing-supported-tls-versions) for more details.
