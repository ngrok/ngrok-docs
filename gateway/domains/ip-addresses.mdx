...
The information on this page about IPs also applies not only to Endpoint IPs but also to the IPs used for [Agent Connections](/gateway/agent/connect-url/), the Dashboard, and the [API](/api/). 
...
These can be configured for [Domains](/gateway/domains/dedicated-ips/) and [Agent Connect URLs](/gateway/agent/connect-url/#dedicated-ips).
...
Policy action](/gateway/traffic-policy/actions/restrict-ips/). If you forget to allow IPv6
...
balancer](/gateway/domains/global-load-balancer/) will return a different set
