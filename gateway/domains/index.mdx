...
- [**Dedicated IP address configuration**](/gateway/domains/dedicated-ips/) - Attach static, dedicated IPs for a Domain's matching endpoints receive traffic on.
...
endpoints](/gateway/endpoints/public-endpoints/) with a hostname matching the
...
| `tcp`             | Not allowed. Public TCP endpoints must match a [TCP Address](/gateway/domains/tcp-addresses/). |

If you configure your Domain to use [dedicated IPs](/gateway/domains/dedicated-ips/), these
...
`app.your-domain.com`. To do so, see [the docs on using custom domains](/gateway/domains/custom-domains).

## Global Load Balancer

The [Global Load Balancer](/gateway/domains/global-load-balancer)
...
Domains manage the [TLS Certificate](/gateway/domains/tls-certificates/) used
...
Certificates](/gateway/domains/tls-certificates/) for additional details on
certificate provisioning and management.

## Dedicated IPs

By default, a Domain's matching Endpoints receive traffic on a set of multi-tenant [IP addresses](/gateway/domains/ip-addresses) that are shared among all ngrok accounts.
...
See the [Dedicated IPs](/gateway/domains/dedicated-ips/) documentation for detailed information on configuration, use cases, and pricing.
