...
<Steps titleSize="h2">
  ...
  <Step title="Wait for the TLS certificate to provision">
    ...
    If `provisioning_job` reports a DNS error, double-check that the CNAME record from the [Add the CNAME record to your domain](#add-the-cname-record-to-your-domain) step is published and resolvable with `dig` or `nslookup`.
