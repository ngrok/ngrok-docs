...
Dedicated IPs let you configure static IP addresses that are exclusively assigned to your account for receiving traffic to your [domains](/gateway/domains/) and [agent connections](/gateway/agent/connect-url/). 
...
- **Protocol support** - Available for any [ngrok-supported protocol](/gateway/endpoints/protocols/), including HTTP/S, TCP, and TLS
...
Dedicated IPs and [Region Resolution](/gateway/domains/region-pinning/) (region pinning) share the same `resolves_to` field on the domain resource:
...
Dedicated IPs are also available for [Agent Connect URLs](/gateway/agent/connect-url/#dedicated-ips), providing account-unique IP addresses for your agents to connect to ngrok's service.
...
See [Agent Connect URL documentation](/gateway/agent/connect-url/#dedicated-ips) for more information.
...
- [Region & IP Resolution](/gateway/domains/region-pinning/): Pin traffic to specific geographic regions
- [IP Addresses](/gateway/domains/ip-addresses/): Learn about ngrok's IP addressing and dynamic IPs
- [Agent Connect URLs](/gateway/agent/connect-url/): Configure custom connect URLs for agents
- [Traffic Policy](/gateway/traffic-policy/): Apply rules and transformations to your traffic
...
Yes, dedicated IPs work with all [ngrok-supported protocols](/gateway/endpoints/protocols/), including HTTP/S, TCP, and TLS.
