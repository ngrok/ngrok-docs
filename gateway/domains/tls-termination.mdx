...
- Uses the [TLS
  Certificate](/gateway/domains/tls-certificates/) attached to the
  [Domain](/gateway/domains/) which matches the Endpoint URL's
  hostname
...
| --------------------------------- | --------------------------------------------------------- |
| [HTTP](/gateway/endpoints/http/)  | None                                                      |
| [HTTPS](/gateway/endpoints/http/) | Always at ngrok's cloud service.                          |
| [TLS](/gateway/endpoints/tls/)    | Default no termination, configurable with `terminate-tls` |
| [TCP](/gateway/endpoints/tcp/)    | Default no termination, configurable with `terminate-tls` |
...
See [Zero-Knowledge TLS at the Agent](/gateway/agent/agent-tls-termination/) for additional details.
...
Action](/gateway/traffic-policy/actions/terminate-tls) enables you to terminate TLS
...
of presence](/gateway/points-of-presence).
...
presence](/gateway/points-of-presence/) via its [global load
balancer](/gateway/domains/global-load-balancer).

## Certificates

When the ngrok cloud service terminates TLS connections, it does so with the
[TLS Certificate](/gateway/domains/tls-certificates/) attached to the
[Domain](/gateway/domains/) which matches the Endpoint URL's
hostname. See the documentation on [TLS
Certificates](/gateway/domains/tls-certificates/) for more details on
how they are
[selected](/gateway/domains/tls-certificates/#certificate-selection),
managed, provisioned and renewed.

You may customize which TLS certificate is chosen for termination with the
[`terminate-tls`](/gateway/traffic-policy/actions/terminate-tls) Traffic Policy action.
...
configuration](/gateway/agent/config/v3/#endpoint-configuration-options) in the agent
configuration file.
...
Read the [Agent TLS Termination Guide](/gateway/agent/agent-tls-termination/) for a step-by-step approach to set it up.
...
| Agent               | Documentation                                                                                           |
| ------------------- | ------------------------------------------------------------------------------------------------------- |
| Agent Config File   | [`agent_tls_termination`](/gateway/agent/config/v3/#endpoint-configuration-options)                             |
