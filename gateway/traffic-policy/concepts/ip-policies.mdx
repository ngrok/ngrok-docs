...
To enforce an IP Policy on your endpoints, use the [`restrict-ips`](/gateway/traffic-policy/actions/restrict-ips) Traffic Policy action.
...
- **Agent config file**: Define the policy inline inside your [ngrok agent config file](/gateway/agent/config/v3) (for example, `ngrok.yml`).
...
To embed the same policy in your [ngrok agent config file](/gateway/agent/config/v3), nest the policy block under `traffic_policy:` inside the endpoint definition:
...
For a full reference of all configuration options, behavior, and examples for the `restrict-ips` action, see the [Restrict IPs action reference](/gateway/traffic-policy/actions/restrict-ips).
...
In the dashboard UI, you can apply [IP Restrictions](/gateway/traffic-policy/examples/add-authentication#ip-restrictions) to users trying to sign in to your ngrok dashboard, traffic trying to access your API or Endpoints, and source IPs trying to start Agent Endpoints on your account.
You can define the IP Policies that make up your restrictions in the dashboard UI.

## Related resources

- [Restrict IPs Traffic Policy action](/gateway/traffic-policy/actions/restrict-ips): Full reference for the `restrict-ips` action, including all configuration options and examples.
