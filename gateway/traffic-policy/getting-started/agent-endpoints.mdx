...
[Traffic Policy](/gateway/traffic-policy/) enables you to manage traffic to your ngrok endpoints by dynamically implementing logic based on the request before it reaches your upstream services. This guide will teach you how to configure an [Agent Endpoint](/gateway/endpoints/agent-endpoints) with a custom Traffic Policy using the [ngrok Agent CLI](/gateway/agent/cli/).
...
- You can edit your [Agent Configuration File](/gateway/agent/config/). This file applies to all endpoints you start with the agent CLI by default. Choose this approach to set up traffic management logic for all endpoints running locally on your machine.
...
<Steps>
  <Step title="Set up your Traffic Policy">
    <Tabs>
...
<<<<<<< HEAD:traffic-policy/getting-started/agent-endpoints.mdx
    Your configuration file `version` [must be set to `3`](/agent/config/v3).
=======
Your configuration file `version` [must be set to `3`](/gateway/agent/config/v3).
>>>>>>> origin/main:gateway/traffic-policy/getting-started/agent-endpoints.mdx
...
- Learn [how the Traffic Policy engine works under the hood](/gateway/traffic-policy/how-it-works).
- Check out [the example gallery](/gateway/traffic-policy/examples/) to see Traffic Policy in real-world scenarios.
- The list of [available actions](/gateway/traffic-policy/actions/), [macros](/gateway/traffic-policy/macros/) and [variables](/gateway/traffic-policy/variables/) you can use.
