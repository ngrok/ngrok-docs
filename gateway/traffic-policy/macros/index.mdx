---
title: Traffic Policy Macros Reference
sidebarTitle: Macros
...
ngrok offers a variety of CEL macros that can be used within the Traffic Policy engine to simplify [traffic management](../concepts/expressions) and [dynamic configuration](/gateway/traffic-policy/how-it-works#cel-interpolation).  
...
Available in [`on_http_request`](/gateway/traffic-policy/how-it-works#phases) and [`on_http_response`](/gateway/traffic-policy/how-it-works#phases) phases.
...
Available in [`on_http_response`](/gateway/traffic-policy/how-it-works#phases) phase only.
...
For more information on secrets, see [Traffic Policy Secrets](/gateway/traffic-policy/secrets/).
