...
Sometimes you don't want everyone to be able to access the app at your [Endpoints](/endpoints).

This guide will walk you through protecting your apps with OAuth authentication via [Traffic Policy](/traffic-policy). All you need is [the ngrok agent CLI](/gateway/agent/cli).
...
<Steps>
  ...
  <Step title="Create a Traffic Policy file">
...
<<<<<<< HEAD:traffic-policy/examples/oauth-protection.mdx
    <CodeGroup>
=======
The following is an example Traffic Policy file that executes the ['oauth' Traffic Policy action](/gateway/traffic-policy/actions/oauth/) on every request to your endpoint.
>>>>>>> origin/main:gateway/traffic-policy/examples/oauth-protection.mdx
  ...
  <Step title="Restart your endpoint with the policy attached">
    <Tabs>
  ...
  <Step title="Verify in a browser">
...
[Traffic Policy](/gateway/traffic-policy/) is a composable CEL-based configuration language that lets you match, filter, and control traffic to your endpoints without touching your app code. [Learn more about the building blocks of Traffic Policy](/gateway/traffic-policy/#traffic-policy-building-blocks).
