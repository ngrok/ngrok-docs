...
This rule uses the [connection variables](/gateway/traffic-policy/variables/connection/) available in [IP Intelligence](/gateway/traffic-policy/variables/ip-intel) to block [Tor](<https://en.wikipedia.org/wiki/Tor_(network)>) exit node IPs.
...
You can also take action on incoming requests that contain specific strings in [the `req.user_agent` request variable](/gateway/traffic-policy/variables/req/#requser-agent).
...
In this example, the Algolia web crawler is exempted from any rate limiting configured on your site. See [the IP Intelligence docs](/gateway/traffic-policy/variables/ip-intel/#ip-categories) for other bots and crawlers that are available.
