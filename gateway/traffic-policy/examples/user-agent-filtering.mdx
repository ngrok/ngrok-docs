...
With Traffic Policy, ngrok automatically parses out useful information from the request's `User-Agent` header and makes them [available as variables](/gateway/traffic-policy/variables/req/#requser-agentname). You can use them to deliver tailored content to users based on their browser and device, or to customize or block for SEO crawlers and bots.
...
Check out the [ngrok variable documentation](/gateway/traffic-policy/variables/req/#requser-agentname) for all the values you can use in your policy.
