...
The [Traffic Policy](/gateway/traffic-policy/) configuration
...
The action will also set the `X-Forwarded-For`, `X-Forwarded-Host`, and `X-Forwarded-Proto` headers when making the upstream request. See [Upstream Headers](/gateway/endpoints/http#upstream-headers) for more information.
...
<Note>
  These variables are only populated when `on_error` is set to `continue` and
  the forward fails.
</Note>

<ConfigField title="actions.ngrok.forward_internal.url" type="string" cel={true}>
  The resolved target URL (`scheme://host:port`) that the action attempted to
  forward to.
</ConfigField>

<ConfigField title="actions.ngrok.forward_internal.error.code" type="string">
  A machine-readable `ERR_NGROK_*` code describing the error that caused the
  forward to fail.
</ConfigField>

<ConfigField title="actions.ngrok.forward_internal.error.message" type="string">
  A human-readable message providing details about the error that caused the
  forward to fail.
</ConfigField>
