...
- On a periodic interval defined by the [`userinfo_refresh_interval`](/gateway/traffic-policy/actions/oauth/#configuration-fields) parameter.
...
- The [upstream headers](/gateway/endpoints/http/#upstream-headers) `ngrok-auth-oauth-access-token` and `ngrok-auth-oauth-refresh-token` are not sent to your application.
...
For more ways to restrict access based on OAuth result variables, see [the Add Authentication example](/gateway/traffic-policy/examples/add-authentication/#oauth).
