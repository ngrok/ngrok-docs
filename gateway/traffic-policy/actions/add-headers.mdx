...
To replace or remove headers use the [`remove-headers`](/gateway/traffic-policy/actions/remove-headers) action then
...
https://httpbin.org and the following header is added to all requests:
...
For this example, assume that ngrok is pointing at the upstream service
https://httpbin.org and two headers are added:
