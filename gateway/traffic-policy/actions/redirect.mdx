...
This action is different from the [URL Rewrite Action](/gateway/traffic-policy/actions/url-rewrite/), which changes the request internally while keeping the client's URL unchanged. 
Redirects, by contrast, instruct the client to issue a new request to the new URL.

## Configuration reference

The [Traffic Policy](/gateway/traffic-policy/) configuration reference for this action.
...
The following [Traffic Policy](/gateway/traffic-policy/) configuration demonstrates how to use the `redirect` action to redirect incoming requests from `/products` to `/store/products`.
...
The following [Traffic Policy](/gateway/traffic-policy/) configuration demonstrates how to use the `redirect` action to redirects requests from an old API endpoint to a new one.
...
The following [Traffic Policy](/gateway/traffic-policy/) configuration demonstrates how to use the `redirect` action to redirect users while using CEL Interpolation.
