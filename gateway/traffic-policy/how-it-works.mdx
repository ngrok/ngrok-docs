...
The following example uses an [expression](#expressions) to check if the client IP is `192.168.1.200`, and uses [the `deny` action](/gateway/traffic-policy/actions/deny) to deny the connection if it is.
...
| `expressions` | No | A list of [CEL expressions](/gateway/traffic-policy/concepts/expressions). All must evaluate to `true` for the actions to run. Omitting `expressions` matches all traffic. |
| `actions` | Yes | An ordered list of [actions](/gateway/traffic-policy/concepts/actions) to execute. Each action requires a `type` and may include a `config` block whose required fields depend on the action type (for example, `oauth` requires `config.provider`). |
...
For a full list of actions, check out the [Action Hub](/gateway/traffic-policy/actions/).
...
- [Variables](/gateway/traffic-policy/variables/) (including action variables)
- [Macros](/gateway/traffic-policy/macros/)
...
- [Create your first Traffic Policy](/gateway/traffic-policy/getting-started/agent-endpoints/) to start managing your traffic.
- Explore [the Traffic Policy Action Hub](/gateway/traffic-policy/actions/) to see all the Actions available to you.
- Explore [practical examples](/gateway/traffic-policy/examples/) to see Traffic Policies in action.
