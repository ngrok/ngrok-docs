...
If you start an endpoint and open the custom domain in a browser before the certificate is ready, you'll see a certificate name-mismatch warning (for example, `NET::ERR_CERT_COMMON_NAME_INVALID` in Chrome).
This is expected during the provisioning window and resolves on its own as soon as the certificate is issued — there's no action to take other than waiting and reloading.
