...
ngrok's [Endpoint Pooling](/gateway/endpoints/endpoint-pooling) allows you to automatically load balance between replicas of your service across multiple clouds by creating two endpoints with the same URL.
...
Start with a single [Agent Endpoint](/gateway/endpoints/agent-endpoints/) on an ngrok URL, or a [reserved domain](https://dashboard.ngrok.com/domains), with the `--pooling-enabled` flag on.
...
Pooling works with [internal Agent Endpoints](/gateway/endpoints/internal-endpoints/), which can only receive traffic from other endpoints associated with your ngrok account.

A [Cloud Endpoint](/gateway/endpoints/cloud-endpoints/) is a persistent public endpoint where you can manage a Traffic Policy for any upstream endpoints it forwards traffic to.
...
<Steps>
  <Step title="Set up your internal Agent Endpoints">
  ...
  <Step title="Set up your Cloud Endpoint">
  ...
  <Step title="Add a Traffic Policy">
...
- [`url-redirect`](/gateway/traffic-policy/actions/redirect)
...
services](/gateway/k8s/load-balancing/load-balancing-kubernetes). If you're
interested in a more production-ready deployment, take a look at the [multicloud
API gateway setup tutorial](/gateway/api-gateway/multicloud).
