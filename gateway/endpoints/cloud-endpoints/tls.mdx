...
import TLSStart from "/snippets/gateway/shared/tls-source-start.mdx"
import TLSEnd from "/snippets/gateway/shared/tls-source-end.mdx"
...
To terminate TLS Cloud Endpoint traffic at an agent or an upstream service, you must create a Cloud Endpoint, then use the [`forward-internal`](/gateway/traffic-policy/actions/forward-internal/) action to forward traffic to an internal endpoint.
...
To learn more about terminating TLS at the agent, see the [Agent Endpoint TLS documentation](/gateway/endpoints/tls#termination).
...
When performing [end-to-end encryption](/gateway/domains/tls-termination/#end-to-end-encryption) by terminating at the upstream service, you become responsible for provisioning, managing, and distributing certificates.
