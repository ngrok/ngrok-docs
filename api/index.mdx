...
libraries in your programming language of choice. Other ways to
interact with the API include native API client libraries, a native CLI and a
Terraform Provider.

#### Other ways to interact with the API

- **[Agent API CLI](/gateway/agent/cli-api/)**: Explore and script the API via the `ngrok api` commands included in the ngrok agent.
...
- **[Agent API](/gateway/agent/api)**: The ngrok agent has its own API if you're trying to dynamically start and stop endpoints and tunnels
...
It is important to provide a well-defined interface to errors
...
[API IP Restrictions](#ip-restrictions) is available on the Pay-as-you-go plan.
