/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */

// @ts-check

/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */
const sidebars = {
  // By default, Docusaurus generates a sidebar from the docs folder structure
  docs: [
    'overview',
    'getting-started',
    {
      label: 'Guides',
      type: 'category',
      link: { type: 'doc', id: 'guides/index' },
      items: [ { type: 'autogenerated', dirName: 'guides',}, ],
    },
    {
      label: 'ngrok Cloud Edge',
      type: 'category',
      link: { type: 'doc', id: 'cloud-edge/index' },
      items: [ { type: 'autogenerated', dirName: 'cloud-edge', }, ],
    },
    {
      label: 'ngrok Secure Tunnels',
      type: 'category',
      link: { type: 'doc', id: 'secure-tunnels/index' },
      items: [ { type: 'autogenerated', dirName: 'secure-tunnels', }, ],
    },
    {
      label: 'ngrok Platform',
      type: 'category',
      link: { type: 'doc', id: 'platform/index' },
      items: [ { type: 'autogenerated', dirName: 'platform', }, ],
    },
    {
      label: 'Using ngrok with...',
      type: 'category',
      link: { type: 'doc', id: 'using-ngrok-with' },
      items: [ { type: 'autogenerated', dirName: 'using-ngrok-with', }, ],
    },
    {
      label: 'Reference',
      type: 'category',
      items: [ 
        {
          type: 'category',
          label: 'ngrok agent',
          link: { type: 'doc', id: 'reference/ngrok-agent' },
          items: [ { type: 'autogenerated', dirName: 'reference/ngrok-agent', }, ],
        },
        'reference/api/reference', //TODO: File issue to update API link title and uri
        {
          type: 'doc',
          label: 'Error Codes',
          id: 'reference/errors/index'
        },
        'reference/event-sources',
        'reference/event-filtering',
        'reference/http-header-templates',
      ],
    },
    'faq',
  ],

  events: [
    'reference/event-sources',
    'reference/event-filtering',
  ],

  errors: [
    { type: 'autogenerated', dirName: 'reference/errors', }
  ],
};

module.exports = sidebars;
