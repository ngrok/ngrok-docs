...
- [**Dedicated IP address configuration**](/domains/dedicated-ips/) - Attach static, dedicated IPs for a Domain's matching endpoints receive traffic on.
...
endpoints](/endpoints/public-endpoints/) with a hostname matching the
...
| `tcp`             | Not allowed. Public TCP endpoints must match a [TCP Address](/domains/tcp-addresses/). |

If you configure your Domain to use [dedicated IPs](/domains/dedicated-ips/), these
...
- Create an endpoint which receives traffic for all of its subdomains, for example,
  ...
  endpoints](/endpoints/http/#wildcard-endpoints) to understand
...
`app.your-domain.com`. To do so, see [the docs on using custom domains](/domains/custom-domains).

## Global Load Balancer

The [Global Load Balancer](/domains/global-load-balancer)
...
Domains manage the [TLS Certificate](/domains/tls-certificates/) used
...
Certificates](/domains/tls-certificates/) for additional details on
certificate provisioning and management.

## Dedicated IPs

By default, a Domain's matching Endpoints receive traffic on a set of multi-tenant [IP addresses](/domains/ip-addresses) that are shared among all ngrok accounts.
...
See the [Dedicated IPs](/domains/dedicated-ips/) documentation for detailed information on configuration, use cases, and pricing.
