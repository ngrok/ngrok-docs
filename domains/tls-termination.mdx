...
- Uses the [TLS
  Certificate](/domains/tls-certificates/) attached to the
  [Domain](/domains/) which matches the Endpoint URL's
  hostname
...
balancer](/domains/global-load-balancer).

## Certificates

When the ngrok cloud service terminates TLS connections, it does so with the
[TLS Certificate](/domains/tls-certificates/) attached to the
[Domain](/domains/) which matches the Endpoint URL's
hostname. See the documentation on [TLS
Certificates](/domains/tls-certificates/) for more details on
how they are
[selected](/domains/tls-certificates/#certificate-selection),
