...
<Steps>
  ...
  <Step title="Wait for the TLS certificate to provision">
    ...
    If `provisioning_job` reports a DNS error, double-check that the CNAME record you added in step 2 is published and resolvable with `dig` or `nslookup`.
    For wildcard custom domains, both CNAME records described in [TLS certificate provisioning](/domains/tls-certificates/#wildcard-domains) must exist before the certificate can be issued.
    </Tip>
  </Step>
</Steps>
...
If you bring your own [wildcard domain](/domains#wildcard-domains) (such as `*.example.com`), you must create a second `CNAME` record with your domain host for [wildcard TLS Certificate provisioning](/domains/tls-certificates/#wildcard-domains).
