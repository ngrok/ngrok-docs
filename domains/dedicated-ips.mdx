...
Dedicated IPs let you configure static IP addresses that are exclusively assigned to your account for receiving traffic to your [domains](/domains/) and [agent connections](/agent/connect-url/). 
...
- **Custom port support** - When used with domains, create [endpoints](/endpoints/) on any inbound port (not restricted to ports 80 and 443)
- **Protocol support** - Available for any [ngrok-supported protocol](/endpoints/protocols/), including HTTP/S, TCP, and TLS
...
When you configure a domain with dedicated IPs, all matching [endpoints](/endpoints/) for that domain receive traffic on your dedicated IP addresses instead of ngrok's shared multi-tenant IPs.

Dedicated IPs and [Region Resolution](/domains/region-pinning/) (region pinning) share the same `resolves_to` field on the domain resource:
...
- [Domains](/domains/): Configure domains for your endpoints
- [Region & IP Resolution](/domains/region-pinning/): Pin traffic to specific geographic regions
- [IP Addresses](/domains/ip-addresses/): Learn about ngrok's IP addressing and dynamic IPs
...
Yes, dedicated IPs work with all [ngrok-supported protocols](/endpoints/protocols/), including HTTP/S, TCP, and TLS.
