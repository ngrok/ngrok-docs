...
1. Create `service.js` — a Node.js HTTP server on port 8080 that returns an HTML page saying "Hello from Node.js HTTP Server!"
2. Install dependencies: `npm install @ngrok/ngrok dotenv`
3. Create a `.env` file with NGROK_AUTHTOKEN and NGROK_DOMAIN — ask me for these values
4. Create `endpoint.js` using `ngrok.forward()` to tunnel port 8080 to my reserved domain with a Google OAuth traffic policy
5. Run `node service.js` in one terminal and `node endpoint.js` in another
...
You can think of it as the [ngrok Agent CLI](/gateway/endpoints/agent-cli-quickstart/) packaged as a JS library.
...
<Steps titleSize="h2">
  ...
  <Step title="Create your endpoint">
    ...
    Create a new file named `endpoint.js` in the same directory as your `.env` file and add the following code.
    This example:
