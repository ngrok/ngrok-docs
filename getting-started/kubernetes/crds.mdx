...
This quickstart uses the [ngrok Kubernetes Operator](/k8s) and [ngrok's custom resources](/gateway/k8s/crds) (CRs) to make the services you've deployed to Kubernetes available on the public internet.
...
<Steps>
  ...
  <Step title="Deploy a sample service">
    <K8sSample />
  </Step>

  <Step title="Create your endpoint manifest">
    ...
    2. A pair of `CloudEndpoint` and `AgentEndpoint` resources: This option allows you to route traffic to any number of Kubernetes services from a single hostname depending on their path, headers, and more using [expressions](/traffic-policy/how-it-works#expressions) and the [`forward-internal` Traffic Policy action](/traffic-policy/actions/forward-internal).
    ...
    For more information about cloud and Agent Endpoints, see [the endpoint types page](/gateway/types/).
...
=======
1. A single `AgentEndpoint` resource: This option is simplest, but is typically limited to exposing one service on a hostname.
2. A pair of `CloudEndpoint` and `AgentEndpoint` resources: This option allows you to route traffic to any number of Kubernetes services from a single hostname depending on their path, headers, and more using [expressions](/gateway/traffic-policy/how-it-works#expressions) and the [`forward-internal` Traffic Policy action](/gateway/traffic-policy/actions/forward-internal).
>>>>>>> origin/main
...
In cases where you need to restrict access to your API or app, [Traffic Policy](/traffic-policy) and the [`restrict-ips`](/gateway/traffic-policy/actions/restrict-ips) let you quickly allow only certain addresses to access your endpoint.
...
>>>>>>> origin/main
  ...
  <Step title="Secure your app with Traffic Policy">
...
- Read the [guide to ngrok's CRDs](/gateway/k8s/guides/using-crds), including how to create a Cloud Endpoint
- [Route requests to many upstream services](/gateway/k8s/guides/how-to/request-routing/) based on the path, headers, and query parameters
- Deploy an [API gateway](/gateway/api-gateway/get-started) in one or [multiple clouds](/gateway/api-gateway/multicloud/)
- [Learn how to configure HTTP/HTTPS/TCP/TLS traffic](/gateway/k8s/guides/endpoint-types/) with `CloudEndpoint` and `AgentEndpoint`.

For more ways to filter and manage traffic:

- [Traffic Policy overview](/traffic-policy)
- [Traffic Policy concepts](/gateway/traffic-policy/how-it-works)
- All [available actions](/gateway/traffic-policy/actions/)
