...
This quickstart uses the [ngrok Kubernetes Operator](/k8s) and the ngrok agent to create a [Kubernetes-bound Endpoint](/endpoints/kubernetes-endpoints/), which is accessible to other pods in your cluster as a native Service. ngrok then handles all routing to the upstream service, which can be running anywhere, like your development laptop or a cloud-based developer environment.
...
- An ngrok account
- The [ngrok CLI](/endpoints/agent-cli-quickstart/#1-install-the-ngrok-agent-cli) installed on your local machine
...
- [Kubernetes Endpoints](/endpoints/kubernetes-endpoints/)
...
- [Traffic Policy concepts](/traffic-policy/how-it-works)
