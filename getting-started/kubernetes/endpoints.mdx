...
import K8sInstallBindings from "/snippets/k8s/_install-bindings.mdx";

This quickstart uses the [ngrok Kubernetes Operator](/k8s) and the ngrok agent to create a [Kubernetes-bound Endpoint](/gateway/endpoints/kubernetes-endpoints/), which is accessible to other pods in your cluster as a native Service. ngrok then handles all routing to the upstream service, which can be running anywhere, like your development laptop or a cloud-based developer environment.
...
- Securely access your customer's APIs or databases (via [site-to-site connectivity](/gateway/site-to-site-connectivity/)).
...
- An ngrok account
<<<<<<< HEAD
- The [ngrok CLI](/gateway/agent-cli-quickstart/) installed on your local machine
=======
- The [ngrok CLI](/gateway/endpoints/agent-cli-quickstart/#1-install-the-ngrok-agent-cli) installed on your local machine
>>>>>>> origin/main
- A running K8s cluster with `kubectl` access with at least one service
  - If you don't have a cluster yet, see the [local cluster guide](/gateway/k8s/guides/local-cluster/) for some options
...
<Steps>
  ...
  <Step title="Start an Agent Endpoint">
  ...
  <Step title="Add Traffic Policy to manipulate requests (optional)">
...
- [Kubernetes Endpoints](/gateway/endpoints/kubernetes-endpoints/)
- [Using Endpoint Bindings with the Operator](/gateway/k8s/guides/bindings/)
...
- All [available actions](/gateway/traffic-policy/actions/), including a few most relevant to these types of endpoints:
  - [`add-headers`](/gateway/traffic-policy/actions/add-headers/)
  - [`rate-limit`](/gateway/traffic-policy/actions/rate-limit/)
