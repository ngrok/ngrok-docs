...
This quickstart uses the [ngrok Kubernetes Operator](/k8s) and the ngrok agent to create a [Kubernetes-bound Endpoint](/gateway/kubernetes-endpoints/), which is accessible to other pods in your cluster as a native Service. ngrok then handles all routing to the upstream service, which can be running anywhere, like your development laptop or a cloud-based developer environment.
...
- An ngrok account
- The [ngrok CLI](/gateway/agent-cli-quickstart/#1-install-the-ngrok-agent-cli) installed on your local machine
...
- [Kubernetes Endpoints](/gateway/kubernetes-endpoints/)
