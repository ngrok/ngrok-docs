...
sidebarTitle: Rust
description: Learn how to get started with the ngrok Rust SDK to create secure tunnels from your Rust applications to the internet.
---

<Prompt description="Give this to your AI coding agent to complete the full quickstart automatically." actions={["copy", "cursor"]}>
Complete the ngrok Rust SDK quickstart:

1. Create a Rust HTTP server project at `ngrok-rust-server/` on port 8080 that responds "Hello from Rust HTTP Server!"
2. Create a second Rust project at `ngrok-rust-demo/` and add these dependencies to `Cargo.toml`: ngrok (0.14.0), tokio, axum, async-trait, hyper, hyper-util, and url
3. Implement `src/main.rs` using `ngrok::Session::builder().authtoken_from_env()` and `session.http_endpoint()` to tunnel port 8080 to a reserved domain with a Google OAuth traffic policy — hardcode the domain string directly in the source
4. Run `cargo run` in the server project in one terminal
5. Run `NGROK_AUTHTOKEN=<token> cargo run` in the demo project in another
6. Confirm the tunnel URL is printed and that visiting it requires a Google login

Ask me for my auth token and reserved domain before starting — the auth token is passed at runtime via env var, the domain is set directly in `src/main.rs`.
</Prompt>
...
You can think of it as the [ngrok Agent CLI](/gateway/endpoints/agent-cli-quickstart/) packaged as a Rust library.
...
<Steps>
  <Step title="Start your app or service">
  ...
  <Step title="Install the Rust SDK">
...
<Note>
This example uses ngrok's default Google OAuth application.
To use your own, see [the OAuth Traffic Policy Action documentation](/gateway/traffic-policy/actions/oauth/#google-example).
</Note>
>>>>>>> origin/main
...
- Dig deeper into [Traffic Policies](/gateway/traffic-policy/), which enable you to [grant conditional access](/gateway/traffic-policy/examples/add-authentication/#restrict-access-by-email-domain), [send custom headers](/gateway/traffic-policy/actions/add-headers/#examples), [rewrite URLs](/gateway/traffic-policy/actions/url-rewrite/#examples), and more with your endpoints.
