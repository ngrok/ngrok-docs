...
<Note>GoDaddy does not support CNAMEs as apex records. An apex record is a DNS record at the root (or apex) of a DNS zone. For example, in the DNS zone `example.com`, an apex record also has the fully qualified name `example.com` (this is sometimes called a naked domain). By convention, the relative name `@` is used to represent apex records.</Note>
