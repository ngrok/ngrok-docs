...
<img src="./img/mcp-server-ngrok-architecture.png" alt="Architecture diagram showing an MCP Host AI app routing through a ngrok Public Cloud Endpoint with Traffic Policy to an Internal Agent Endpoint and ngrok Agent serving an MCP Server Process" width="4200" height="1163" />
...
<Steps>
  <Step title="Install the ngrok Agent and configure internal Agent Endpoints">
...
=======
Internal endpoints are private endpoints that only receive traffic when forwarded through the [forward-internal Traffic Policy action](/gateway/traffic-policy/actions/forward-internal/).
This allows you to route traffic to an application through ngrok without making it publicly addressable.
Internal endpoint URL hostnames must end with `.internal`.
>>>>>>> origin/main
  ...
  <Step title="Create a public Cloud Endpoint">
    [Cloud Endpoints](../gateway/cloud-endpoints/) are persistent, always-on endpoints whose creation, deletion and configuration is managed centrally via the Dashboard or API.
...
=======
[Cloud Endpoints](/gateway/endpoints/cloud-endpoints/) are persistent, always-on endpoints whose creation, deletion and configuration is managed centrally via the Dashboard or API.
They exist permanently until they are explicitly deleted.
Cloud Endpoints do not forward their traffic to an agent by default and instead only use their attached Traffic Policy to handle connections.
>>>>>>> origin/main

  <Step title="Attach Traffic Policy to your Cloud Endpoint">
...
=======
Use ngrok's Traffic Policy to handle routing rules (forward-internal action above), rate limiting, and authentication via IP Intel.
A full list of available Traffic Policy actions can be seen [here](/gateway/traffic-policy/actions/).
These actions can be used singularly or layered on top of each other (executed sequentially from top to bottom).
>>>>>>> origin/main
  ...
  <Step title="Test your MCP gateway">
    ...
    <Prompt description="Try using this prompt on your agent to verify your MCP gateway is routing traffic: 'What tools do you have available in your current MCP configuration?'" actions={["copy", "cursor"]}>
    What tools do you have available in your current MCP configuration?
    </Prompt>
...
- To learn more, see the [Traffic Policy](/gateway/traffic-policy/) documentation.
