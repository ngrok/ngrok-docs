...
The configuration file and the ngrok executable can live anywhere on the machine, for example `/path/to/ngrok/ngrok.yml` and `/path/to/ngrok/ngrok`.
...
[Cloud Endpoints](/gateway/endpoints/cloud-endpoints/) are persistent, always-on endpoints whose creation, deletion, and configuration are managed centrally via the Dashboard or API.
...
Give it a public URL such as `https://mcp.example.com`.
