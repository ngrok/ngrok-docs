...
Because using FTP or FTPS (FTP over SSL) requires multiple dynamic ports for transferring data between server and client, ngrok is not a good solution for that traffic. It is recommended to use SFTP instead which relies on a single port number.
