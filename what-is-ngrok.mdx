...
- **API Gateway**: Use ngrok's [Traffic Policy](/gateway/traffic-policy/actions/) to secure, protect,
  accelerate and transform traffic to your production APIs.
- **Kubernetes Ingress**: Run the [ngrok Kubernetes Operator](/gateway/k8s/) to create ingress
  to Kubernetes services running in any cluster.
- **Identity-Aware Proxy**: Use ngrok's [OAuth](/gateway/traffic-policy/actions/oauth/),
  [JWT Validation](/gateway/traffic-policy/actions/jwt-validation/), or [OpenID
  Connect](/gateway/traffic-policy/actions/oidc/) actions to federate your
  app's authentication to an identity provider.
- **Load Balancer**: Use [Endpoint
  Pools](/gateway/endpoints/endpoint-pooling/) to load-balance traffic for
  scalability, failover, or to do blue/green and canary deployments.

### Ingress to external networks

- **APIs in customer networks**: Run the lightweight [ngrok agent](/gateway/agent/) or
  [Kubernetes Operator](/gateway/k8s/) in your customers' environments to securely
  connect to APIs in their networks without complex network configuration.
- **APIs on devices**: Run [ngrok as a service](/gateway/agent/#running-ngrok-in-the-background) on
...
- **Webhook Testing**: Run ngrok on your local machine to get a URL to receive
  ...
  requests](/gateway/agent/web-inspection-interface/) for fast development.
...
- **SSH**: Create [TCP endpoints](/gateway/endpoints/tcp/) to enable SSH access to remote machines.
- **RDP**: Create [TCP endpoints](/gateway/endpoints/tcp/) to enable RDP access to remote machines.
