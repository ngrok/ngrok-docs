...
description: Create public endpoints with hostnames matching your domain and configure domain-level behaviors.
---
...
You may create a Domain with a wildcard name, for example, `*.example.com`. A wildcard
...
<Note>
	Wildcard domains are available on our Pay-as-you-go plan self service. When
	you create a wildcard domain and run endpoints on the subdomains (for example,
	`*.foo.com` with endpoints `https://a.foo.com` and `https://b.foo.com`), we
	bill you endpoint hours for each individual endpoint, as well as custom domain
	hours for the wildcard domain.
</Note>

<Note>
	Reserving subdomains of a wildcard domain within the ngrok dashboard count
	towards the number of reserved domains in your account. For example, if you
	reserve `foo.example.com` and `*.example.com`, you have reserved two domains.
</Note>
...
```bash
ngrok http 80 --url 'https://${randomUrl}'
```

<Note>
	Random domain generation is only available on paid plans. The free plan is
	limited to your dev domain for public endpoints.
</Note>
