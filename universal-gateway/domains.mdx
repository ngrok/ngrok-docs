...
On the **free plan**, you can only use your automatically assigned dev domain for public endpoints. You cannot choose or reserve custom domain names—that capability requires a paid plan.
...
<Note>
Random domain generation (using `--url 'https://'`) is only available on paid plans. The free plan is limited to using your automatically assigned dev domain for public endpoints.
</Note>
