...
<Note>
Wildcard domains are available on our Pay-as-you-go plan self service. When you create a wildcard domain and run endpoints on the subdomains (for example, `*.foo.com` with endpoints `https://a.foo.com` and `https://b.foo.com`), we bill you endpoint hours for each individual endpoint, as well as custom domain hours for the wildcard domain.
</Note>
