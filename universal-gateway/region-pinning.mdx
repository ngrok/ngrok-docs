...
You can do this through the **API** or **dashboard**. 
...
2. **What happens if I forget to add the `ips` field when creating a Domain?**
    1. A: it defaults back to standard **GLOBAL** routing, where traffic is automatically routed through the *best option* from our physical [Points of Presence](/universal-gateway/points-of-presence). This is the baseline behavior for Domains, and is used by the vast majority of customers. 
3. **Do I have to specify every region manually?** 
    1. No: we provide regional aliases as a “shortcut” to select *multiple* ngrok Points of Presence within a geographical area. 
