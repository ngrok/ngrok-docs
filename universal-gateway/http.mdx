...
matching a wildcard pattern like `*.example.com`. To create a public wildcard endpoint, you must first reserve a domain with a wildcard pattern (see [domains](/universal-gateway/domains/#wildcard-endpoints)).
...
With the [Agent CLI](/agent/cli), you can create separate public endpoints on subdomains that match a wildcard endpoint pattern, no matter where the upstream services are running. See [the Domains docs](/universal-gateway/domains/#wildcard-endpoints) for more information.
