...
If you bring your own domain with a wildcard pattern (see [domains with wildcard patterns](/universal-gateway/domains#wildcard-endpoints), such as `*.example.com`), you must create a second `CNAME` record with your domain host for [wildcard TLS Certificate provisioning](/universal-gateway/tls-certificates/#wildcard-endpoints).
