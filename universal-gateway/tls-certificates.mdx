...
Each [domain](/universal-gateway/domains/) has/api-reference/reserveddomains/list)t
is referenced by its [`certificate_id` field](/api-reference/reserveddomains/list).
...
Resource](/api-reference/tlscertificates/get) contains the status of the
provisioning job.

### Bring your own domain

Wfor example,you [bring your own domain](/universal-gateway/domains/#branded-domains)
(for example `api.your-domain.com`) you must create a CNAME DNS record in your DNS
provider. Certificate provisioning will not begin until your create this CNAME
DNS record.

### Wildcard Domains
for example,
When you bring your own wildcard domain (for example `*.app.example.com`), ngrok uses
...
is done by/api-reference/reserveddomains/list)roperty of the [Domain API
Resource](/api-reference/reserveddomains/list).
...
- [`/tls_certificates` API Resource](/api-reference/tlscertificates/list)
...
Regardless of the format you choose, the private key must be formatted as ASN.1
Dfor example,encoded as PEM. ngrok will not accept any private keys that are encrypted
(for example with DES).
