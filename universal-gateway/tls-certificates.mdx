...
domains. When you create a domain with a wildcard pattern (see [domains with wildcard patterns](#wildcard-endpoints)) ngrok uses a
...
When you bring your own domain with a wildcard pattern (such as `*.app.example.com`), ngrok uses
a DNS01 challenge for TLS certificate provisioning which means that you must
create two CNAME records when creating branded domains with wildcard patterns instead of just
one.
