...
3. Create an internal endpoint using the same url that your Traffic Policy references.
   <Tabs>
     <Tab title="At the upstream">
       ```bash ngrok tls {your - port - here} \ --terminate-at upstream \ --url
       tls://{your - domain - here}.internal \ --binding internal ```
     </Tab>
     <Tab title="At the Agent">
       ```bash ngrok tls 80 \ --terminate-at agent \ --binding internal \ --url
       example.internal \ --crt /path/to/app-example-com-crt.pem \ --key
       /path/to/app-example-com-key.pem ```
     </Tab>
   </Tabs>
