...
<Tip>
  By using the same URL with `--pooling-enabled`, ngrok automatically creates an
  [Endpoint Pool](/universal-gateway/endpoint-pooling) that distributes traffic
  round-robin across all healthy instances with automatic failover.
</Tip>
...
<Tip>
  For a production PaaS alternative, consider using a [custom
  domain](/universal-gateway/custom-domains/) you own instead of an ngrok
  subdomain.
</Tip>
