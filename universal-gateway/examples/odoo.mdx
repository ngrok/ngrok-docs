...
This guide follows the style of the [n8n example](/universal-gateway/examples/n8n/) and adds Odoo-specific steps, including using a static domain and applying a Traffic Policy.
...
<Info>Why a static domain? Odoo stores and uses absolute links. A stable host prevents broken links in emails, reports, and third-party callbacks.</Info>
...
Odoo updates `web.base.url` based on the host used at login.
To keep it stable:
...
<Tip>You can combine rules—for example, applying Basic Auth for `/web*` and providing unrestricted access for public website paths.</Tip>
