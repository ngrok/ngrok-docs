...
- Layer in more authentication mechanisms, like the [`oauth`](/traffic-policy/actions/oauth) or [`basic-auth`](/traffic-policy/actions/basic-auth) actions using the same pattern of checking the subsequent action result variables with an expression.
