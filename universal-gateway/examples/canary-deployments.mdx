...
<Tabs>
  <Tab title="Random distribution">
    ...
    on_http_request:
      ...
      **What's happening here?** This policy first returns a random `double` between `0` and `1`.
    ...
    <Tab title="By header">
      ...
      **What's happening here?** This policy checks the incoming request for the presence of the `x-canary` and validates that its value is `true`.
