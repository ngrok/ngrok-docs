...
**What's happening here?** This policy first checks whether an HTTP request's URL contains the `/wp-admin` path.
If it does, then it applies the `restrict-ips` action to allow only the addresses you explicitly added.
