...
sidebarTitle: Expose n8n Workflows
...
ngrok works with the [self-hosted and enterprise editions](https://docs.n8n.io/hosting/) of n8n, including Docker, Docker Compose, and npm-based installs, to securely expose them to the public internet.
...
## Step 2: Run n8n + ngrok

Create a `docker-compose.yaml` file to run both n8n and the ngrok agent together. The example below uses environment variables for the domain you reserved earlier and your ngrok authtoken. If you don't already have an authtoken, you can see the instructions for getting one [here](/getting-started/installing-ngrok/#step-2-get-your-authtoken).
...
And create a `.env` file in the same directory with the following contents, replacing `NGROK_DOMAIN` and `NGROK_AUTHTOKEN` with your values:

```js .env highlight={1,2}
...
You can then start both services with:

```bash
docker compose up
```

It's worth reading the log output from the containers starting up, ngrok will print out an error message if there's anything wrong with your ngrok configuration.

## 3. Try out your n8n endpoint

You can now access your n8n instance at the domain you reserved earlier. Visit
...
services:
  ...
  ngrok:
    ...
    depends_on:
      - n8n
    env_file: .env
    command: http --url=${NGROK_DOMAIN} n8n:5678 # [!code --]
    command: http --url=${NGROK_DOMAIN} --traffic-policy-file /etc/n8n.yaml n8n:5678 # [!code ++]
    volumes: # [!code ++]
      - ./n8n.yaml:/etc/n8n.yaml # [!code ++]
    ports:
      - "4040:4040" # ngrok web UI
...
Restart your Docker Compose setup to apply the new policy.
...
This policy filters for _only_ traffic that contains the specific `/webhook/$WORKFLOW_UUID` path, then verifies that it both originates from GitHub (or another [supported provider](/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
</Info>
