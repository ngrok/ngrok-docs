...
For example, if you want to verify a webhook from GitHub, you can edit your `n8n.yaml` file (or else create one) and paste in the policy below.
...
**What's happening here?** This policy filters for _only_ traffic that contains the specific `/webhook/$WORKFLOW_UUID` path, then verifies that it both originates from GitHub (or another [supported provider](/traffic-policy/actions/verify-webhook/#supported-providers)) and contains the appropriate secret.
