...
**What's happening here?** This policy checks whether every TCP connection originates from an IP address matching one on the `allow` list.
If they match, the policy allows them connect to your ngrok agent and your Minecraft server, but if they don't, ngrok denies the request.
