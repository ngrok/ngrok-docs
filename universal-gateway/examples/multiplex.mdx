...
- Add authentication like [JWT
  validation](/traffic-policy/actions/jwt-validation) or [IP restrictions](/traffic-policy/actions/restrict-ips) to some or all of your multiplexed services, or even [combine multiple methods](/universal-gateway/examples/combine-auth-methods/) to let users authenticate with one `OR` another method.
