...
You can use any provider/issuer, but we have a guide for defining an API and [generating tokens with Auth0](/integrations/jwt-validation/auth0).
...
- Explore all the configuration options in the [`jwt-validate`](/traffic-policy/actions/jwt-validation) and [`oauth`](/traffic-policy/actions/oauth) actions.
