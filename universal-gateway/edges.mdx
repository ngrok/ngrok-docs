...
<Warning>
  Edges, Routes, Modules, Backends and Labeled Tunnels are deprecated and will
  be sunset on December 31st, 2025. See the [Migration Guide](#migration-guide).
</Warning>
...
Edges have been replaced with new features which are both simpler and more powerful
...
backend will balance traffic among all ngrok endpoints whose labels match its
label selector.
...
Agents and Agent SDKs can start labeled tunnels. Using the previous example of
...
| Module                                                                             | Description                                                                                                    |
| ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| [Circuit Breaker](/api-reference/edgeroutecircuitbreakermodule/get)                | Protect upstream services by rejecting traffic when they become overwhelmed.                                   |
| [Compression](/api-reference/edgeroutecompressionmodule/get)                       | Accelerate upstream services by compressing HTTP response bodies with gzip or deflate.                         |
| [IP Restrictions](/api-reference/edgerouteiprestrictionmodule/get)                 | Allow or deny traffic based on the source IP of connections.                                                   |
| [Mutual TLS](/api-reference/httpsedgemutualtlsmodule/get/)                         | Enforce mutual TLS auth with a configured set of CAs.                                                          |
| [OAuth](/api-reference/edgerouteoauthmodule/get/)                                  | Enforce an OAuth flow to well-known IdPs like Google, optionally authorizing users by domain or email address. |
| [OpenID Connect](/api-reference/edgerouteoidcmodule/get/)                          | Enforce an OpenID Connect flow to a federated IdP.                                                             |
| [Request Headers](/api-reference/edgerouterequestheadersmodule/get/)               | Add or remove headers from HTTP requests before they are sent to your upstream service.                        |
| [Response Headers](/api-reference/edgerouteresponseheadersmodule/get/)             | Add or remove headers from HTTP responses before they are returned to the client.                              |
| [SAML](/api-reference/edgeroutesamlmodule/get)                                     | Enforce a SAML flow to a federated IdP, optionally authorizing users by group.                                 |
| [TLS Termination](/api-reference/httpsedgemutualtlsmodule/get/)                    | Customize TLS termination behavior, like the minimum supported version of TLS.                                 |
| [Traffic Policy](/api-reference/edgeroutetrafficpolicymodule/get)                  | Influence and control traffic to and from your upstream service by configuring a powerful policy engine.       |
| [User Agent Filter](/api-reference/edgerouteuseragentfiltermodule/get/)            | Block bots or browsers with rules that allow or deny HTTP requests based on `User-Agent` header.               |
| [Webhook Verification](/api-reference/edgeroutewebhookverificationmodule/get/)     | Restrict access by verifying HTTP requests are signed by a webhook provider like Slack or GitHub.              |
| [Websocket TCP Converter](/api-reference/edgeroutewebsockettcpconvertermodule/get) | Convert binary websocket connections to backend TCP connections.                                               |
...
This primitive is called a Route. Each Route is defined using a path selector,
...
| Module                                                           | Description                                                                                                          |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [Mutual TLS](/api-reference/tlsedgemutualtlsmodule/get)          | Perform mutual TLS authentication when terminating TLS connections with a configured set of certificate authorities. |
| [TLS](/api-reference/tlsedgetlsterminationmodule/get)            | Configure whether ngrok terminates TLS traffic at its edge or forwards the TLS traffic through unterminated.         |
| [IP Restrictions](/api-reference/tlsedgetrafficpolicymodule/get) | Allow or deny traffic based on the source IP of connections                                                          |
...
| Module                                                           | Description                                                                                              |
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| [IP Restrictions](/api-reference/tcpedgeiprestrictionmodule/get) | Allow or deny traffic based on the source IP of connections                                              |
