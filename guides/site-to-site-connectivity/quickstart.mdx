...
<Tip>
  Running in K8s? Check out [this
  guide](/guides/site-to-site-connectivity/customer-networks) on securely
  connecting your local cluster to customer networks.
</Tip>
...
<Tabs>
  <Tab title="Using ngrok.yml">```bash ngrok start --all ```</Tab>
  <Tab title="Using a different config file">
    ```bash ngrok start --config /path/to/ngrok.yml --all ```
  </Tab>
</Tabs>
