...
sidebarTitle: End Customer Guide
...
Our customers include Twilio, Okta, Zoom, Microsoft, Zendesk, Cyera, and many more.
...
API](/api-reference/abusereports/create). Our abuse response team investigates
...
ngrok also logs changes to account configurations, such as agent start/stop, API key creation, and more. You can see the list in our [Log Store](/obs/events/reference/) documentation.
...
By default, the ngrok agent attempts to connect to the default Connect URL at `connect.ngrok-agent.com:443`, which resolves to a dynamic list of IP addresses. You can view the full list of addresses and IPs available for the agent to connect to in our [AWS-hosted `tunnel.json` file](https://s3.amazonaws.com/dns.ngrok.com/tunnel.json).
