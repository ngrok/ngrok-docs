...
![Architecture diagram showing ngrokd in a local network connecting through Private Agent Endpoints in ngrok Cloud to an ngrok agent serving REST API and Postgres DB in a remote customer network](./img/ngrokd-site-to-site-architecture.png)
...
![ngrok dashboard New Service User dialog with a name field for creating an isolated service account per customer](./img/new-service-user-dialog.png)

Next, create an authtoken assigned to this specific service user.

![ngrok dashboard New Authtoken dialog with description and owner assigned to a Service User for isolated network access](./img/new-authtoken-service-user-dialog.png)
...
![ngrokctl list output showing Discovered Bound Endpoints with TCP and HTTP endpoints assigned local virtual addresses](./img/ngrokctl-list-discovered-bound-endpoints.png)
