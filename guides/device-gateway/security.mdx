...
Each device or site gets its own authtoken scoped with [ACL bindings](/agent/authtokens/#acl-bindings).
If one token is compromised, revoke it immediately without affecting the rest of your fleet:

- **Create** per-device or per-site tokens via the [Authtokens API](/api/resources/authtokens/)
