...
- **Per-device auth tokens** scoped with [ACL bindings](/agent/authtokens/#acl-bindings) so each token can only create endpoints on its assigned domain
...
- **Create** tokens via the [Authtokens API](/api/resources/authtokens/)
...
This keeps your attack surface small and means you only pay for active endpoints.
...
- Use [Event Subscriptions](/obs/event-subscriptions/) to stream events to your logging infrastructure
