...
- An ngrok agent configured on your local machine. See the [Agent CLI Quickstart](/gateway/agent-cli-quickstart/) for instructions on how to install the ngrok agent.
...
An [internal endpoint](/gateway/internal-endpoints/) enables a service inside the factory network to be reachable
...
A [Cloud Endpoint](/gateway/cloud-endpoints/) is a permanent, externally accessible entry point into the factory
network that's also:
