...
A Service User does not belong to a particular user account.
Run the following command to create a new Service User, providing your API key and a description:
...
The application persists tunnels in a local JSON file, `ngrok_config.json`, and recreates them when the application is restarted.
You could extend this application to store tunnel configuration in a backend database.
