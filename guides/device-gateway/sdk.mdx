...
![AWS Route 53 Quick create record form showing a wildcard CNAME record for the device gateway subdomain pointing to the ngrok CNAME target](./img/aws-route53-cname-record-device-gateway.png)
...
![ngrok dashboard Domains page showing a reserved domain's detail panel with the DNS Targets 2 targets link highlighted](./img/dashboard-domain-dns-targets-2-targets.png)

5. Click **Check Status**, as denoted by the arrow in the screenshot below.

![ngrok dashboard DNS Targets panel showing CNAME and TLS record values with the Check Status button highlighted](./img/dashboard-domain-dns-check-status.png)

6. You should see a successful response similar to the screenshot below.

![ngrok dashboard DNS Targets panel showing successful CNAME and TLS record validation with green checkmarks](./img/dashboard-domain-dns-validation-success.png)
...
![ngrok dashboard New Authtoken dialog with a bot user as owner and a wildcard domain ACL binding rule](./img/new-authtoken-bot-user-acl-rules.png)
