...
description: Step-by-step guide for manually migrating deprecated ngrok Edge configurations to Cloud Endpoints using Traffic Policies.
---
...
| Edge Type | Replaced By | Policy Phases Used | | --------- | ------------------------------- | ------------------------------------- | | HTTPS | Cloud Endpoint + Agent Endpoint | `on_http_request`, `on_http_response` | | TLS/TCP | Cloud Endpoint + Agent Endpoint | `on_tcp_connect` |
...
```yaml
{ "https_edges": [...], "next_page_uri": "/edges/https?page=2" }
```

Be sure to keep your [API rate limits](/api/#rate-limits) in mind.
...
| Edge Type | URL Format | | --------- | ---------------------------- | | HTTPS | `https://yourdomain.com` | | TLS | `tls://yourdomain.com:443` | | TCP | `tcp://yourdomain.com:12345` |
...
| Match Type | CEL Expression | | ------------- | --------------------------------- | | `exact_path` | `req.url.path == "/foo"` | | `path_prefix` | `req.url.path.startsWith("/api")` |
...
| Module | Actions | | ------------------------- | -------------------------------------------------------- | | `oauth` | `oauth` + `set-vars` + `custom-response` for restriction | | `oidc` | `oidc` with optional auth_id + session durations | | `ip_restrictions` | `restrict-ips` | | `request_headers` | `add-headers`, `remove-headers` | | `response_headers` | `add-headers`, `remove-headers` | | `circuit_breaker` | `circuit-breaker` | | `webhook_verification` | `verify-webhook` | | `compression` | `compress-response` | | `user_agent_filter` | `set-vars` + `deny` (based on user-agent match) | | `websocket_tcp_converter` | ⚠️ Not supported |
...
Your edge’s backend defines where traffic goes.
These should be placed **after all other actions you defined above.
These should be added to the end of the phase.**

### Tunnel group backends

If the edge uses a `tunnel_group` backend (identified by labels):

- Construct an internal domain for each label (for example, `service=app` → `service-app.internal`)
...
Here, `forward-internal` is defined to forward to [`https://service-app.internal`](https://service-app.internal) and when an error occurs (like the endpoint being offline) fallback to some custom text (by leveraging `on_error: continue` and `custom-response`) describing how to get back online (this is optional).
...
Here, an Agent Endpoint has been started with the URL [`https://service-app.internal`](https://service-app.internal) which points to a service running locally on the machine on port 80.

### TCP/TLS backends

These are generally Tunnel Groups and should follow the same rules as above.
However, wanted to call out that you should use the `on_tcp_connect` phase here instead:
...
| Step | Done? | | ------------------------------------------------------------- | ----- | | Listed all edges and their `hostports` | | | Converted modules to policy actions | | | Converted backends to `forward-internal` or `custom-response` | | | Created necessary internal endpoints | | | Created Cloud Endpoints | | | Deleted the old edge | |
