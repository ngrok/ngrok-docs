...
Setting up a custom Connect URL can be useful because it ensures that no one can bring their own ngrok account. In this case, the network admins could continue to block traffic to our default Connect URLs and only allow your custom branded Connect URL. For this, you'll need to:

- Set up a [custom Connect URL in your ngrok Dashboard](https://dashboard.ngrok.com/tunnels/ingress)
- Edit your ngrok agent configuration file with a [`connect_url`](/agent/config/v3/#connect_url) parameter, set to your custom Connect URL
