...
- The update URL (the default is `update.ngrok-agent.com`), so that the ngrok agent can quickly update itself. Agent versions `3.35.0` and earlier use `update.equinox.io` as the update URL.
