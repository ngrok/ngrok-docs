...
![Architecture diagram showing a centralized Cloud Endpoint with Traffic Policy routing traffic via wildcard domains to developer internal endpoints on individual machines](./img/secure-dev-productivity-architecture.png)
...
![ngrok dashboard Invite Team Member dialog showing RBAC permission options for Developer, Team, Billing, and Admin roles](./img/invite-team-member-rbac-dialog.png)
...
![ngrok authtoken ACL Rules field showing a bind:alias1.internal rule that restricts the token to a specific internal endpoint](./img/authtoken-acl-rule-bind-internal.png)
...
![ngrok New Domain dialog with a wildcard domain *.devtest.example.com entered for reserving a custom subdomain for the developer Cloud Endpoint](./img/new-wildcard-domain-dialog.png)
