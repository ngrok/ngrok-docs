...
- If your local service is not running on the same machine as the ngrok agent, it is recommended to set up TLS encryption for the ngrok agent to the upstream service leg of the tunnel using the [local HTTPS feature](/gateway/http/#https-forwarding).
- For custom domains, use ngrok's [Automated TLS certificates](/gateway/tls-certificates/#automated) to have ngrok automatically provision a TLS certificate for your endpoint from [Let's Encrypt](https://letsencrypt.org/about/).
...
See [Terminating TLS Connections](/gateway/tls-termination/) for more details.
