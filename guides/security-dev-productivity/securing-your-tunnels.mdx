...
<Tip>Certificates automatically provisioned from Let's Encrypt are securely managed, and their private keys are not accessible to the requester. This ensures the security and integrity of the certificate by preventing key exposure.</Tip>

### Using a custom connect URL

If your organization uses a custom Connect URL, your default ngrok configuration will not work.
Edit your ngrok agent configuration to add a [`connect_url`](/agent/config/v3/#connect-url) parameter to use the custom Connect URL of your organization.
...
It is possible to specify the minimum TLS version that clients are required to use to talk to the ngrok cloud service for your endpoint.
See [the mTLS guide](/agent/agent-mutual-tls-termination/) for more details.

### Observability

ngrok provides functionality for consuming logs for events in the system.
For more information, check out the [Log Exporting documentation](/obs/).

### Blocking non-corporate accounts

For a tighter security policy, most administrators will want to block ngrok accounts that are not a part of their organization.
The fastest approach is to block DNS lookups to the URLs used by the agent to connect to the ngrok cloud.
These are the DNS lookups to block:
...
You need an API key to authenticate with [the ngrok REST API](/api/).
API keys are Base64 encoded strings that you can find [in the dashboard](https://dashboard.ngrok.com/get-started/your-authtoken) or from [an API endpoint](/api-reference/apikeys/list), which makes it easy to rotate your API keys.

Consider restricting the IPs permitted to access the API.
You can do so [in the ngrok Dashboard](https://dashboard.ngrok.com/ip-restrictions).
