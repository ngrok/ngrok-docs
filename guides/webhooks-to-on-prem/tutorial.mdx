...
Webhooks hit one public address that you configure for your cloud endpoint. A single Traffic Policy on that address checks the signature on every request, rejects anything that fails, and only forwards verified requests. The internal service that receives webhooks connects outbound to ngrok; it never accepts inbound connections and is never publicly addressable.
...
Repeat for every service that will receive webhook, e.g., `notification-service.internal` and `deployment-service.internal`.
