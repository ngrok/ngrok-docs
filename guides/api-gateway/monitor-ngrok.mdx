...
- Browse to the ngrok [Traffic Inspector](https://dashboard.ngrok.com/traffic-inspector) on your ngrok account dashboard.
  ...
  ![ngrok Traffic Inspector showing a list of captured HTTP requests with status codes and response times](./img/monitor-ngrok/ngrok-traffic-inspector-request-list.png)
...
- Click a request in the Traffic Inspector list, then click **Enable full capture** in the sidebar.
  ...
  ![ngrok Traffic Inspector with the Enable Full Capture toggle highlighted](./img/monitor-ngrok/traffic-inspector-enable-full-capture-toggle.png)

- This button takes you to your account settings, where you can enable full capture under **Observability**.

  ![ngrok account settings page with the Enable Full Capture option enabled](./img/monitor-ngrok/account-settings-enable-full-capture.png)
...
- Click the **Replay** button and notice that the request is replayed from the ngrok host.
  ...
  ![ngrok Traffic Inspector replay button for resending a captured HTTP request](./img/monitor-ngrok/traffic-inspector-replay-request.png)

- You can also replay a request with changes to alter any of the headers or POST data.

  ![ngrok Traffic Inspector replay editor with modified request headers](./img/monitor-ngrok/traffic-inspector-replay-modified-request.png)
...
- In Traffic Inspector, note that ngrok returned the error code `429` (rate limit) and that the duration of the request was instant, meaning it created no load on your API.
  ...
  ![ngrok traffic policy rate limit configuration using a sliding window algorithm on POST and PUT requests](./img/monitor-ngrok/traffic-policy-rate-limit-config.png)
...
- In the Traffic Inspector, click one of the `429` events, then click **Replay**.
  ...
  ![ngrok Traffic Inspector replaying a request after adjusting the rate limit policy](./img/monitor-ngrok/traffic-inspector-replay-after-rate-limit.png)
...
- In the ngrok dashboard, browse to [**Log Exporting**](https://dashboard.ngrok.com/log-exporting) and add a new Log Export.
  ...
  ![ngrok dashboard event subscription creation form with destination and description fields](./img/monitor-ngrok/ngrok-event-subscription-create-form.png)

- In the **New Log Export** sidebar, enter `traffic` as the **Description** and then **Add** a new log source.
  Choose `http_request_complete`.

  ![ngrok event subscription form adding an HTTP request log source type](./img/monitor-ngrok/ngrok-event-subscription-add-log-source.png)
...
- Open the Datadog site and browse to the **Log Explorer** page from the navigation panel.
  ...
  ![Datadog Log Explorer showing incoming ngrok HTTP request events](./img/monitor-ngrok/datadog-ngrok-event-logs.png)
...
- In the Datadog site, browse to the **Dashboards > Dashboard List** page from the navigation panel.
  ...
  ![Datadog dashboard list showing available monitoring dashboards](./img/monitor-ngrok/datadog-dashboard-list.png)

You need to clone the default dashboard to add a new widget to the dashboard.

- Click **Clone** at the top right of the page.

  ![Datadog default ngrok monitoring dashboard with request metrics and status code graphs](./img/monitor-ngrok/datadog-default-ngrok-dashboard.png)

- In the cloned dashboard, click **Add widgets**.

  ![Datadog dashboard in edit mode with the add widget panel open](./img/monitor-ngrok/datadog-add-widget-panel.png)
...
- 4. Name the widget `Errors in the last hour`.
  ...
  ![Datadog widget editor showing query configuration for graphing ngrok request metrics](./img/monitor-ngrok/datadog-widget-query-editor.png)
...
![Datadog custom dashboard displaying ngrok request rates and error metrics](./img/monitor-ngrok/datadog-custom-ngrok-dashboard.png)
...
- In Datadog, browse to **Integrations > Add integration > Webhooks**.
  ...
  ![Datadog webhook integration settings page for forwarding ngrok events](./img/monitor-ngrok/datadog-add-webhook-integration.png)
...
- Under **Webhooks**, at the bottom of the configuration page, click **New**.
  ...
  ![Datadog webhook configuration form with URL and name fields for ngrok event delivery](./img/monitor-ngrok/datadog-webhook-url-configuration.png)
...
- In the monitor configuration, choose **Query** for your monitor scope, set the search query to `@http.status_code:500`, the time to the **last 1 hour**, and the notification message:
  ...
  ![Datadog Logs monitor configuration with an HTTP 500 status code query and notification message](./img/monitor-ngrok/datadog-logs-monitor-http500-configuration.png)
...
- Notice that the alert appears on the ntfy page.
  ...
  ![Datadog monitor notification settings showing a webhook notification channel](./img/monitor-ngrok/datadog-monitor-notification-settings.png)
