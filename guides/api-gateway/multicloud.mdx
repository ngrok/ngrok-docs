...
2. Set up a single hostname, attached to a [Cloud Endpoint](/universal-gateway/cloud-endpoints/) that routes traffic to services on multiple clouds based on pathname, an internal [Agent
   Endpoint](/universal-gateway/agent-endpoints) then receives in your cluster.
...
- **Two or more cloud providers**.
  - These are referred to as `cloud A` and `cloud B`.
...
```
{"message":"Hello from `abc` on cloud A!","host":"$YOUR_DOMAIN","req_headers":{"host":"$YOUR_DOMAIN","user-agent":"curl/8.7.1","accept":"*/*","accept-encoding":"gzip"},"method":"GET","url":"/abc","time":"2025-03-04T01:10:25.812Z","status":200}
```

And when you run `curl https://$YOUR_DOMAIN/xyz`?

```
{"message":"Hello from `xyz` on cloud B!","host":"$YOUR_DOMAIN","req_headers":{"host":"$YOUR_DOMAIN","user-agent":"curl/8.7.1","accept":"*/*","accept-encoding":"gzip"},"method":"GET","url":"/xyz","time":"2025-03-04T01:10:25.812Z","status":200}
```
...
For example, one of your services (like `abc-app`, if you're following along with the demo service) may need additional protection from unintentional misuse and malicious attacks.
...
2. Explore other opportunities to manage and take action on API traffic in the
   ...
   performing at all times, and be alerted to errors.
3. Learn [how to monitor your API traffic](/guides/api-gateway/monitor-ngrok/), so you'll know how your service is performing at all times, and be alerted to errors.
