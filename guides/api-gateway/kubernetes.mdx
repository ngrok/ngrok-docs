...
- **A Kubernetes cluster**—see the [local cluster options](/k8s/guides/local-cluster) doc for some options.
...
If you want to quickly wire up a POC using ngrok, use the [demo
...
The following manifests create two different but similar deployments that
traffic will be directed to. Using the container environment variables section, they
...
You can check out the [installation doc](/k8s/installation/helm) for more details and
...
<Tabs>
    ...
    <Tab>
    	...
    	```
    		...
    		cloud endpoint. This guide refers to it as `<CLOUD_ENDPOINT_ID>`.
...
For example, one of your services (like `abc`, if you're following along with the demo service) may need additional protection from unintentional misuse and malicious attacks.
...
There's still a lot more to explore with ngrok's API gateway. Next up:
...
4. Learn [how to monitor your API traffic](/guides/api-gateway/monitor-ngrok/), so you'll know how
