...
You've developed an API server, deployed it to your Kubernetes cluster, and now
...
- **A Kubernetes cluster**—see our [local cluster options](/k8s/guides/local-cluster) doc for some options.
- **A domain**: This can be your free dev domain, or if you're on a paid plan, one that you choose [in the ngrok
  ...
  API](/api/resources/reserved-domains/#create-reserved-domain). Pay-as-you-go plans allow you to bring your own [custom branded
...
You can check out our [installation doc](/k8s/installation/helm) for more details and
...
<Tabs>
    ...
    <Tab>
    		...
    		API](/api), which you can use to create a cloud endpoint and
    		...
    		Create a cloud endpoint on `$YOUR_DOMAIN`, passing your
...
Enter our [Traffic Policy system](/traffic-policy), which lets you filter
...
guide](/integrations/jwt/auth0-jwt.mdx) with more details.
...
3. Extend your API gateway into [multicloud](/guides/api-gateway/multicloud)
   by deploying the Kubernetes Operator on a second (or third) cluster and
   provisioning an `AgentEndpoint` resource.
4. Learn [how to monitor your API traffic](/guides/api-gateway/other-guides/monitor-ngrok/), so you'll know how
   your service is performing at all times, and be alerted to errors.
